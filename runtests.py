...
def main(argv):
    ...
    parser.add_argument(
        "--verbose", "-v", action="count", default=1, help="more verbosity"
    )
    parser.add_argument(
        "--no-build",
        "-n",
    ...
    parser.add_argument(
        "--doctests", action="store_true", default=False, help="Run doctests in module"
    )
    parser.add_argument(
        "--refguide-check",
        action="store_true",
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "produce HTML for C code coverage information "
    ...
    parser.add_argument(
        ...
        default="fast",
        help=(
            "'fast', 'full', or something that could be "
    ...
    parser.add_argument(
        "--pythonpath", "-p", default=None, help="Paths to prepend to PYTHONPATH"
    )
    parser.add_argument("--tests", "-t", action="append", help="Specify tests to run")
    parser.add_argument(
        "--python", action="store_true", help="Start a Python shell with PYTHONPATH set"
    )
    parser.add_argument(
        "--ipython",
        "-i",
    ...
    parser.add_argument(
        "--shell", action="store_true", help="Start Unix shell with PYTHONPATH set"
    )
    ...
    parser.add_argument(
        "--bench", action="store_true", help="Run benchmark suite instead of test suite"
    )
    parser.add_argument(
        "--bench-compare",
        action="append",
    ...
    parser.add_argument(
        "--mypy", action="store_true", default=False, help="Run mypy on the codebase"
    )
    parser.add_argument(
        "--doc",
        action="append",
    ...
    if args.bench:
        ...
        if not args.bench_compare:
            ...
            print(
                "Running benchmarks for Scipy version %s at %s"
    ...
    shutil.copyfile(
        os.path.join(ROOT_DIR, ".coveragerc"), os.path.join(test_dir, ".coveragerc")
    )
...
def build_project(args):
    ...
    if not all(root_ok):
        print(
            "To build the project, run runtests.py in git checkout or unpacked source"
        )
        sys.exit(1)
...
def run_asv(cmd):
    cwd = os.path.join(os.path.abspath(os.path.dirname(__file__)), "benchmarks")
    # Always use ccache, if installed
    env = dict(os.environ)
    env["PATH"] = os.pathsep.join(EXTRA_PATH + env.get("PATH", "").split(os.pathsep))
