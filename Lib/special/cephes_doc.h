...
#define betaq_doc "betaq(a,b,p) returns the pth quantile of the beta distribution.  It is\neffectively the inverse of betacdf returning the value of x for which \nbetacdf(a,b,x) = p.   SEE ALSO betaincinv"
#define binomcdf_doc "binomcdf(k,n,p) returns the sum of the terms 0 through k of the\nBinomial probability density:  sum(nCj p**j (1-p)**(n-j),j=0..k)"
#define binomcdfc_doc "binomcdfc(k,n,p) returns the sum of the terms k+1 through n of the\nBinomial probability density: sum(nCj p**j (1-p)**(n-j), j=k+1..n)"
#define binomcdfinv_doc "binominv(k,n,y) finds the probability p such that the sum of the\nterms 0 through k of the Binomial probability density is equal to the\ngiven cumulative probability y."
#define cbrt_doc "cbrt(x) returns the real cube root of x."
#define chi2cdf_doc "chi2cdf(v,x) Returns the area under the left hand tail (from 0 to x) of the Chi\nsquare probability density function with v degrees of freedom:\n1/(2**(v/2) * gamma(v/2)) * integral(t**(v/2-1) * exp(-t/2), t=0..x)"
#define chi2cdfc_doc "chdtrc(v,x) returns the area under the right hand tail (from x to\ninfinity) of the Chi square probability density function with v\ndegrees of freedom:\n1/(2**(v/2) * gamma(v/2)) * integral(t**(v/2-1) * exp(-t/2), t=x..inf)"
#define chi2p_doc "chi2p(v,p) returns the argument x such that chi2cdfc(v,x) is equal\nto p."
...
#define fcdf_doc "fcdf(dfnum,dfden,x) returns the area from zero to x under the F density\nfunction (also known as Snedcor's density or the variance ratio\ndensity).  This is the density of X = (unum/dfnum)/(uden/dfden), where unum and\nuden are random variables having Chi square distributions with dfnum and\ndfden degrees of freedom, respectively."
#define fcdfc_doc "fcdfc(dfnum,dfden,x) returns the area from x to infinity under the F\ndensity function."
#define fp_doc "fp(dfnum,dfden,p) finds the F density argument x such that the\nintegral from x to infinity of the F density is equal to the given\nprobability p."
...
#define gammacdf_doc "gammacdf(a,b,x) returns the integral from zero to x of the gamma\nprobability density function: a**b / gamma(b) * integral(t**(b-1) exp(-at),t=0..x).\nThe arguments a and b are used differently here than in other definitions."
#define gammacdfc_doc "gammacdfc(a,b,x) returns the integral from x to infinity of the gamma\nprobability density function.  SEE gammacdf, gammaq"
...
#define gammaln_doc "gammaln(x) returns the base e logarithm of the absolute value of the\ngamma function of x: ln(|gamma(x)|)"
#define gammaq_doc "gammaq(a,b,p) returns pth quantile of the gamma distribution.  It is \nthe inverse of the gamma cdf returning the value of x for which \ngamcdf(b,a,x) = p."
...
#define kolmogorovcdfc_doc "kolmogorovcdfc(y) returns Kolmogorov's limiting distribution of a\ntwo-sided test or probability that sqrt(n) * max deviation > y."
#define kolmogorovp_doc "kolmogorovp(p) returns y such that kolmogorovcdfc(y) = p"
...
#define negbinomcdf_doc "negbinomcdf(k,n,p) returns the sum of the terms 0 through k of the\nnegative binomial distribution: sum((n+j-1)Cj p**n (1-p)**j,j=0..k).\nIn a sequence of Bernoulli trials this is the probability that k or\nfewer failures precede the nth success."
#define negbinomcdfc_doc "negbinomcdfc(k,n,p) returns the sum of the terms k+1 to infinity of the\nnegative binomial distribution."
#define negbinomcdfinv_doc "negbinomcdfinv(k,n,y) finds the argument p such that negbinom(k,n,p) is equal\nto y."
#define normalcdf_doc "normalcdf(x) returns the area under the Gaussian probability density\nfunction, integrated from minus infinity to x:\n1/sqrt(2*pi) * integral(exp(-t**2 / 2),t=-inf..x)"
#define normalq_doc "normalq(y) returns the argument x for which the area udnder the\nGaussian probability density function (integrated from minus infinity\nto x) is equal to y."
#define poissoncdf_doc "poissoncdf(k,m) returns the sum of the first k terms of the Poisson\ndistribution: sum(exp(-m) * m**j / j!, j=0..k) = gammaincc( k+1, m).\nArguments must both be positive and k an integer."
#define poissoncdfc_doc "poissoncdfc(k,m) returns the sum of the terms from k+1 to infinity of the\nPoisson distribution: sum(exp(-m) * m**j / j!, j=k+1..inf) = gammainc( k+1, m).\nArguments must both be positive and k an integer."
#define poissoncdfinv_doc "pdtri(k,y) returns the Poisson variable m such that the sum\nfrom 0 to k of the Poisson density is equal to the given probability\ny:  calculated by gammaincinv( k+1, y).  k must be a nonnegative integer and\ny between 0 and 1."
...
#define smirnovcdfc_doc "smirnovcdfc(n,e) returns the exact Smirnov statistic for a one-sided\ntest.  It is equal to the probability that the maximum absolute difference between a\ntheoretical distribution and an empirical one based on n samples is greater than e."
#define smirnovp_doc "smirnovp(n,y) returns e such that smirnovcdfc(n,e) = y."
...
#define studentcdf_doc "studentcdf(k,t) returns the integral from minus infinity to t of the Student t\ndistribution with integer k > 0 degrees of freedom:\ngamma((k+1)/2)/(sqrt(k*pi)*gamma(k/2)) * integral((1+x**2/k)**(-k/2-1/2),x=-inf..t)"
#define studentq_doc "studentq(k,p) returns the argument t such that studentcdf(k,t) is equal to p."
