...
C                Mathieu functions for m ף 12  or q ף 300 or
C                q ע m*m
...
C                to x, and Y0(t)/t with respect to t from x to ל
C       Input :  x   --- Variable in the limits  ( x ע 0 )
C       Output:  TTJ --- Integration of [1-J0(t)]/t from 0 to x
C                TTY --- Integration of Y0(t)/t from x to ל
C       ==========================================================
C
        IMPLICIT DOUBLE PRECISION (A-H,O-Z)
        PI=3.141592653589793D0
...
C       Input :  x  --- Argument of ג(x)  ( |x| ף 1 )
C       Output:  GA --- ג(x)
C       ================================================
C
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)
...
C       Input :  x   --- Argument of L1(x) ( x ע 0 )
C       Output:  SL1 --- L1(x)
C       ================================================
C
        IMPLICIT DOUBLE PRECISION (A-H,O-Z)
...
C       Input :  x   --- Argument of L0(x) ( x ע 0 )
C       Output:  SL0 --- L0(x)
C       ================================================
C
        IMPLICIT DOUBLE PRECISION (A-H,O-Z)
...
C                to x, and K0(t)/t with respect to t from x to ל
C       Input :  x   --- Variable in the limits  ( x ע 0 )
C       Output:  TTI --- Integration of [I0(t)-1]/t from 0 to x
C                TTK --- Integration of K0(t)/t from x to ל
C       =========================================================
C
        IMPLICIT DOUBLE PRECISION (A-H,O-Z)
        PI=3.141592653589793D0
...
C                point for backward recurrence
C            (2) GAM0 for computing gamma function (|x| ף 1)
C       =========================================================
C
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)
	DIMENSION VL(0:*),DL(0:*)
...
C                 in the interval [0,ל], and the corresponding
...
C       Input :  x  --- Upper limit of the integral ( x ע 0 )
...
C       Input :  x   --- Argument of H1(x) ( x ע 0 )
C       Output:  SH1 --- H1(x)
C       =============================================
C
        IMPLICIT DOUBLE PRECISION (A-H,O-Z)
...
C       Input  : HK  --- Modulus k ( 0 ף k ף 1 )
...
C                 k  --- Modulus   ( 0 ף k ף 1.0 )
C                 c  --- Parameter ( 0 ף c ף 1.0 )
...
C       Input :  x   --- Argument of H0(x) ( x ע 0 )
C       Output:  SH0 --- H0(x)
C       =============================================
C
        IMPLICIT DOUBLE PRECISION (A-H,O-Z)
...
C                respect to t from 0 and x ( x ע 0 )
...
C                and their derivatives
C       Input:   x --- Argument of In(x) and Kn(x) ( 0 ף x ף 700 )
C                n --- Order of In(x) and Kn(x)
C       Output:  BI(n) --- In(x)
C                DI(n) --- In'(x)
C                BK(n) --- Kn(x)
...
C
C       =======================================================
C       Purpose: Compute modified Fresnel integrals Fס(x) 
C                and Kס(x)
C       Input :  x   --- Argument of Fס(x) and Kס(x)
C                KS  --- Sign code
...
C       Input :  x   --- Argument of Pm(x)  ( -1 ף x ף 1 )
C                m   --- Order of Pmv(x)
C                v   --- Degree of Pmv(x)
C       Output:  PMV --- Pmv(x)
...
C
C       =========================================================
C       Purpose: Compute the gamma function ג(z) or ln[ג(z)]
...
C                       KF=0 for ln[ג(z)]
C                       KF=1 for ג(z)
C       Output:  GR --- Real part of ln[ג(z)] or ג(z)
C                GI --- Imaginary part of ln[ג(z)] or ג(z)
C       ========================================================
C
        IMPLICIT DOUBLE PRECISION (A-H,O-Z)
        DIMENSION A(10)
...
C
C       ==================================================
C       Purpose: Compute gamma function ג(x) or ln[ג(x)]
C       Input:   x  --- Argument of ג(x) ( x > 0 )
C                KF --- Function code
C                       KF=1 for ג(x); KF=0 for ln[ג(x)]
C       Output:  GL --- ג(x) or ln[ג(x)]
C       ==================================================
C
        IMPLICIT DOUBLE PRECISION (A-H,O-Z)
        DIMENSION A(10)
...
C
C       ======================================================
C       Purpose: Compute parabolic cylinder functions W(a,סx)
C                and their derivatives
C       Input  : a --- Parameter  ( 0 ף |a| ף 5 )
C                x --- Argument of W(a,סx)  ( 0 ף |x| ף 5 )
C       Output : W1F --- W(a,x)
C                W1D --- W'(a,x)
C                W2F --- W(a,-x)
C                W2D --- W'(a,-x)
...
C                with respect to t from 0 to x ( x ע 0 )
C       Input :  x  --- Upper limit of the integral
...
C       Input :  n  --- Order of Bessel functions ( n ף 101 )
...
C                 in the interval [-ל,ל], and the corresponding
...
C       Input :  x --- Argument of kn(x)  ( x ע 0 )
...
C       ============================================
C       Purpose: Compute exponential integral En(x)
C       Input :  x --- Argument of En(x) ( x ף 20 )
C                n --- Order of En(x)
C       Output:  EN(n) --- En(x)
...
C                            and second kinds
C                m  --- Order of Mathieu functions
C                q  --- Parameter of Mathieu functions ( q ע 0 )
