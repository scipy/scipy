...
c----------------------------------------------------------------------c
      subroutine csubmat (n,job,i1,i2,j1,j2,a,ja,ia,nr,nc,ao,jao,iao)
...
c-----------------------------------------------------------------------
      subroutine cfilter (n,job,drptol,a,ja,ia,b,jb,ib,len,ierr)
      complex*8 a(*),b(*)
      real*4 drptol
...
c-----------------------------------------------------------------------
      subroutine cfilterm (n,job,drop,a,ja,b,jb,len,ierr)
      complex*8 a(*),b(*)
      real*4 drop
...
c-----------------------------------------------------------------------
      subroutine ccsort (n,a,ja,ia,iwork,values) 
      logical values
...
c 
         ...
         if (values) call cdvperm (nnz, a(ifirst), iwork) 
c
...
c-----------------------------------------------------------------------
      subroutine cclncsr (job,value2,nrow,a,ja,ia,indu,iwk)
c     .. Scalar Arguments ..
...
c----------------------------------------------------------------------- 
      subroutine ccopmat (nrow,a,ja,ia,ao,jao,iao,ipos,job) 
      complex*8 a(*),ao(*) 
...
c-----------------------------------------------------------------------
      subroutine cmsrcop (nrow,a,ja,ao,jao,job) 
      complex*8 a(*),ao(*) 
...
c----------------------------------------------------------------------- 
      complex*8 function cgetelm (i,j,a,ja,ia,iadd,sorted) 
c-----------------------------------------------------------------------
c     purpose:
c     -------- 
...
c-----------------------------------------------------------------------
      subroutine cgetdia (nrow,ncol,job,a,ja,ia,len,diag,idiag,ioff)
      complex*8 diag(*),a(*)
...
c
c diag  = complex*8 array of length nrow containing the wanted diagonal.
...
c-----------------------------------------------------------------------
      subroutine ctransp (nrow,ncol,a,ja,ia,iwk,ierr)
...
c------------------------------------------------------------------------ 
      subroutine cgetl (n,a,ja,ia,ao,jao,iao)
...
c-----------------------------------------------------------------------
      subroutine cgetu (n,a,ja,ia,ao,jao,iao)
      integer n, ia(*), ja(*), iao(*), jao(*)
      complex*8 a(*), ao(*)
c------------------------------------------------------------------------
...
c----------------------------------------------------------------------- 
      subroutine camask (nrow,ncol,a,ja,ia,jmask,imask,
...
c----------------------------------------------------------------------- 
      subroutine crperm (nrow,a,ja,ia,ao,jao,iao,perm,job)
...
c-----------------------------------------------------------------------
      subroutine ccperm (nrow,a,ja,ia,ao,jao,iao,perm,job) 
...
c----------------------------------------------------------------------- 
      subroutine cdperm (nrow,a,ja,ia,ao,jao,iao,perm,qperm,job)
...
c 
      call crperm (nrow,a,ja,ia,ao,jao,iao,perm,locjob)
c
...
c
      if (job .le. 2) then
         call ccperm (nrow,ao,jao,iao,ao,jao,iao,perm,locjob) 
      else 
         call ccperm (nrow,ao,jao,iao,ao,jao,iao,qperm,locjob) 
      endif 
c     
      return
c-------end-of-dperm----------------------------------------------------
...
c-----------------------------------------------------------------------
      subroutine cdperm1 (i1,i2,a,ja,ia,b,jb,ib,perm,ipos,job)
...
c-----------------------------------------------------------------------
      subroutine cdperm2 (i1,i2,a,ja,ia,b,jb,ib,cperm,rperm,istart,
     *        ipos,job)
...
c-----------------------------------------------------------------------
      subroutine cdmperm (nrow,a,ja,ao,jao,perm,job)
...
c     
      ...
      n2 = n1+1
c      
      call cdperm (nrow,a,ja,ja,ao(n2),jao(n2),jao,perm,perm,job) 
c     
      jao(1) = n2
      do 101 j=1, nrow 
         ao(perm(j)) = a(j) 
...
c-----------------------------------------------------------------------
c-----------------------------------------------------------------------
      subroutine cextbdg (n,a,ja,ia,bdiag,nblk,ao,jao,iao)
      implicit complex*8 (a-h,o-z)
      complex*8 bdiag(*),a(*),ao(*)
...
c on return:
c----------
c bdiag = complex*8 array of size (n x nblk) containing the diagonal
...
c----------------------------------------------------------------------- 
      subroutine cgetbwd (n,a,ja,ia,ml,mu)
c-----------------------------------------------------------------------
...
c----------------------------------------------------------------------- 
      subroutine crnrms (nrow, nrm, a, ja, ia, diag) 
      complex*8 a(*)
      real*4 diag(nrow), scal 
...
c----------------------------------------------------------------------- 
      subroutine ccnrms (nrow, nrm, a, ja, ia, diag) 
      complex*8 a(*)
      real*4 diag(nrow) 
      integer ja(*), ia(nrow+1) 
c-----------------------------------------------------------------------
...
c----------------------------------------------------------------------- 
      subroutine croscal (nrow,job,nrm,a,ja,ia,diag,b,jb,ib,ierr) 
...
c-----------------------------------------------------------------
      call crnrms (nrow,nrm,a,ja,ia,diag)
      ierr = 0
      do 1 j=1, nrow
 ...
 1    continue
      call cdiamua (nrow,job,a,ja,ia,diag,b,jb,ib)
      return
c-------end-of-roscal---------------------------------------------------
c-----------------------------------------------------------------------
      end
c-----------------------------------------------------------------------
      subroutine ccoscal (nrow,job,nrm,a,ja,ia,diag,b,jb,ib,ierr) 
c----------------------------------------------------------------------- 
      complex*8 a(*),b(*),diag(nrow) 
...
c 2)     algorithm in place (B can take the place of A).
c-----------------------------------------------------------------
      call ccnrms (nrow,nrm,a,ja,ia,diag)
      ierr = 0
      do 1 j=1, nrow
 ...
 1    continue
      call camudia (nrow,job,a,ja,ia,diag,b,jb,ib)
      return
c--------end-of-coscal-------------------------------------------------- 
c-----------------------------------------------------------------------
      end
c-----------------------------------------------------------------------
      subroutine caddblk (nrowa, ncola, a, ja, ia, ipos, jpos, job,
...
c----------------------------------------------------------------------- 
      subroutine cxtrows (i1,i2,a,ja,ia,ao,jao,iao,iperm,job)
