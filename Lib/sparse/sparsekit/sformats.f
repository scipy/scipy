...
c----------------------------------------------------------------------c
      subroutine scsrdns (nrow,ncol,a,ja,ia,dns,ndns,ierr) 
      real*4 dns(ndns,*),a(*)
      integer ja(*),ia(*)
c-----------------------------------------------------------------------
...
c----------------------------------------------------------------------- 
      subroutine sdnscsr (nrow,ncol,nzmax,dns,ndns,a,ja,ia,ierr)
      real*4 dns(ndns,*),a(*)
      integer ia(*),ja(*)
c-----------------------------------------------------------------------
...
c----------------------------------------------------------------------- 
      subroutine scoocsr (nrow,nnz,a,ir,jc,ao,jao,iao)
c----------------------------------------------------------------------- 
      real*4 a(*),ao(*),x
...
c----------------------------------------------------------------------- 
      subroutine scoicsr (n,nnz,job,a,ja,ia,iwk)
...
c-----------------------------------------------------------------------
      subroutine scsrcoo (nrow,job,nzmax,a,ja,ia,nnz,ao,ir,jc,ierr)
c-----------------------------------------------------------------------
      real*4 a(*),ao(*) 
...
c----------------------------------------------------------------------- 
      subroutine scsrssr (nrow,a,ja,ia,nzmax,ao,jao,iao,ierr)
      real*4 a(*), ao(*), t
...
c----------------------------------------------------------------------- 
      subroutine sssrcsr (job, value2, nrow, a, ja, ia, nzmax,
...
c-----------------------------------------------------------------------
      subroutine sxssrcsr (nrow,a,ja,ia,nzmax,ao,jao,iao,indu,ierr)
...
c-----------------------------------------------------------------------
      subroutine scsrell (nrow,a,ja,ia,maxcol,coef,jcoef,ncoef,
     *                   ndiag,ierr)
      integer ia(nrow+1), ja(*), jcoef(ncoef,1)  
      real*4 a(*), coef(ncoef,1)
c----------------------------------------------------------------------- 
...
c-----------------------------------------------------------------------
      subroutine sellcsr (nrow,coef,jcoef,ncoef,ndiag,a,ja,ia,nzmax,
     *                    ierr)
      integer ia(nrow+1), ja(*), jcoef(ncoef,1) 
      real*4 a(*), coef(ncoef,1)
c----------------------------------------------------------------------- 
...
c-----------------------------------------------------------------------
      subroutine scsrmsr (n,a,ja,ia,ao,jao,wk,iwk)
      real*4 a(*),ao(*),wk(n)
...
c-----------------------------------------------------------------------
      subroutine smsrcsr (n,a,ja,ao,jao,iao,wk,iwk)
      real*4 a(*),ao(*),wk(n)
...
c----------------------------------------------------------------------- 
      subroutine scsrcsc (n,job,ipos,a,ja,ia,ao,jao,iao)
...
c
c----------------------------------------------------------------------- 
      call scsrcsc2 (n,n,job,ipos,a,ja,ia,ao,jao,iao)
      end
c-----------------------------------------------------------------------
      subroutine scsrcsc2 (n,n2,job,ipos,a,ja,ia,ao,jao,iao)
...
c-----------------------------------------------------------------------
      subroutine scsrlnk (n,a,ja,ia,link) 
      real*4 a(*) 
...
c-----------------------------------------------------------------------
      subroutine slnkcsr (n, a, jcol, istart, link, ao, jao, iao) 
      real*4 a(*), ao(*) 
...
c-----------------------------------------------------------------------
      subroutine scsrdia (n,idiag,job,a,ja,ia,ndiag,
...
c      to allow for description of diagonal along non-square arrays
      subroutine sdiacsr (m,n,job,idiag,diag,ndiag,ioff,a,ja,ia)
      real*4 diag(ndiag,idiag), a(*), t
      integer ia(*), ja(*), ioff(*)
c----------------------------------------------------------------------- 
...
c-----------------------------------------------------------------------
      subroutine sbsrcsr (job, n, m, na, a, ja, ia, ao, jao, iao)
      implicit none
...
c-----------------------------------------------------------------------
      subroutine scsrbsr (job,nrow,m,na,a,ja,ia,ao,jao,iao,iw,ierr)
      implicit none
...
c-----------------------------------------------------------------------
      subroutine scsrbnd (n,a,ja,ia,job,abd,nabd,lowd,ml,mu,ierr)
      real*4 a(*),abd(nabd,n)
      integer ia(n+1),ja(*)
c----------------------------------------------------------------------- 
...
c----------------------------------------------------------------------- 
      ierr = 0
c-----------
      if (job .eq. 1) call sgetbwd(n,a,ja,ia,ml,mu)
      m = ml+mu+1
      if (lowd .eq. 0) lowd = m
      if (m .gt. lowd)  ierr = -2
...
c-----------------------------------------------------------------------
      subroutine sbndcsr (n,abd,nabd,lowd,ml,mu,a,ja,ia,len,ierr)
      real*4 a(*),abd(nabd,*), t
      integer ia(n+1),ja(*)
c----------------------------------------------------------------------- 
...
c----------------------------------------------------------------------- 
      subroutine scsrssk (n,imod,a,ja,ia,asky,isky,nzmax,ierr)
      real*4 a(*),asky(nzmax) 
...
c-----------------------------------------------------------------------
      subroutine ssskssr (n,imod,asky,isky,ao,jao,iao,nzmax,ierr)
      real*4 asky(*),ao(nzmax) 
...
c-----------------------------------------------------------------------
      subroutine scsrjad (nrow, a, ja, ia, idiag, iperm, ao, jao, iao) 
...
c----------------------------------------------------------------------- 
      subroutine sjadcsr (nrow, idiag, a, ja, ia, iperm, ao, jao, iao) 
...
c-----------------------------------------------------------------------
      ...
      subroutine scooell (job,n,nnz,a,ja,ia,ao,jao,lda,ncmax,nc,ierr)
      implicit none
      integer job,n,nnz,lda,ncmax,nc,ierr
      integer ja(nnz),ia(nnz),jao(lda,ncmax)
      real*4  a(nnz),ao(lda,ncmax)
c-----------------------------------------------------------------------
...
c-----end-of-cooell-----------------------------------------------------
c-----------------------------------------------------------------------
      subroutine sxcooell (n,nnz,a,ja,ia,ac,jac,nac,ner,ncmax,ierr)
C-----------------------------------------------------------------------
C   coordinate format to ellpack format.
C-----------------------------------------------------------------------
C
...
c----------------------------------------------------------------------- 
      subroutine scsruss (nrow,a,ja,ia,diag,al,jal,ial,au,jau,iau) 
      real*4 a(*),al(*),diag(*),au(*) 
...
c-----------------------------------------------------------------------
      subroutine susscsr (nrow,a,ja,ia,diag,al,jal,ial,au,jau,iau) 
      real*4 a(*),al(*),diag(*),au(*) 
...
c----------------------------------------------------------------------- 
      subroutine scsrsss (nrow,a,ja,ia,sorted,diag,al,jal,ial,au)
      real*4 a(*),al(*),diag(*),au(*) 
...
c 
      if (.not. sorted) then
         call scsort (nrow, al, jal, ial, au, .true.) 
      endif
c
c copy u
c 
...
c
      subroutine sssscsr (nrow,a,ja,ia,diag,al,jal,ial,au) 
      real*4 a(*),al(*),diag(*),au(*) 
      integer ja(*),ia(nrow+1),jal(*),ial(nrow+1) 
c-----------------------------------------------------------------------
c Unsymmetric Sparse Skyline   format   to Compressed Sparse Row 
...
c-----------------------------------------------------------------------
      subroutine scsrvbr (n,ia,ja,a,nr,nc,kvstr,kvstc,ib,jb,kb,
...
c----------------------------end-of-csrvbr------------------------------
c----------------------------------------------------------------------c
      subroutine svbrcsr (ia, ja, a, nr, kvstr, kvstc, ib, jb, kb,
     &   b, nzmax, ierr)
c-----------------------------------------------------------------------
