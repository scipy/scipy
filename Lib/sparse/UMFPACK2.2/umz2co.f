        SUBROUTINE UMZ2CO (N, NZ, TRANSA, XX, XSIZE, INFO, ICNTL,
     $          II, ISIZE, W, WP, WHO)
...
C       transa:         ignored (do not transpose the input matrix)
C                       transa is assumed to be .false.
...
C       who:            who called UMZ2CO, 1: UMZ2FA, 2: UMZ2RF
C
...
C=======================================================================
C
C       called by subroutines:  UMZ2FA, UMZ2RF
C       subroutines called:     UMZ2ER, UMZ2P2
...
C                print the offending entry on the diagnostic I/O unit
                 CALL UMZ2P2 (WHO, 99, ROW, COL, XX(P), IO)
              ENDIF 
              II (P)    = II (NZ1)
              II (NZ+P) = II (NZ+NZ1)
              XX (P)    = XX (NZ1)
...
C          invalid entries found - set warning flag and continue
           CALL UMZ2ER (WHO, ICNTL, INFO, 1, NINVLD)
        ENDIF 
...
C                   the input matrix.
                    ...
                    CALL UMZ2P2 (WHO, 98, ROW, COL, XX (P), IO)
...
C          duplicate entries found - set warning flag and continue
           CALL UMZ2ER (WHO, ICNTL, INFO, 2, NDUPL)
        ENDIF 
        NZ = NZ1
...
C          set error flag if all entries are invalid
           CALL UMZ2ER (WHO, ICNTL, INFO, -2, -1)
        ENDIF 
        RETURN
        END 
