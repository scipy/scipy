...
C=======================================================================
C
C  print input/output arguments for UMC2FA, UMC2RF, and UMC2SO
...
C  disable all I/O, you can also replace the UMC2P2 routine with a
...
C=======================================================================
C
C       who:            what routine called UMC2P1:
...
C                       occurs if < 0.
C
C       Parameters to print, see UMC2FA, UMC2RF, or UMC2SO for
C       descriptions:
C
...
C=======================================================================
C
C       called by subroutines:  UMC2FA, UMC2RF, UMC2SO
...
C  ----------------------------------
C  transc:  TRANSC argument in UMC2SO
C  transa:  TRANSA argument in UMC2FA or UMC2RF when matrix factorized
...
C  luir1:   Index (luir1 ... lui2) is needed for a call to UMC2RF
...
C  factra:  0/1 if TRANSA argument was false/true in UMC2FA or UMC2RF
C  nzoff:   number of entries in off-diagonal part
C  nblks:   number of diagonal blocks
C  facjob:  JOB argument in UMC2FA or UMC2RF when matrix factorized 
C  nz:      entries in A
C  facn:    N argument in UMC2FA or UMC2RF when matrix factorized 
C  facne:   NE argument in UMC2FA or UMC2RF when matrix factorized 
C
...
C-----------------------------------------------------------------------
        ...
        IF (WHO .EQ. 1) THEN 
           IF (WHERE .EQ. 1) THEN 
              WRITE (IO, 6) 'UMC2FA input:'
              PRLU = .FALSE.
           ELSE 
              WRITE (IO, 6) 'UMC2FA output:'
              PRLU = .TRUE.
           ENDIF 
        ELSE IF (WHO .EQ. 2) THEN 
           IF (WHERE .EQ. 1) THEN 
              WRITE (IO, 6) 'UMC2RF input:'
              PRLU = .TRUE.
           ELSE 
              WRITE (IO, 6) 'UMC2RF output:'
              PRLU = .TRUE.
           ENDIF 
        ELSE IF (WHO .EQ. 3) THEN 
           IF (WHERE .EQ. 1) THEN 
              WRITE (IO, 6) 'UMC2SO input:'
              PRLU = .TRUE.
           ELSE 
              WRITE (IO, 6) 'UMC2SO output:'
              PRLU = .FALSE.
           ENDIF 
        ENDIF 
...
C-----------------------------------------------------------------------
        ...
        IF (WHERE .EQ. 1) THEN 
           WRITE (IO, 1)
     $     'Control parameters, normally initialized by UMC21I:'
...
C-----------------------------------------------------------------------
     ...
     $        ' : memory needed in INDEX for next call to UMC2RF'
              WRITE (IO, 1) '   INFO (23): ', INFO (23),
     $        ' : memory needed in VALUE for next call to UMC2RF'
           ELSE 
...
C-----------------------------------------------------------------------
C  print input matrix A, in triplet form, for UMC2FA and UMC2RF
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
C  print the LU factors:  UMC2FA output, UMC2RF input/output,
C                         and UMC2SO input
C-----------------------------------------------------------------------
...
C          -------------------------------------------------------------
C          on input to UMC2RF, only the symbol information is used
C          -------------------------------------------------------------
     ...
     $           'KEEP (4...5) gives the location of LU factors'
                 WRITE (IO, 1)
     $           '   which must be preserved for calls to UMC2RF: '
              ELSE 
                 WRITE (IO, 1)
     $           'KEEP (1...5) gives the location of LU factors'
                 WRITE (IO, 1)
     $           '   which must be preserved for calls to UMC2SO: '
                 WRITE (IO, 1) '      VALUE ( KEEP (1): ', LUX1,
     $           ' ... KEEP (2): ', LUX2,' )'
                 WRITE (IO, 1) '      INDEX ( KEEP (3): ', LUI1,
     $           ' ... KEEP (5): ', LUI2,' )'
                 WRITE (IO, 1) '   and for calls to UMC2RF: '
              ENDIF 
...
C-----------------------------------------------------------------------
C  print B (on input) or W and X (on output) for UMC2SO
C-----------------------------------------------------------------------
...
240           CONTINUE 
     ...
     $           INFO (1)
              ELSE 
                 IF (LW .EQ. 4*N) THEN 
C                   UMC2SO did iterative refinement
...
C-----------------------------------------------------------------------
        ...
        IF (WHO .EQ. 1) THEN 
           IF (WHERE .EQ. 1) THEN 
              WRITE (IO, 6) 'end of UMC2FA input '
           ELSE 
              WRITE (IO, 6) 'end of UMC2FA output'
           ENDIF 
        ELSE IF (WHO .EQ. 2) THEN 
           IF (WHERE .EQ. 1) THEN 
              WRITE (IO, 6) 'end of UMC2RF input '
           ELSE 
              WRITE (IO, 6) 'end of UMC2RF output'
           ENDIF 
        ELSE IF (WHO .EQ. 3) THEN 
           IF (WHERE .EQ. 1) THEN 
              WRITE (IO, 6) 'end of UMC2SO input '
           ELSE 
              WRITE (IO, 6) 'end of UMC2SO output'
           ENDIF 
        ENDIF 
