        SUBROUTINE UMZ2S2 (N, JOB, TRANSC, LUXSIZ, LUX,
...
C       transc:         ignored.  Only non-transposed factors are used
...
C       Cntl:           control parameters, see UMZ21I
...
C
C       X (1..n):       solution
C       Info:           integer informational output, see UMZ21I
C       Rinfo:          real informational output, see UMZ21I
C
...
C=======================================================================
C
C       called by subroutine:   UMZ2SO
C       subroutines called:     UMZ2ER, UMZ2SL, UMZ2SU
C       functions called:       IZAMAX, ABS, MAX
        INTRINSIC ABS, MAX
        INTEGER IZAMAX
...
10            CONTINUE 
              IF (NBLKS .EQ. 1) THEN 
                 CALL UMZ2SL (NLU, NPIV, N, LUI(6), LUI(6+NLU), LUX,X,Z)
              ELSE 
                 DO 20 BLK = 1, NBLKS 
                    K1 = BLKP (BLK)
                    K2 = BLKP (BLK+1) - 1
                    ...
                    IF (KN .GT. 1) THEN 
                       ...
                       NLU = LUI (P+1)
                       NPIV = LUI (P+2)
                       CALL UMZ2SL (NLU, NPIV, KN, LUI (P+5),
...
C             ----------------------------------------------------------
              ...
              IF (NBLKS .EQ. 1) THEN 
                 DO 110 I = 1, N 
                    R (I) = B (I)
110              CONTINUE 
                 CALL UMZ2SU (NLU, NPIV, N, LUI(6), LUI(6+NLU), LUX,R,Z)
              ELSE 
...
130                 CONTINUE 
     ...
     $                    LUI (P+5+NLU), LUX (LUI (P)), X (K1), Z)
                       DO 140 I = K1, K2 
                          R (I) = B (I) - X (I)
140                    CONTINUE 
                       CALL UMZ2SU (NLU, NPIV, KN, LUI (P+5),
...
C             ----------------------------------------------------------
C             transa and transc are both .false., and C = A is stored
C             in column form.
C             ----------------------------------------------------------
...
210                 CONTINUE 
                 ENDIF 
                 IF (NBLKS .EQ. 1) THEN 
                    CALL UMZ2SL (NLU, NPIV, N,LUI(6),LUI(6+NLU),LUX,R,Z)
                    CALL UMZ2SU (NLU, NPIV, N,LUI(6),LUI(6+NLU),LUX,R,Z)
                 ELSE 
