        SUBROUTINE UMC2F2 (CP, NZ, N, PN, CPERM, RPERM, ITAIL, XTAIL,
...
C=======================================================================
C
C  UMC2F2 factorizes the n-by-n input matrix at the head of II/XX
...
C       Keep (6)        integer control parameter, see UMC21I
...
C       xrmax:          maximum real memory usage for UMC2RF
C       xruse:          current real memory usage for UMC2RF
C
...
C=======================================================================
C
C       called by subroutine:   UMC2F1
C       subroutines called:     UMC2ER, UMC2FG, CGEMV, CGEMM 
C       functions called:       ICAMAX, ABS, MAX, MIN
        INTEGER ICAMAX
...
C       When UMC2F2 returns, then luip is given by luip =
...
C  e:       an element
C  eson:    an element
C  s:       a renumbered element (1..nlu) for UMC2SO and UMC2RF
...
C  maxdr:   maximum ludegr for any LU arrowhead, for UMC2RF
C  maxdc:   maximum ludegc for any LU arrowhead, for UMC2RF
...
C       largest that n can be.  This condition is checked in UMC2FA.
        KLEFT = N
        NPIV = 0
        NLU = 0
        MINDEG = 1
...
C          use XX (wxp..wxp+xs-1) as workspace for values [
           ...
           IF (XS .GT. XTAIL-XHEAD) THEN 
              ...
              INFO (15) = INFO (15) + 1
              CALL UMC2FG (XX, XSIZE, XHEAD, XTAIL, XUSE,
...
C          -------------------------------------------------------------
           ...
           MAXVAL = ABS (XX (WXP - 1 + ICAMAX (CDEG, XX (WXP), 1)))
...
C          -------------------------------------------------------------
           ...
           MAXVAL = ABS (XX (WXP-1 + ICAMAX (FEDEGC, XX (WXP), 1)))
...
C=======================================================================
        ...
        IF (K-K0 .GE. NB .OR. MOVELU) THEN 
           CALL CGEMM ('N', 'N', FFLEFC, FFLEFR, K-K0,
     $          -ONE, XX (FFXP + (FFDIMR - K) * FFDIMC), FFDIMC,
...
C          this list of sons is for the refactorization (UMC2RF) only.
           LUSONP = LURP + LUDEGR
           IP = LUSONP
           E = SONLST
C          while (e > 0) do
...
C          Otherwise, some arrays in UMC2RF or UMC2SO would have
...
C          in UMC2F1 have been passed to this routine as WiR and WiC,
C          which now contain the row and column permutations.  Rperm and
C          Cperm in this routine (UMC2F2) are not defined.
        ENDIF 
