        SUBROUTINE UMZ2RF (N, NE, JOB, TRANSA, LVALUE, LINDEX, VALUE,
...
C  a prior factorization by UMZ2FA or UMZ2RF.  The matrix A should have
C  the same nonzero pattern as the matrix factorized by UMZ2FA or
C  UMZ2RF.  The matrix can have different numerical values.  No
C  variations are made in the pivot order computed by UMZ2FA.  If a
...
C  factors of the matrix factorized by UMZ2FA.  If BTF (block triangular
...
C
C  This routine must be preceded by a call to UMZ2FA or UMZ2RF.
C  A call to UMZ2RF can be followed by any number of calls to UMZ2SO,
C  which solves a linear system using the LU factors computed by this
C  routine or by UMZ2FA.  A call to UMZ2RF can also be followed by any
C  number of calls to UMZ2RF.
...
C           the same, or different, as the last call to UMZ2FA.
...
C           by the last call to UMZ2FA or UMZ2RF (Info (23)).
...
C           although a larger will typically be required to complete
C           the factorization.  The exact value required is computed
C           by the last call to UMZ2FA or UMZ2RF (Info (22)).
C           This value assumes that the ne, job, and transa parameters
C           are the same as the last call.  No garbage collection ever
C           occurs in the Index array, since UMZ2RF does not create
C           external fragmentation in Index.  The lindex parameter is
C           usually the same as in the last call to UMZ2FA, however.
...
C           to UMZ2FA or UMZ2RF).  Modified on output.  On input,
...
C               from the last call to UMZ2FA or UMZ2RF). Modified on
C               output.
...
C               used in the last call to UMZ2FA).  On output, the new
C               LU factors needed for UMZ2RF start here in Index.
...
C
C           Keep (6 ... 8):  Unused.  These are used by UMZ2FA only.
...
C           Must be set by caller on input (not modified).
C           Control arguments, see UMZ21I for a
C           description, which sets the default values.  The current
C           version of UMZ2RF does not actually use Cntl.  It is
C           included to make the argument list of UMZ2RF the same as
C           UMZ2FA.  UMZ2RF may use Cntl in future releases.

C           ------------------------------------------------------------
C  Icntl:   An integer array of size 20.
C           Must be set by caller on input (not modified).
C           Integer control arguments, see UMZ21I for a description,
C           which sets the default values.  UMZ2RF uses Icntl (1),
C           Icntl (2), Icntl (3), and Icntl (7).

C           ------------------------------------------------------------
C  Info:    An integer array of size 40.
C           Need not be set by caller on input.  Modified on output.
C           It contains information about the execution of UMZ2RF.
C
...
C
C           Info (14): zero.  Used by UMZ2FA only.
C
...
C               UMZ2RF can be called if the value of Info (17) 
C               returned by UMZ2FA was less than n, but the order
...
C
C           Rinfo (3): zero.  Used by UMZ2FA only.
C
...
C## End of user documentation for UMZ2RF ###############################
...
C       subroutines called:     UMZ2ER, UMZ2P1, UMZ2CO, UMZ2R0
...
C  luir1:   Index (luir1 ... lui2) is needed for this call to UMZ2RF
...
C  n1:      N argument in UMZ2FA or UMZ2RF when prior matrix factorized 
C
C  Other:
C  ------
C  i:       loop index
...
C-----------------------------------------------------------------------
        ...
        IF (N .LT. 1) THEN 
C          n is too small
           CALL UMZ2ER (2, ICNTL, INFO, -1, -1)
           GO TO 9000
        ENDIF 
        IF (NE .LT. 1) THEN 
C          ne is too small
           CALL UMZ2ER (2, ICNTL, INFO, -2, -1)
           GO TO 9000
        ENDIF 

        IF (TRANSA) THEN 
C          transa must be false
           CALL UMZ2ER (2, ICNTL, INFO, -8, 0) 
           GO TO 9000
        ENDIF 
...
C-----------------------------------------------------------------------
        ...
        IF (BADLU) THEN 
           CALL UMZ2ER (2, ICNTL, INFO, -7, 0)
C          error return, LU factors are corrupted:
           GO TO 9000
        ENDIF 
        IF (2*NE .GT. LUIR1) THEN 
           CALL UMZ2ER (2, ICNTL, INFO, -2, LUIR1/2)
...
C  lindex are unmodified from the prior call to UMZ2FA, then
...
C       ne1 = Index (lindex), not required for UMZ2RF
        N1 = INDEX (LINDEX-1)
        NZ1 = INDEX (LINDEX-2)
C       presr1 = Index (lindex-3) .ne. 0, not required for UMZ2RF
        NBLKS = INDEX (LINDEX-4)
C       nzoff1 = Index (lindex-5), not required for UMZ2RF
C       trans1 = Index (lindex-6) .ne. 0, not required for UMZ2RF
...
C          set error flag if out of integer memory
           CALL UMZ2ER (2, ICNTL, INFO, -3, IUSE)
        ENDIF 
        IF (LVALUE .LT. XUSE) THEN 
C          set error flag if out of real memory
           CALL UMZ2ER (2, ICNTL, INFO, -4, XUSE)
        ENDIF 
        IF (INFO (1) .LT. 0) THEN 
...
40         CONTINUE 
     ...
     $          INDEX (CPERMP), INDEX (RPERMP), NE)
           IF (INFO (1) .LT. 0) THEN 
C             error return, if UMZ2R0 fails
              GO TO 9000
           ENDIF 
