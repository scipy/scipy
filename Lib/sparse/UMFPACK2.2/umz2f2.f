        SUBROUTINE UMZ2F2 (CP, NZ, N, PN, CPERM, RPERM, ITAIL, XTAIL,
...
C=== UMZ2F2 ============================================================
C
C  Unsymmetric-pattern MultiFrontal Package (UMFPACK). Version 2.2z
...
C=======================================================================
C
C  UMZ2F2 factorizes the n-by-n input matrix at the head of II/XX
...
C       Icntl:          integer control parameters, see UMZ21I
C       Cntl:           real control parameters, see UMZ21I
C       Keep (6)        integer control parameter, see UMZ21I
...
C       xrmax:          maximum real memory usage for UMZ2RF
C       xruse:          current real memory usage for UMZ2RF
C
...
C
C       Info:           integer informational output, see UMZ2FA
C       Rinfo:          real informational output, see UMZ2FA
C       if pgiven:
...
C=======================================================================
C
C       called by subroutine:   UMZ2F1
C       subroutines called:     UMZ2ER, UMZ2FG, ZGEMV, ZGEMM 
C       functions called:       IZAMAX, ABS, MAX, MIN
        INTEGER IZAMAX
...
C       When UMZ2F2 returns, then luip is given by luip =
...
C  e:       an element
C  eson:    an element
C  s:       a renumbered element (1..nlu) for UMZ2SO and UMZ2RF
...
C  maxdr:   maximum ludegr for any LU arrowhead, for UMZ2RF
C  maxdc:   maximum ludegc for any LU arrowhead, for UMZ2RF
...
C       largest that n can be.  This condition is checked in UMZ2FA.
        KLEFT = N
        NPIV = 0
        NLU = 0
        MINDEG = 1
...
C          use XX (wxp..wxp+xs-1) as workspace for values [
           ...
           IF (XS .GT. XTAIL-XHEAD) THEN 
              ...
              INFO (15) = INFO (15) + 1
              CALL UMZ2FG (XX, XSIZE, XHEAD, XTAIL, XUSE,
...
C          -------------------------------------------------------------
           ...
           MAXVAL = ABS (XX (WXP - 1 + IZAMAX (CDEG, XX (WXP), 1)))
           RINFO (3) = RINFO (3) + 2.0*(CDEG)
           TOLER = RELPT * MAXVAL
           RDEG = N+1
...
C       ----------------------------------------------------------------
C       get memory usage for next call to UMZ2RF
C       ----------------------------------------------------------------
...
C          -------------------------------------------------------------
           ...
           MAXVAL = ABS (XX (WXP-1 + IZAMAX (FEDEGC, XX (WXP), 1)))
           RINFO (3) = RINFO (3) + 2.0*(FEDEGC)
           TOLER = RELPT * MAXVAL
           RDEG = N+1
...
C=======================================================================
        ...
        IF (K-K0 .GE. NB .OR. MOVELU) THEN 
           CALL ZGEMM ('N', 'N', FFLEFC, FFLEFR, K-K0,
     $          -ONE, XX (FFXP + (FFDIMR - K) * FFDIMC), FFDIMC,
...
C          this list of sons is for the refactorization (UMZ2RF) only.
           LUSONP = LURP + LUDEGR
           IP = LUSONP
           E = SONLST
C          while (e > 0) do
...
C          Otherwise, some arrays in UMZ2RF or UMZ2SO would have
...
C          in UMZ2F1 have been passed to this routine as WiR and WiC,
C          which now contain the row and column permutations.  Rperm and
C          Cperm in this routine (UMZ2F2) are not defined.
        ENDIF 
...
C          error return if out of integer memory
           CALL UMZ2ER (1, ICNTL, INFO, -3, INFO (19))
        ENDIF 
        IF (XHEAD .GT. XTAIL) THEN 
C          error return if out of real memory
           CALL UMZ2ER (1, ICNTL, INFO, -4, INFO (21))
        ENDIF 
        RETURN
        END 
