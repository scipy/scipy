		      ...
		      const GpReal*,const GpReal*))
{
  ...
  engine= (Engine *)GmMalloc(size+lname);
  if (!engine) return 0;
...
void GpDelEngine(Engine *engine)
{
  ...
  if (eng==engine) gistEngines= engine->next;
    ...
    /* Because of recursive deletes necessary to deal with X window
       ...
       and GmFree must NOT be called.  Fix this someday.  */
...
/* (this is unused except for FORTRAN GKS interface, see fort.c) */
extern int gistTID;
int gistTID= -1;        /* set to <0 if non-GKS map is installed */
...
static void MoreScratch(long np, long ns)
{
  if (scratch) GmFree(scratch);
  if (np) {
    np+= 64;
    scratch= (void *)GmMalloc(sizeof(GpPoint)*np);
    scratchPoints= np;
    scratchSegs= (sizeof(GpPoint)*np)/sizeof(GpSegment);
  } else {
    ns+= 32;
    scratch= (void *)GmMalloc(sizeof(GpSegment)*ns);
    scratchSegs= ns;
...
int GdBeginDr(Drawing *drawing, GpBox *damage, int landscape)
{
  int needToRedraw= 0;
  Engine *eng;
...
void GpDamage(Engine *eng, Drawing *drawing, GpBox *box)
{
...
void GdDetach(Drawing *drawing, Engine *engine)
{
  Engine *eng;
  for (eng=GpNextEngine(0) ; eng ; eng=GpNextEngine(eng)) {
