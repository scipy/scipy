...
/* On Suns, need to load with /usr/lib/debug/malloc.o to use this */
#ifdef MM_DEBUG
int verify;
#endif
...
void get_time(double *cpu, double *sys, double *wall)
{
...
#if 0
static GpTransform noTrans= {{0., 1., 0., 1.}, {0., 1., 0., 1.}};
static GpTransform tickTrans= {{.25, .6, .5, .85}, {0., 1., 1., 10.}};
static GaTickStyle tickStyle= {
...
int HandleInput(FILE *file, void *context)
{
  ...
  if (!noKeyboard) {
    gets(line);
    token= strtok(line, ", \t\n");
    if (!token) { GhBeforeWait(); return 0; }
  } else {
    token= context;
  }
  if (strcmp(token, "q")==0 || strcmp(token, "quit")==0) return 1;
...
/* ------- A and P level tests -------- */
  ...
  if (strcmp(token, "0")==0) {
    ...
    printf("elapsed seconds: %f user, %f system, %f wall\n",
	   user-user0, system-system0, wall-wall0);
    printf("Plots per wall second= %f\n", NFRAMES/(wall-wall0));
...
int main(int argc, char *argv[])
{
  ...
  noKeyboard= (argc>1 && argv[1][0]=='-');

#ifndef NO_XLIB
  AddFDispatcher(stdin, &HandleInput, 0);
  ...
  /*  GpActivate(engine);  */
  GhSetPlotter(0);
  if (waitTest || noKeyboard) GhWaitDisplay();
  ...
  mesh.iMax= N;
  mesh.jMax= N;
  mesh.x= xmesh= (GpReal *)malloc(sizeof(GpReal)*N*N);
  mesh.y= ymesh= (GpReal *)malloc(sizeof(GpReal)*N*N);
  mesh.reg= ireg= (int *)malloc(sizeof(int)*(N*N+N+1));
  mesh.triangle= 0;
  cmesh= (GpColor *)malloc(sizeof(GpColor)*N*N);
  z= (GpReal *)malloc(sizeof(GpReal)*N*N);
  u= (GpReal *)malloc(sizeof(GpReal)*N*N);
  v= (GpReal *)malloc(sizeof(GpReal)*N*N);
  j= 0;
  for (i=0 ; i<N*N+N+1 ; i++) {
  ...
  printf("Initializing %dx%d contour function, STANDBY...", N, N);
  fflush(stdout);
  ...
  if (!noKeyboard) {
    ...
    DispatchEvents();
#else
    GhSetXHandler(&Blammo);
    while (!HandleInput(stdin, (void *)0));
#endif
  } else {
    char **command= noKeyTest;
    while (!HandleInput(stdin, *command++));
  }
...
void Blammo(char *msg)
{
  printf("\nExiting on X error:\n");
  printf(msg);
  printf("\n\n");
  if (noKeyboard) exit(0);  /* not 1 -- want make to continue */
  else exit(1);
}
