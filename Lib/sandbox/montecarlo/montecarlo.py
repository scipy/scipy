...
class sampler(object):
    ...
    def sample(self, n):
        """Returns a tuple of two arrays (samples, logprobs) containing
        the sample points x_j and their log probability densities p(x_j)
        under the model p.
        """
        raise NotImplementedError, "derived classes must override sample()"


#class intsampler(sampler):
#    def __init__(self, table):
#        """Initializes a sampler from a discrete distribution with
#        probability mass function given by 'table' as:
#
#        x       0           1           ...     k-1
...
def tablesampler(table):
    """ This is a slow Python version of intsampler.  It is obselete,
    except as a reference implementation of 'intsampler', which is
