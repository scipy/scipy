    ...
    **Blocksize**
    ...
    def getnnz(self, axis=None):
        if axis is not None:
            raise NotImplementedError(
                "getnnz over an axis is not implemented for BSR format"
            )
        R, C = self.blocksize
    ...
    def __repr__(self):
        ...
        return (
            ...
            % (self.shape + (self.dtype.type, self.nnz) + self.blocksize + (format,))
        )
    ...
    @np.deprecate(
        message="BSR matvec is deprecated in SciPy 0.19.0. Use * operator instead."
    )
    ...
    @np.deprecate(
        message="BSR matmat is deprecated in SciPy 0.19.0. Use * operator instead."
    )
    ...
    def tocsr(self, copy=False):
        ...
        R, C = self.blocksize
        nnz = self.nnz
        idx_dtype = get_index_dtype((self.indptr, self.indices), maxval=max(nnz, N))
    ...
    def transpose(self, axes=None, copy=False):
        ...
        if self.nnz == 0:
            return bsr_matrix((N, M), blocksize=(C, R), dtype=self.dtype, copy=copy)
        ...
        return bsr_matrix((data, indices, indptr), shape=(N, M), copy=copy)
