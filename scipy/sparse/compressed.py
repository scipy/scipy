...
class _cs_matrix(_data_matrix, _minmax_mixin, IndexMixin):
        ...
        if isspmatrix(arg1):
            ...
            if isshape(arg1):
                ...
                if len(arg1) == 2:
                    ...
                    raise ValueError(
                        "unrecognized {}_matrix constructor usage".format(self.format)
                    )
    ...
    def check_format(self, full_check=True):
        ...
        if self.indptr.dtype.kind != "i":
            warn(
                "indptr array has non-integer dtype ({})".format(
        ...
        if self.indices.dtype.kind != "i":
            warn(
                "indices array has non-integer dtype ({})".format(
        ...
        if len(self.indptr) != major_dim + 1:
            raise ValueError(
                "index pointer size ({}) should be ({})".format(
        ...
        if full_check:
            ...
            if self.nnz > 0:
                if self.indices.max() >= minor_dim:
                    raise ValueError(
                        "{} index values must be < {}".format(minor_name, minor_dim)
                    )
                if self.indices.min() < 0:
                    raise ValueError("{} index values must be >= 0".format(minor_name))
    ...
    def multiply(self, other):
        ...
        if isspmatrix(other):
            ...
            # self is a row.
                ...
                copy = dia_matrix(
                    (self.toarray().ravel(), [0]), shape=(self.shape[1], self.shape[1])
                )
            ...
            # self is a column.
                ...
                copy = dia_matrix(
                    (self.toarray().ravel(), [0]), shape=(self.shape[0], self.shape[0])
                )
    ...
    def _mul_sparse_matrix(self, other):
        ...
        idx_dtype = get_index_dtype(
            (self.indptr, self.indices, other.indptr, other.indices), maxval=nnz
        )
    ...
    def diagonal(self, k=0):
        ...
        y = np.empty(min(rows + min(k, 0), cols - max(k, 0)), dtype=upcast(self.dtype))
    ...
    def maximum(self, other):
        return self._maximum_minimum(
            other, np.maximum, "_maximum_", lambda x: np.asarray(x) > 0
        )
    ...
    def minimum(self, other):
        return self._maximum_minimum(
            other, np.minimum, "_minimum_", lambda x: np.asarray(x) < 0
        )
    ...
    def sum(self, axis=None, dtype=None, out=None):
        ...
        if not hasattr(self, "blocksize") and axis in self._swap(((1, -1), (0, 2)))[0]:
    ...
    def _major_index_fancy(self, idx):
        ...
        csr_row_index(
            M, indices, self.indptr, self.indices, self.data, res_indices, res_data
        )
    ...
    def _minor_index_fancy(self, idx):
        ...
        csr_column_index1(
            k, idx, M, N, self.indptr, self.indices, col_offsets, res_indptr
        )
    ...
    def _set_many(self, i, j, x):
        ...
        if -1 not in offsets:
            ...
            warn(
                "Changing the sparsity structure of a {}_matrix is expensive."
    ...
    def tocoo(self, copy=True):
        ...
        return coo_matrix(
            (self.data, (row, col)), self.shape, copy=copy, dtype=self.dtype
        )
    ...
    has_canonical_format = property(
        fget=__get_has_canonical_format, fset=__set_has_canonical_format
    )
    ...
    def sort_indices(self):
        ...
        if not self.has_sorted_indices:
            _sparsetools.csr_sort_indices(
                len(self.indptr) - 1, self.indptr, self.indices, self.data
            )
            self.has_sorted_indices = True
    ...
    def resize(self, *shape):
        ...
        if hasattr(self, "blocksize"):
            ...
            if rm or rn:
                raise ValueError(
                    "shape must be divisible into %s blocks. Got %s"
                    % (self.blocksize, shape)
                )
...
def _process_slice(sl, num):
    if sl is None:
        ...
        i0, i1 = sl, sl + 1
        if i0 < 0 or i1 > num:
            raise IndexError("index out of bounds: 0 <= %d < %d <= %d" % (i0, i1, num))
    else:
