...
@pytest.mark.parametrize("slice_",
    [
     ...
     np.s_[:],
     np.s_[:, :],
     # slice X slice with step=1.
     np.s_[2:3],
     np.s_[2:-1, :3],
     np.s_[:3, 1:-1],
     # First slice with step != 1.
     np.s_[0:4:2],
     np.s_[0:4:2, 1:5:2],
     np.s_[0:4:2, :],
     np.s_[0:4:2, -1],
     np.s_[0:4:2, [1, 3]],
     # Second slice with step != 1.
     np.s_[:, 0:5:2],
     np.s_[1, 1:5:2],
     np.s_[[0, 2], 0:5:2],
     # int indexing.
...
def test_slicing(slice_):
    dense = np.arange(50).reshape(5, 10)
    csr = csr_matrix(dense)
    actual = csr[slice_]
    expected = dense[slice_]

    if isinstance(actual, np.number):
        assert actual == expected
    else:
        cmp = actual.toarray() if hasattr(actual, "toarray") else actual.A
        assert_array_equal(cmp.reshape(*expected.shape), expected)
