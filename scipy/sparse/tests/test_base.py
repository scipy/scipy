...
test_arrays = False  # set to False for *_matrix
bsr_object = bsr_array if test_arrays else bsr_matrix
coo_object = coo_array if test_arrays else coo_matrix
csc_object = csc_array if test_arrays else csc_matrix
csr_object = csr_array if test_arrays else csr_matrix
dia_object = dia_array if test_arrays else dia_matrix
dok_object = dok_array if test_arrays else dok_matrix
lil_object = lil_array if test_arrays else lil_matrix

asobject = np.asarray if test_arrays else asmatrix
...
class _TestCommon:
    ...
    @property
    def is_array_test(self):
        return isinstance(self.datsp, sparray)
    ...
    def test_eq(self):
        ...
        def check(dtype):
            ...
            datbsr = bsr_object(dat)
            datcsr = csr_object(dat)
            datcsc = csc_object(dat)
            datlil = lil_object(dat)
    ...
    def test_sum(self):
        ...
        def check(dtype, j):
            dat = asobject(matrices[j], dtype=dtype)
    ...
    def test_sum_out(self):
        ...
        datsp_out = asobject(np.zeros((3, 1)))
    ...
    def test_todense(self):
        ...
        out = asobject(np.zeros(self.datsp.shape, dtype=self.datsp.dtype))
        chk = self.datsp.todense(out=out)
        assert_array_equal(self.dat, out)
        assert_array_equal(self.dat, chk)
        assert np.may_share_memory(chk, out)
        ...
        def check(name):
            if name == "sign":
                pytest.skip("sign conflicts with comparison op "
                            "support on Numpy")
            if self.spcreator in (dok_object, lil_object):
    ...
    def test_resize(self):
        ...
        if self.is_array_test and S.format in ["coo", "dok", "csr"]:
            S.resize(1)
        else:
            assert_raises(ValueError, S.resize, 1)
...
class _TestSlicing:
    ...
    def test_get_horiz_slice(self):
        B = asobject(arange(50.).reshape(5,10))
        A = self.spcreator(B)
...
class _TestSlicingAssign:
    ...
    def test_self_self_assignment(self):
        # Tests whether a row of one lil_object can be assigned to
        # another.
...
class _TestFancyIndexing:
    ...
    def test_missized_masking(self):
        ...
        B = asobject(arange(M * N).reshape(M, N))
        A = self.spcreator(B)
    ...
    def test_argmax(self):
        ...
        for D in [D1, D2, D3, D4, D5]:
            ...
            assert_equal(mat.argmax(axis=0),
                         asobject(np.argmax(D, axis=0)))
            assert_equal(mat.argmin(axis=0),
                         asobject(np.argmin(D, axis=0)))

            assert_equal(mat.argmax(axis=1),
                         asobject(np.argmax(D, axis=1).reshape(-1, 1)))
            assert_equal(mat.argmin(axis=1),
                         asobject(np.argmin(D, axis=1).reshape(-1, 1)))
...
class TestCSR(sparse_test_class()):
    ...
    def test_constructor5(self):
        ...
        data = array([1,2,3,4])
        csr = csr_object((data, indices, indptr))
    ...
    def test_constructor_smallcol(self):
        ...
        a = csr_object((data, col, ptr), shape=(3, 3))
    ...
    def test_sort_indices(self):
        ...
        indptr = array([0, 3, 5])
        asp = csr_object((data, indices, indptr), shape=(2,10))
        bsp = asp.copy()
        asp.sort_indices()
    ...
    def test_ufuncs(self):
        X = csr_object(np.arange(20).reshape(4, 5) / 20.)
        for f in ["sin", "tan", "arcsin", "arctan", "sinh", "tanh",
                  "arcsinh", "arctanh", "rint", "sign", "expm1", "log1p",
                  "deg2rad", "rad2deg", "floor", "ceil", "trunc", "sqrt"]:
            assert_equal(hasattr(csr_object, f), True)
            X2 = getattr(X, f)()
    ...
    def test_unsorted_arithmetic(self):
        ...
        indptr = array([0, 2, 6])
        bsp = csr_object((data, indices, indptr), shape=(2,10))
    ...
    def test_has_sorted_indices(self):
        ...
        data = np.array([1, 1])
        indptr = np.array([0, 2])
        M = csr_object((data, sorted_inds, indptr)).copy()
        assert_equal(True, M.has_sorted_indices)
        assert isinstance(M.has_sorted_indices, bool)

        M = csr_object((data, unsorted_inds, indptr)).copy()
    ...
    def test_has_canonical_format(self):
        ...
        M = csr_object((np.array([2]), np.array([0]), np.array([0, 1])))
        ...
        M = csr_object((data, indices, indptr)).copy()
    ...
    def test_scalar_idx_dtype(self):
        ...
        vals = np.zeros(0)
        a = csr_object((vals, indices, indptr), shape=(1, 2**31-1))
        b = csr_object((vals, indices, indptr), shape=(1, 2**31))
        ij = np.zeros((2, 0), dtype=np.int32)
        c = csr_object((vals, ij), shape=(1, 2**31-1))
        d = csr_object((vals, ij), shape=(1, 2**31))
        e = csr_object((1, 2**31-1))
        f = csr_object((1, 2**31))
...
class TestCSC(sparse_test_class()):
    ...
    def test_constructor4(self):
        ...
        ij = vstack((row,col))
        csc = csc_object((data,ij),(4,3))
        assert_array_equal(arange(12).reshape(4, 3), csc.toarray())

        # with duplicates (should sum the duplicates)
        csc = csc_object(([1,1,1,1], ([0,2,2,0], [0,1,1,0])))
        assert csc.nnz == 2
    ...
    def test_constructor5(self):
        ...
        indices = array([0,5,1,2])
        data = array([1,2,3,4])
        csc = csc_object((data, indices, indptr))
    ...
    def test_eliminate_zeros(self):
        ...
        indices = array([1, 2, 3, 4, 5, 6, 7, 8])
        indptr = array([0, 3, 8])
        asp = csc_object((data, indices, indptr), shape=(10,2))
        bsp = asp.copy()
        asp.eliminate_zeros()
        assert_array_equal(asp.nnz, 3)
        assert_array_equal(asp.data,[1, 2, 3])
    ...
    def test_sort_indices(self):
        ...
        row = array([7, 2, 1, 5, 4])
        ptr = [0, 3, 5]
        asp = csc_object((data, row, ptr), shape=(10,2))
        bsp = asp.copy()
        asp.sort_indices()
        assert_array_equal(asp.indices,[1, 2, 7, 4, 5])
        assert_array_equal(asp.toarray(), bsp.toarray())
    ...
    def test_ufuncs(self):
        X = csc_object(np.arange(21).reshape(7, 3) / 21.)
        for f in ["sin", "tan", "arcsin", "arctan", "sinh", "tanh",
                  "arcsinh", "arctanh", "rint", "sign", "expm1", "log1p",
                  "deg2rad", "rad2deg", "floor", "ceil", "trunc", "sqrt"]:
            assert_equal(hasattr(csr_object, f), True)
    ...
    def test_unsorted_arithmetic(self):
        ...
        indices = array([8, 1, 5, 7, 2, 4])
        indptr = array([0, 2, 6])
        bsp = csc_object((data, indices, indptr), shape=(10,2))
        assert_equal((asp + bsp).toarray(), asp.toarray() + bsp.toarray())
    ...
    def test_scalar_idx_dtype(self):
        ...
        indices = np.zeros(0, dtype=np.int32)
        vals = np.zeros(0)
        a = csc_object((vals, indices, indptr), shape=(2**31-1, 1))
        b = csc_object((vals, indices, indptr), shape=(2**31, 1))
        ij = np.zeros((2, 0), dtype=np.int32)
        c = csc_object((vals, ij), shape=(2**31-1, 1))
        d = csc_object((vals, ij), shape=(2**31, 1))
        e = csr_object((1, 2**31-1))
        f = csr_object((1, 2**31))
        assert_equal(a.indptr.dtype, np.int32)
        assert_equal(b.indptr.dtype, np.int64)
...
class TestDOK(sparse_test_class(minmax=False, nnz_axis=False)):
    ...
    def test_ctor(self):
        # Empty ctor
        assert_raises(TypeError, dok_object)
        ...
        data = [[0, 1, 2], [3, 0, 0]]
        d = dok_object(data, dtype=np.float32)
...
class TestLIL(sparse_test_class(minmax=False)):
    ...
    def test_dot(self):
        ...
        B = lil_object((10, 10), dtype=np.complex128)
        B[0, 3] = 10
        B[5, 6] = 20j
    ...
    def test_inplace_ops(self):
        A = lil_object([[0, 2, 3], [4, 0, 6]])
        B = lil_object([[0, 1, 0], [0, 2, 3]])
        ...
        # Ticket 1604.
        A = lil_object((1, 3), dtype=np.dtype('float64'))
        B = array([0.1, 0.1, 0.1])
        A[0, :] += B
    ...
    def test_fancy_indexing_lil(self):
        M = asobject(arange(25).reshape(5, 5))
        A = lil_object(M)
...
class TestCOO(sparse_test_class(getset=False,
    ...
    def test_constructor1(self):
        ...
        coo = coo_object((data,(row,col)),(4,3))
        assert_array_equal(arange(12).reshape(4, 3), coo.toarray())

        # using Python lists and a specified dtype
        coo = coo_object(([2**63 + 1, 1], ([0, 1], [0, 1])), dtype=np.uint64)
        dense = array([[2**63 + 1, 0], [0, 1]], dtype=np.uint64)
    ...
    def test_constructor2(self):
        ...
        coo = coo_object((data,(row,col)),(3,3))
    ...
    def test_todok_duplicates(self):
        coo = coo_object(([1,1,1,1], ([0,2,2,0], [0,1,1,0])))
        dok = coo.todok()
    ...
    def test_eliminate_zeros(self):
        ...
        asp = coo_object((data, (row, col)), shape=(2,10))
        bsp = asp.copy()
        asp.eliminate_zeros()
    ...
    def test_large_dimensions_reshape(self):
        ...
        # exceeds 2^31-1
        mat1 = coo_object(([1], ([3000000], [1000])), (3000001, 1001))
        mat2 = coo_object(([1], ([1000], [3000000])), (1001, 3000001))
...
class TestDIA(sparse_test_class(getset=False, slicing=False, slicing_assign=False,
    ...
    def test_constructor1(self):
        ...
        assert_equal(dia_object((data, offsets), shape=(4, 4)).toarray(), D)
    ...
    def test_setdiag_dtype(self):
        m = dia_object(np.eye(3))
        assert_equal(m.offsets.dtype, np.int32)
        m.setdiag((3,), k=2)
        assert_equal(m.offsets.dtype, np.int32)

        m = dia_object(np.eye(4))
    ...
    def test_convert_gh14555(self):
        ...
        m = dia_object(([[1, 1, 0]], [-1]), shape=(4, 2))
        expected = m.toarray()
    ...
    def test_mul_scalar(self):
        # repro for gh-20434
        m = dia_object([[1, 2], [0, 4]])
        res = m * 3
        assert isinstance(res, dia_object)
        assert_array_equal(res.toarray(), [[3, 6], [0, 12]])

        res2 = m.multiply(3)
        assert isinstance(res2, dia_object)
...
class TestBSR(sparse_test_class(getset=False,
    ...
    def test_constructor1(self):
        ...
        Asp = bsr_object((data,indices,indptr),shape=(6,12))
        assert_equal(Asp.toarray(), A)

        # infer shape from arrays
        Asp = bsr_object((data,indices,indptr))
        assert_equal(Asp.toarray(), A)
    ...
    def test_constructor2(self):
        ...
        for shape in [(1,1), (5,1), (1,10), (10,4), (3,7), (2,1)]:
            A = zeros(shape)
            assert_equal(bsr_object(A).toarray(), A)
        A = zeros((4,6))
        assert_equal(bsr_object(A, blocksize=(2, 2)).toarray(), A)
        assert_equal(bsr_object(A, blocksize=(2, 3)).toarray(), A)

        A = kron([[1,0,2,0],[0,0,0,0],[0,0,4,5]], [[0,1,2],[3,0,5]])
        assert_equal(bsr_object(A).toarray(), A)
        assert_equal(bsr_object(A, shape=(6, 12)).toarray(), A)
        assert_equal(bsr_object(A, blocksize=(1, 1)).toarray(), A)
        assert_equal(bsr_object(A, blocksize=(2, 3)).toarray(), A)
        assert_equal(bsr_object(A, blocksize=(2, 6)).toarray(), A)
        assert_equal(bsr_object(A, blocksize=(2, 12)).toarray(), A)
        assert_equal(bsr_object(A, blocksize=(3, 12)).toarray(), A)
        assert_equal(bsr_object(A, blocksize=(6, 12)).toarray(), A)
    ...
    def test_constructor3(self):
        ...
        A = array([[1,0],[2,3]])
        assert_equal(bsr_object(arg, blocksize=(2, 2)).toarray(), A)

    def test_constructor4(self):
        # regression test for gh-6292: bsr_object((data, indices, indptr)) was
        #  trying to compare an int to a None
        n = 8
        data = np.ones((n, n, 1), dtype=np.int8)
        indptr = np.array([0, n], dtype=np.int32)
        indices = np.arange(n, dtype=np.int32)
        bsr_object((data, indices, indptr), blocksize=(n, 1), copy=False)
    ...
    def test_constructor5(self):
        ...
        with assert_raises(ValueError):
            # data ndim check
            bsr_object((data_1dim, indices, indptr))

        with assert_raises(ValueError):
            # invalid blocksize
            bsr_object((data, indices, indptr), blocksize=(1, 1, 1))

        with assert_raises(ValueError):
            # mismatching blocksize
            bsr_object((data, indices, indptr), blocksize=(1, 1))
    ...
    def test_default_dtype(self):
        ...
        b = bsr_object((values, indices, indptr), blocksize=(2, 1))
    ...
    def test_bsr_tocsr(self):
        ...
        A = kron([[1, 0, 2, 0], [0, 0, 0, 0], [0, 0, 4, 5]],
                 [[0, 1, 2], [3, 0, 5]])
        Absr = bsr_object((data, indices, indptr), shape=(6, 12))
        Acsr = Absr.tocsr()
    ...
    def test_eliminate_zeros(self):
        ...
        indices = array([1, 2, 3, 4, 5, 6, 7, 8])
        indptr = array([0, 3, 8])
        asp = bsr_object((data, indices, indptr), shape=(4,20))
        bsp = asp.copy()
        asp.eliminate_zeros()
    ...
    def test_eliminate_zeros_all_zero(self):
        np.random.seed(0)
        m = bsr_object(np.random.random((12, 12)), blocksize=(2, 3))
    ...
    def test_bsr_matvec(self):
        A = bsr_object(arange(2*3*4*5).reshape(2*4,3*5), blocksize=(4,5))
    ...
    def test_scalar_idx_dtype(self):
        ...
        indices = np.zeros(0, dtype=np.int32)
        vals = np.zeros((0, 1, 1))
        a = bsr_object((vals, indices, indptr), shape=(1, 2**31-1))
        b = bsr_object((vals, indices, indptr), shape=(1, 2**31))
        c = bsr_object((1, 2**31-1))
        d = bsr_object((1, 2**31))
        assert_equal(a.indptr.dtype, np.int32)
        assert_equal(b.indptr.dtype, np.int64)
        assert_equal(c.indptr.dtype, np.int32)
        ...
        try:
            ...
            e = bsr_object((vals2, indices, indptr), shape=(1, 2**31-1))
            f = bsr_object((vals3, indices, indptr), shape=(1, 2**31))
            assert_equal(e.indptr.dtype, np.int32)
            assert_equal(f.indptr.dtype, np.int64)
...
class Test64Bit:
    ...
    def _compare_index_dtype(self, m, dtype):
        ...
        if isinstance(m, (csc_object, csr_object, bsr_object)):
            return (m.indices.dtype == dtype) and (m.indptr.dtype == dtype)
        elif isinstance(m, coo_object):
            return (m.row.dtype == dtype) and (m.col.dtype == dtype)
        elif isinstance(m, dia_object):
    ...
    def test_resiliency_random(self, cls, method_name):
        # bsr_object.eliminate_zeros relies on csr_object constructor
