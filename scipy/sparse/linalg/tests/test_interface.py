...
class TestDotTests:
    ...
    def check_matvec(
        ...
        check_operator: bool = False, check_dot: bool = False
    ):
        """
        ...
        `matvec` and `*`, and between `rmatvec` and the composition of `.H` and `*`.
        ...
        rtol = 1000 * np.finfo(data_dtype).eps
    ...
    def check_matmat(
        ...
        If `check_operators` is set to `True`, equivalence is checked between
        `matmat` and `@`, and between `rmatvec` and the composition of `.H` and `@`.
        ...
        rtol = 100000 * np.finfo(data_dtype).eps
        assert_allclose(op_U_H_V, UH_opH_V, rtol=rtol)
    ...
    def test_scaling_square(self, args):
        ...
        self.check_matvec(
            ...
            check_operator=True, check_dot=True
        )
        self.check_matmat(
            op, data_dtype=args.data_dtype, complex_data=args.complex,
            check_operator=True, check_dot=True
