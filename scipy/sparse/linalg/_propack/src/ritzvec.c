              ...
              float* restrict V, const int ldv, float* restrict work, const int in_lwrk, int* restrict iwork)
{
    ...
    sgemm_ovwr(1, dim, dim+1, dim, 1.0f, &work[ip], dim, 0.0f, &work[imt], dim+1, &work[iwrk], lwrk);

    if (jobu) {
        mstart = (which == 0) ? dim - k : 0;  // smallest : largest
        // Form left Ritz-vectors, U = U * X^T
        sgemm_ovwr_left(1, m, k, dim+1, 1.0f, U, ldu, &work[imt + mstart], dim+1, &work[iwrk], lwrk);
    }
    ...
    if (jobv)
    {
        ...
        sgemm_ovwr_left(1, n, k, dim, 1.0f, V, ldv, &work[iqt + mstart], dim, &work[iwrk], lwrk);
    }
}
              ...
              double* restrict V, const int ldv, double* restrict work, const int in_lwrk, int* restrict iwork)
{
    ...
    // Compute left singular vectors for B, X = P^T * M^T
    dgemm_ovwr(1, dim, dim+1, dim, 1.0, &work[ip], dim, 0.0, &work[imt], dim+1, &work[iwrk], lwrk);

    if (jobu) {
        mstart = (which == 0) ? dim - k : 0;  // smallest : largest
        // Form left Ritz-vectors, U = U * X^T
        dgemm_ovwr_left(1, m, k, dim+1, 1.0, U, ldu, &work[imt + mstart], dim+1, &work[iwrk], lwrk);
    }
    ...
    if (jobv)
    {
        mstart = (which == 0) ? dim - k : 0;  // smallest : largest
        // Form right Ritz-vectors, V = V * Q
        dgemm_ovwr_left(1, n, k, dim, 1.0, V, ldv, &work[iqt + mstart], dim, &work[iwrk], lwrk);
    }
}
              ...
              PROPACK_CPLXF_TYPE* restrict cwork, const int lcwrk, int* restrict iwork)
{
    ...
    if (jobu) {
        mstart = (which == 0) ? dim - k : 0;  // smallest : largest
        // Form left Ritz-vectors, U = U * X^T
        csgemm_ovwr_left(1, m, k, dim+1, U, ldu, &work[imt + mstart], dim+1, cwork, lcwrk);
    }
              ...
              PROPACK_CPLX_TYPE* restrict zwork, const int lzwrk, int* restrict iwork)
{
    ...
    if (jobu) {
        mstart = (which == 0) ? dim - k : 0;  // smallest : largest
        // Form left Ritz-vectors, U = U * X^T
        zdgemm_ovwr_left(1, m, k, dim+1, U, ldu, &work[imt + mstart], dim+1, zwork, lzwrk);
    }

    if (jobv) {
        mstart = (which == 0) ? dim - k : 0;  // smallest : largest
        // Form right Ritz-vectors, V = V * Q
        zdgemm_ovwr_left(1, n, k, dim, V, ldv, &work[iqt + mstart], dim, zwork, lzwrk);
    }
}
