...
from scipy._lib._array_api import (
    is_lazy_array, xp_capabilities, xp_copy, xp_vector_norm, is_pydata_sparse_array,
)
...
def cg(A, b, x0=None, *, rtol=1e-5, atol=0., maxiter=None, M=None, callback=None):
    ...
    batch of lower-dimensional slices; see :ref:`linalg_batch`
    and `LinearOperator` for details.
    ...
    # NOTE: would be `b.ndim > 1` without column vectors
    ...
    x, b = map(functools.partial(xp.broadcast_to, shape=result_shape), [x, b])
    x = xp_copy(x, xp=xp)
    if not xp.any(bnrm2):
        return b, 0
