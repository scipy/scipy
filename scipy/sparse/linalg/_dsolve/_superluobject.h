...
#define _CHECK_INTEGER(x)                                                                                              \
    (PyArray_ISINTEGER((PyArrayObject *) x) && PyArray_ITEMSIZE((PyArrayObject *) x) == sizeof(int))
...
typedef struct {
    PyObject_HEAD int jmpbuf_valid;
    jmp_buf jmpbuf;
...
#define SLU_BEGIN_THREADS                                                                                              \
    do {                                                                                                               \
        if (_save == NULL)                                                                                             \
            _save = PyEval_SaveThread();                                                                               \
    } while (0)
#define SLU_END_THREADS                                                                                                \
    do {                                                                                                               \
        if (_save) {                                                                                                   \
...
#define CHECK_SLU_TYPE(type) (type == NPY_FLOAT || type == NPY_DOUBLE || type == NPY_CFLOAT || type == NPY_CDOUBLE)
...
#define TYPE_GENERIC_FUNC(name, returntype)                                                                            \
    ...
    static returntype name(int type, name##_ARGS) {                                                                    \
        switch (type) {                                                                                                \
        case NPY_FLOAT:                                                                                                \
            s##name(name##_ARGS_REF);                                                                                  \
            break;                                                                                                     \
        case NPY_DOUBLE:                                                                                               \
            d##name(name##_ARGS_REF);                                                                                  \
            break;                                                                                                     \
        case NPY_CFLOAT:                                                                                               \
            c##name(name##_ARGS_REF);                                                                                  \
            break;                                                                                                     \
        case NPY_CDOUBLE:                                                                                              \
            z##name(name##_ARGS_REF);                                                                                  \
            break;                                                                                                     \
        default:                                                                                                       \
            return;                                                                                                    \
        }                                                                                                              \
...
#define gstrf_ARGS                                                                                                     \
    superlu_options_t *a, SuperMatrix *b, int c, int d, int *e, void *f, int g, int *h, int *i, SuperMatrix *j,        \
        SuperMatrix *k, GlobalLU_t *l, SuperLUStat_t *m, int *n
#define gstrf_ARGS_REF a, b, c, d, e, f, g, h, i, j, k, l, m, n

#define gsitrf_ARGS gstrf_ARGS
#define gsitrf_ARGS_REF gstrf_ARGS_REF

#define gstrs_ARGS trans_t a, SuperMatrix *b, SuperMatrix *c, int *d, int *e, SuperMatrix *f, SuperLUStat_t *g, int *h
#define gstrs_ARGS_REF a, b, c, d, e, f, g, h

#define gssv_ARGS                                                                                                      \
    superlu_options_t *a, SuperMatrix *b, int *c, int *d, SuperMatrix *e, SuperMatrix *f, SuperMatrix *g,              \
        SuperLUStat_t *h, int *i
#define gssv_ARGS_REF a, b, c, d, e, f, g, h, i

#define Create_Dense_Matrix_ARGS SuperMatrix *a, int b, int c, void *d, int e, Stype_t f, Dtype_t g, Mtype_t h
#define Create_Dense_Matrix_ARGS_REF a, b, c, d, e, f, g, h

#define Create_CompRow_Matrix_ARGS                                                                                     \
    SuperMatrix *a, int b, int c, int d, void *e, int *f, int *g, Stype_t h, Dtype_t i, Mtype_t j
...
#define Create_SuperNode_Matrix_ARGS                                                                                   \
    SuperMatrix *a, int b, int c, int d, void *e, int *f, int *g, int *h, int *i, int *j, Stype_t k, Dtype_t l,        \
        Mtype_t m
