...
class LinearOperator:
    ...
    matmat : callable f(V)
        Returns ``A @ V``,
        where ``V`` is a dense matrix with dimensions ``(..., N, K)``.
    dtype : dtype
        Data type of the matrix.
    rmatmat : callable f(V)
        Returns ``A^H @ V``,
        where ``V`` is a dense matrix with dimensions ``(..., M, K)``.
    ...
    def _matmat(self, X):
        ...
        # NOTE: we can't use `matvec` directly as we can't assume that user-defined
        # `matvec` functions support batching.
        return np.concat(
    ...
    def _rdot(self, x):
        ...
        if isinstance(x, LinearOperator):
            ...
            # otherwise, treat input as a matrix if the shape fits
            matrix = x.ndim >= 2 and x.shape[-2] == M
            # otherwise, treat as a row-vector
...
class _AdjointMatrixOperator(MatrixLinearOperator):
    def __init__(self, adjoint_array):
        if adjoint_array.ndim > 2:
            if issparse(adjoint_array):
                A_T = sparse.swapaxes(adjoint_array, -1, -2)
            else:
                A_T = adjoint_array.mT
        else:
            A_T = adjoint_array.T
        self.A = A_T.conj()
