...
c        %--------------------------------------------------%
c
         call ccopy(kplusp,ritz,1,workl(kplusp2+1),1)
         call ccopy(kplusp,bounds,1,workl(kplusp2+kplusp+1),1)
c
c        %---------------------------------------------------%
c        | Select the wanted Ritz values and their bounds    |
c        | to be used in the convergence test.               |
...
c
            if (msglvl .gt. 4) then
               call cvout(logfil, kplusp, workl(kplusp2+1), ndigit,
     &             '_naup2: Eigenvalues computed by _neigh:')
               call cvout(logfil, kplusp, workl(kplusp2+kplusp+1),
     &                     ndigit,
