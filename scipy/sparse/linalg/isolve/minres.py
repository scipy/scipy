...
def minres(A, b, x0=None, shift=0.0, tol=1e-5, maxiter=None, xtype=None,
    ...
    if show:
        print_(first + 'Solution of symmetric Ax = b')
        print_(first + 'n      =  %3g     shift  =  %23.14e'  % (n,shift))
        print_(first + 'itnlim =  %3g     rtol   =  %11.2e'   % (maxiter,tol))
        print_()
    ...
    if show:
        print_()
        print_()
        print_('   Itn     x(1)     Compatible    LS       norm(A)  cond(A) gbar/|A|')
    ...
    if show:
        print_()
        print_(last + ' istop   =  %3g               itn   =%5g' % (istop,itn))
        print_(last + ' Anorm   =  %12.4e      Acond =  %12.4e'  % (Anorm,Acond))
        print_(last + ' rnorm   =  %12.4e      ynorm =  %12.4e'  % (rnorm,ynorm))
        print_(last + ' Arnorm  =  %12.4e'                       %  (Arnorm,))
        print_(last + msg[istop+1])
