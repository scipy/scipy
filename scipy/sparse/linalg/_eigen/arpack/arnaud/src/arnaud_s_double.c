...
static ARNAUD_INT sortr_LM(const double, const double);
static ARNAUD_INT sortr_SM(const double, const double);
static ARNAUD_INT sortr_LA(const double, const double);
static ARNAUD_INT sortr_SA(const double, const double);

static const double unfl = DBL_MIN;    // 2.2250738585072014e-308;
static const double ulp = DBL_EPSILON; // 2.220446049250313e-16;

static void dsaup2(struct ARNAUD_state_d*, double*, double*, ARNAUD_INT, double*, ARNAUD_INT, double*, double*, double*, ARNAUD_INT, double*, ARNAUD_INT*, double*);
static void dsconv(ARNAUD_INT, double*, double*, double, ARNAUD_INT*);
static void dseigt(double, ARNAUD_INT, double*, ARNAUD_INT, double*, double*, double*, ARNAUD_INT*);
static void dsaitr(struct ARNAUD_state_d*, ARNAUD_INT, ARNAUD_INT, double*, double*, double*, ARNAUD_INT, double*, ARNAUD_INT, ARNAUD_INT*, double*);
static void dsapps(ARNAUD_INT, ARNAUD_INT*, ARNAUD_INT, double*, double*, ARNAUD_INT, double*, ARNAUD_INT, double*, double* , ARNAUD_INT, double*);
static void dsgets(struct ARNAUD_state_d*, ARNAUD_INT*, ARNAUD_INT*, double*, double*, double*);
static void dgetv0(struct ARNAUD_state_d *, ARNAUD_INT, ARNAUD_INT, ARNAUD_INT, double*, ARNAUD_INT, double*, double*, ARNAUD_INT*, double*);
static void dsortr(const enum ARNAUD_which w, const ARNAUD_INT apply, const ARNAUD_INT n, double* x1, double* x2);
static void dsesrt(const enum ARNAUD_which w, const ARNAUD_INT apply, const ARNAUD_INT n, double* x, ARNAUD_INT na, double* a, const ARNAUD_INT lda);
static void dstqrb(ARNAUD_INT n, double* d, double* e, double* z, double* work, ARNAUD_INT* info);
...
void
ARNAUD_dseupd(struct ARNAUD_state_d *V, ARNAUD_INT rvec, ARNAUD_INT howmny, ARNAUD_INT* select,
       double* d, double* z, ARNAUD_INT ldz, double sigma, double* resid, double* v,
       ARNAUD_INT ldv, ARNAUD_INT* ipntr, double* workd, double* workl)
{
    ...
    if (V->bmat)
    {
        bnorm2 = ARNAUD_BLAS(dnrm2)(&V->n, workd, &int1);
    } else {
        bnorm2 = rnorm;
    }
    ...
    if (rvec) {
        ...
        tmp_int = V->ncv - 1;
        ARNAUD_BLAS(dcopy)(&tmp_int, &workl[ih+1], &int1, &workl[ihb], &int1);
        ARNAUD_BLAS(dcopy)(&V->ncv, &workl[ih+ldh], &int1, &workl[ihd], &int1);

        ARNAUD_BLAS(dsteqr)("I", &V->ncv, &workl[ihd], &workl[ihb], &workl[iq], &ldq, &workl[iw], &ierr);
            ...
            if (V->ncv > 1)
            {
                do
                {
                    if (select[leftptr])
                    {
                        ...
                        ARNAUD_BLAS(dcopy)(&V->ncv, &workl[iq + V->ncv*leftptr], &int1, &workl[iw], &int1);
                        ARNAUD_BLAS(dcopy)(&V->ncv, &workl[iq + V->ncv*rightptr], &int1, &workl[iq + V->ncv*leftptr], &int1);
                        ARNAUD_BLAS(dcopy)(&V->ncv, &workl[iw], &int1, &workl[iq + V->ncv*rightptr], &int1);
        ...
        ARNAUD_BLAS(dcopy)(&V->nconv, &workl[ihd], &int1, d, &int1);

    } else {

        // Ritz vectors not required. Load Ritz values into D.

        ARNAUD_BLAS(dcopy)(&V->nconv, &workl[ritz], &int1, d, &int1);
        ARNAUD_BLAS(dcopy)(&V->ncv, &workl[ritz], &int1, &workl[ihd], &int1);
    }
    ...
    if (TYP == REGULAR)
    {
        ...
        if (rvec) {
            ...
            ARNAUD_BLAS(dcopy)(&V->ncv, &workl[bounds], &int1, &workl[ihb], &int1);
        }
        ...
        ARNAUD_BLAS(dcopy)(&V->ncv, &workl[ihd], &int1, &workl[iw], &int1);
        if (TYP == SHIFTI)
        {
        ...
        if (rvec) {
            ...
            ARNAUD_BLAS(dcopy)(&V->ncv, &workl[bounds], &int1, &workl[ihb], &int1);
            temp = bnorm2 / rnorm;
            ARNAUD_BLAS(dscal)(&V->ncv, &temp, &workl[ihb], &int1);
    ...
    if ((rvec) && (howmny == 0))
    {
        ...
        ARNAUD_BLAS(dgeqr2)(&V->ncv, &V->nconv, &workl[iq], &ldq, &workl[iw + V->ncv], &workl[ihb], &ierr);
        ...
        ARNAUD_BLAS(dorm2r)("R", "N", &V->n, &V->ncv, &V->nconv, &workl[iq], &ldq, &workl[iw + V->ncv], v, &ldv, &workd[V->n], &ierr);
        ARNAUD_BLAS(dlacpy)("A", &V->n, &V->nconv, v, &ldv, z, &ldz);
        ...
        ARNAUD_BLAS(dorm2r)("L", "T", &V->ncv, &int1, &V->nconv, &workl[iq], &ldq, &workl[iw + V->ncv], &workl[ihb], &V->ncv, &temp, &ierr);
    ...
    if ((TYP == REGULAR) && (rvec))
    {
        ...
        ARNAUD_BLAS(dscal)(&V->ncv, &bnorm2, &workl[ihb], &int1);
    ...
    if ((rvec) && (TYP != REGULAR))
    {
        ARNAUD_BLAS(dger)(&V->n, &V->nconv, &dbl1, resid, &int1, &workl[iw], &int1, z, &ldz);
    }
...
void
ARNAUD_dsaupd(struct ARNAUD_state_d *V, double* resid, double* v, ARNAUD_INT ldv,
       ARNAUD_INT* ipntr, double* workd, double* workl)
{
...
void
dsaup2(struct ARNAUD_state_d *V, double* resid, double* v, ARNAUD_INT ldv,
       double* h, ARNAUD_INT ldh, double* ritz, double* bounds,
       double* q, ARNAUD_INT ldq, double* workl, ARNAUD_INT* ipntr, double* workd)
{
...
LINE20:
    ...
    ARNAUD_BLAS(dcopy)(&V->aup2_kplusp, ritz, &int1, &workl[V->aup2_kplusp], &int1);
    ARNAUD_BLAS(dcopy)(&V->aup2_kplusp, bounds, &int1, &workl[2*V->aup2_kplusp], &int1);
    ...
    ARNAUD_BLAS(dcopy)(&V->aup2_nev, &bounds[V->np], &int1, &workl[V->np], &int1);
    ...
    if ((V->nconv >= V->aup2_nev0) || (V->aup2_iter > V->maxiter) || (V->np == 0))
    {
        ...
        if (V->which == which_BE)
        {
            ...
            if (V->aup2_nev > 1)
            {
                ...
                ARNAUD_BLAS(dswap)(&tmp_int, &ritz[nevm2], &int1, &ritz[tmp_int2], &int1);
                ARNAUD_BLAS(dswap)(&tmp_int, &bounds[nevm2], &int1, &bounds[tmp_int2], &int1);
            }
...
LINE50:
    ...
    if (V->shift == 0) { ARNAUD_BLAS(dcopy)(&V->np, workl, &int1, ritz, &int1); }
    ...
    if (V->bmat)
    {
        ARNAUD_BLAS(dcopy)(&V->n, resid, &int1, &workd[V->n], &int1);
        ipntr[0] = V->n;
        ipntr[1] = 0;
        V->ido = ido_BX;
        ...
        ARNAUD_BLAS(dcopy)(&V->n, resid, &int1, workd, &int1);
    }
...
LINE100:
    ...
    if (V->bmat)
    {
        V->aup2_rnorm = sqrt(fabs(ARNAUD_BLAS(ddot)(&V->n, resid, &int1, workd, &int1)));
    } else {
        V->aup2_rnorm = ARNAUD_BLAS(dnrm2)(&V->n, resid, &int1);
    }
...
void
dsconv(ARNAUD_INT n, double *ritz, double *bounds, double tol, ARNAUD_INT* nconv)
{
    const double eps23 = pow(ulp, 2.0 / 3.0);
    double temp = 0.0;
...
void
dseigt(double rnorm, ARNAUD_INT n, double* h, ARNAUD_INT ldh, double* eig, double* bounds,
       double* workl, ARNAUD_INT* ierr)
{
    ARNAUD_INT int1 = 1, tmp_int;
    ARNAUD_BLAS(dcopy)(&n, &h[ldh], &int1, eig, &int1);
    tmp_int = n - 1;
    ARNAUD_BLAS(dcopy)(&tmp_int, &h[1], &int1, workl, &int1);
    dstqrb(n, eig, workl, bounds, &workl[n], ierr);
    if (*ierr != 0) { return; }
    for (ARNAUD_INT k = 0; k < n; k++) { bounds[k] = rnorm * fabs(bounds[k]); }
    return;
}


void
dsaitr(struct ARNAUD_state_d *V, ARNAUD_INT k, ARNAUD_INT np, double* resid, double* rnorm,
       double* v, ARNAUD_INT ldv, double* h, ARNAUD_INT ldh, ARNAUD_INT* ipntr, double* workd)
{
...
LINE40:
    ...
    ARNAUD_BLAS(dcopy)(&n, resid, &int1, &v[ldv*(V->aitr_j)], &int1);
    if (*rnorm >= unfl)
    {
        temp1 = 1.0 / *rnorm;
        ARNAUD_BLAS(dscal)(&n, &temp1, &v[ldv*(V->aitr_j)], &int1);
        ARNAUD_BLAS(dscal)(&n, &temp1, &workd[ipj], &int1);
    } else {
        ARNAUD_BLAS(dlascl)("G", &i, &i, rnorm, &dbl1, &n, &int1, &v[ldv*(V->aitr_j)], &n, &infol);
        ARNAUD_BLAS(dlascl)("G", &i, &i, rnorm, &dbl1, &n, &int1, &workd[ipj], &n, &infol);
    }
    ...
    V->aitr_step3 = 1;
    ARNAUD_BLAS(dcopy)(&n, &v[ldv*(V->aitr_j)], &int1, &workd[ivj], &int1);
    ipntr[0] = ivj;
    ipntr[1] = irj;
    ipntr[2] = ipj;
    V->ido = ido_OPX;
...
LINE50:
    ...
    ARNAUD_BLAS(dcopy)(&n, &workd[irj], &int1, resid, &int1);
    ...
    if (V->bmat)
    {
        ...
        ARNAUD_BLAS(dcopy)(&n, resid, &int1, &workd[ipj], &int1);
    }
...
LINE65:
    ...
    if (V->mode == 2)
    {
        ...
        V->aitr_wnorm = ARNAUD_BLAS(ddot)(&n, resid, &int1, &workd[ivj], &int1);
        V->aitr_wnorm = sqrt(fabs(V->aitr_wnorm));
    } else if (V->bmat) {
        V->aitr_wnorm = ARNAUD_BLAS(ddot)(&n, resid, &int1, &workd[ipj], &int1);
        V->aitr_wnorm = sqrt(fabs(V->aitr_wnorm));
    } else {
        V->aitr_wnorm = ARNAUD_BLAS(dnrm2)(&n, resid, &int1);
    }
    ...
    if (V->mode != 2)
    {
        ARNAUD_BLAS(dgemv)("T", &n, &tmp_int, &dbl1, v, &ldv, &workd[ipj], &int1, &dbl0, &workd[irj], &int1);
    } else {
        ARNAUD_BLAS(dgemv)("T", &n, &tmp_int, &dbl1, v, &ldv, &workd[ivj], &int1, &dbl0, &workd[irj], &int1);
    }

    //  Orthogonalize r_{j} against V_{j}.
    //  RESID contains OP*v_{j}. See STEP 3.

    ARNAUD_BLAS(dgemv)("N", &n, &tmp_int, &dblm1, v, &ldv, &workd[irj], &int1, &dbl1, resid, &int1);
    ...
    if (V->bmat)
    {
        ARNAUD_BLAS(dcopy)(&n, resid, &int1, &workd[irj], &int1);
        ipntr[0] = irj;
        ipntr[1] = ipj;
        V->ido = ido_BX;
...
LINE70:
    ...
    if (V->bmat)
    {
        *rnorm = ARNAUD_BLAS(ddot)(&n, resid, &int1, &workd[ipj], &int1);
        *rnorm = sqrt(fabs(*rnorm));
    } else {
        *rnorm = ARNAUD_BLAS(dnrm2)(&n, resid, &int1);
    }
...
LINE90:
    ...
    if (V->bmat)
    {
        V->aitr_rnorm1 = ARNAUD_BLAS(ddot)(&n, resid, &int1, &workd[ipj], &int1);
        V->aitr_rnorm1 = sqrt(fabs(V->aitr_rnorm1));
    } else {
        V->aitr_rnorm1 = ARNAUD_BLAS(dnrm2)(&n, resid, &int1);
    }
...
LINE100:
    ...
    if (h[V->aitr_j] < 0.0)
    {
        ...
        if (V->aitr_j < k + np - 1)
        {
            ARNAUD_BLAS(dscal)(&n, &dblm1, &v[V->aitr_j + 1], &int1);
        } else {
            ARNAUD_BLAS(dscal)(&n, &dblm1, resid, &int1);
        }
    }
...
void
dsapps(ARNAUD_INT n, ARNAUD_INT* kev, ARNAUD_INT np, double* shift, double* v, ARNAUD_INT ldv, double* h, ARNAUD_INT ldh,
       double* resid, double* q, ARNAUD_INT ldq, double* workd)
{
    ...
    ARNAUD_BLAS(dlaset)("A", &kplusp, &kplusp, &dbl0, &dbl1, q, &ldq);
    ...
    for (jj = 0; jj < np; jj++)
    {
        ...
        while (istart < kplusp - 1)
        {
            ...
            if (istart == iend)
            {
                ...
                {
                    h[iend] = -h[iend];
                    ARNAUD_BLAS(dscal)(&kplusp, &dblm1, &q[ldq*(iend)], &int1);
                }
                continue;
            }
            ...
            for (i = istart; i < iend; i++)
            {
                ...
                ARNAUD_BLAS(drot)(&tmp_int, &q[ldq*i], &int1, &q[ldq*(i+1)], &int1, &c, &s);
    ...
    if (h[*kev] > 0.0)
    {
        ARNAUD_BLAS(dgemv)("N", &n, &kplusp, &dbl1, v, &ldv, &q[ldq*(*kev)], &int1, &dbl0, &workd[n], &int1);
    }
    ...
    {
        tmp_int = kplusp - i;
        ARNAUD_BLAS(dgemv)("N", &n, &tmp_int, &dbl1, v, &ldv, &q[ldq*(*kev-i-1)], &int1, &dbl0, workd, &int1);
        ARNAUD_BLAS(dcopy)(&n, workd, &int1, &v[ldv*(kplusp-i-1)], &int1);
    }
    // 130
    ...
    for (i = 0; i < *kev; i++)
    {
        ARNAUD_BLAS(dcopy)(&n, &v[ldv*(np+i)], &int1, &v[ldv*i], &int1);
    }
    // 140

    if (h[*kev] > 0.0)
    {
        ARNAUD_BLAS(dcopy)(&n, &workd[n], &int1, &v[ldv*(*kev)], &int1);
    }
    ...
    ARNAUD_BLAS(dscal)(&n, &q[kplusp-1 + (*kev-1)*ldq], resid, &int1);
    if (h[*kev] > 0.0)
    {
        ARNAUD_BLAS(daxpy)(&n, &h[*kev], &v[ldv*(*kev)], &int1, resid, &int1);
    }
...
void
dsgets(struct ARNAUD_state_d *V, ARNAUD_INT* kev, ARNAUD_INT* np, double* ritz,
       double* bounds, double* shifts)
{
    ...
    if (V->which == which_BE)
    {
        ...
        if (*kev > 1)
        {
            ...
            ARNAUD_BLAS(dswap)(&tmp1, ritz, &int1, &ritz[tmp2], &int1);
            ARNAUD_BLAS(dswap)(&tmp1, bounds, &int1, &bounds[tmp2], &int1);
        }
    } else {
    ...
    if ((V->shift == 1) && (*np > 0))
    {
        ...
        ARNAUD_BLAS(dcopy)(np, ritz, &int1, shifts, &int1);
    }
}


void
dgetv0(struct ARNAUD_state_d *V, ARNAUD_INT initv, ARNAUD_INT n, ARNAUD_INT j,
       double* v, ARNAUD_INT ldv, double* resid, double* rnorm, ARNAUD_INT* ipntr, double* workd)
{
    ...
    if (V->ido == ido_RANDOM)
    {
        ...
        if (V->getv0_itry == 1)
        {
            ipntr[0] = 0;
            ipntr[1] = n;
            ARNAUD_BLAS(dcopy)(&n, resid, &int1, workd, &int1);
            V->ido = ido_RANDOM_OPX;
            return;
        } else if ((V->getv0_itry > 1) && (V->bmat == 1))
        {
            ARNAUD_BLAS(dcopy)(&n, resid, &int1, &workd[n], &int1);
        }
    }
    ...
    if (V->getv0_itry == 1)
    {
        ARNAUD_BLAS(dcopy)(&n, &workd[n], &int1, resid, &int1);
    }
    if (V->bmat)
    {
        ipntr[0] = n;
...
LINE20:
    ...
    if (V->bmat)
    {
        V->getv0_rnorm0 = ARNAUD_BLAS(ddot)(&n, resid, &int1, workd, &int1);
        V->getv0_rnorm0 = sqrt(fabs(V->getv0_rnorm0));
    } else {
        V->getv0_rnorm0 = ARNAUD_BLAS(dnrm2)(&n, resid, &int1);
    }
...
LINE30:
    ...
    ARNAUD_BLAS(dgemv)("T", &n, &j, &dbl1, v, &ldv, workd, &int1, &dbl0, &workd[n], &int1);
    ARNAUD_BLAS(dgemv)("N", &n, &j, &dblm1, v, &ldv, &workd[n], &int1, &dbl1, resid, &int1);
...
LINE40:
    if (V->bmat)
    {
        *rnorm = ARNAUD_BLAS(ddot)(&n, resid, &int1, workd, &int1);
        *rnorm = sqrt(fabs(*rnorm));
    } else {
        *rnorm = ARNAUD_BLAS(dnrm2)(&n, resid, &int1);
    }
...
static void
dsortr(const enum ARNAUD_which w, const ARNAUD_INT apply, const ARNAUD_INT n, double* x1, double* x2)
{
...
static void
dsesrt(const enum ARNAUD_which w, const ARNAUD_INT apply, const ARNAUD_INT n, double* x, ARNAUD_INT na, double* a, const ARNAUD_INT lda)
{
    ...
    while (gap != 0)
    {
        for (i = gap; i < n; i++)
        {
            ...
            while ((pos >= 0) && (f(x[pos], x[pos + gap])))
            {
                ...
                if (apply)
                {
                    ARNAUD_BLAS(dswap)(&na, &a[lda*pos], &int1, &a[lda*(pos+gap)], &int1);
                }
                pos -= gap;
            }
        }
...
void
dstqrb(ARNAUD_INT n, double* d, double* e, double* z, double* work, ARNAUD_INT* info)
{
    ...
    *info = 0;
    ...
    while (jtot < nmaxit)
    {
        ...
        tmp_int = lend - l + 1;
        anorm = ARNAUD_BLAS(dlanst)("I", &tmp_int, &d[l-1], &e[l-1]);
        iscale = 0;
        ...
        {
            iscale = 1;
            ARNAUD_BLAS(dlascl)("G", &int0, &int0, &anorm, &ssfmax, &tmp_int, &int1, &d[l-1], &n, info);
            tmp_int -= 1;
            ARNAUD_BLAS(dlascl)("G", &int0, &int0, &anorm, &ssfmax, &tmp_int, &int1, &e[l-1], &n, info);
        } else if (anorm < ssfmin) {
            iscale = 2;
            ARNAUD_BLAS(dlascl)("G", &int0, &int0, &anorm, &ssfmin, &tmp_int, &int1, &d[l-1], &n, info);
            tmp_int -= 1;
            ARNAUD_BLAS(dlascl)("G", &int0, &int0, &anorm, &ssfmin, &tmp_int, &int1, &e[l-1], &n, info);
        }
        ...
        if (lend > l)
        {
            ...
            while (1)
            {
                ...
                if (m == l + 1)
                {
                    ARNAUD_BLAS(dlaev2)(&d[l - 1], &e[l - 1], &d[l], &rt1, &rt2, &c, &s);
                    work[l - 1] = c;
                    work[n - 1 + l - 1] = s;
                    tst    = z[l];
                    z[l]   = c*tst - s*z[l-1];
                ...
                // 70
                tmp_int = m - l + 1;
                ARNAUD_BLAS(dlasr)("R", "V", "B", &int1, &tmp_int, &work[l-1], &work[n-1+l-1], &z[l-1], &int1);
            ...
            while (1)
            {
                ...
                if (m == l - 1)
                {
                    ARNAUD_BLAS(dlaev2)(&d[l-2], &e[l-2], &d[l-1], &rt1, &rt2, &c, &s);
                    tst    = z[l-1];
                    z[l-1] = c*tst - s*z[l-2];
                    z[l-2] = s*tst + c*z[l-2];
                    d[l-2] = rt1;
                ...
                tmp_int = l - m + 1;
                ARNAUD_BLAS(dlasr)("R", "V", "F", &int1, &tmp_int, &work[m-1], &work[n-1+m-1], &z[m-1], &int1);
        ...
        if (iscale == 1)
        {
            ...
            ARNAUD_BLAS(dlascl)("G", &int0, &int0, &ssfmax, &anorm, &tmp_int, &int1, &d[lsv-1], &n, info);
            tmp_int -= 1;
            ARNAUD_BLAS(dlascl)("G", &int0, &int0, &ssfmax, &anorm, &tmp_int, &int1, &e[lsv-1], &n, info);

        } else if (iscale == 2) {

            ARNAUD_BLAS(dlascl)("G", &int0, &int0, &ssfmin, &anorm, &tmp_int, &int1, &d[lsv-1], &n, info);
            tmp_int -= 1;
            ARNAUD_BLAS(dlascl)("G", &int0, &int0, &ssfmin, &anorm, &tmp_int, &int1, &e[lsv-1], &n, info);
...
ARNAUD_INT sortr_LM(const double x1, const double x2) { return (fabs(x1) > fabs(x2)); }
ARNAUD_INT sortr_SM(const double x1, const double x2) { return (fabs(x1) < fabs(x2)); }
ARNAUD_INT sortr_LA(const double x1, const double x2) { return (x1 > x2); }
ARNAUD_INT sortr_SA(const double x1, const double x2) { return (x1 < x2); }
