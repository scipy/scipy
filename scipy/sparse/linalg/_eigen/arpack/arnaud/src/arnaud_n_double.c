...
typedef ARNAUD_INT ARNAUD_compare_cfunc(const double, const double, const double, const double);

static ARNAUD_INT sortc_LM(const double, const double, const double, const double);
static ARNAUD_INT sortc_SM(const double, const double, const double, const double);
static ARNAUD_INT sortc_LR(const double, const double, const double, const double);
static ARNAUD_INT sortc_SR(const double, const double, const double, const double);
static ARNAUD_INT sortc_LI(const double, const double, const double, const double);
static ARNAUD_INT sortc_SI(const double, const double, const double, const double);
...
static void dnaup2(struct ARNAUD_state_d*, double*, double*, ARNAUD_INT, double*, ARNAUD_INT, double*, double*, double*, double*, ARNAUD_INT, double*, ARNAUD_INT*, double*);
static void dnconv(ARNAUD_INT n, double* ritzr, double* ritzi, double* bounds, const double tol, ARNAUD_INT* nconv);
static void dneigh(double*,ARNAUD_INT,double*,ARNAUD_INT,double*,double*,double*,double*,ARNAUD_INT,double*,ARNAUD_INT*);
static void dnaitr(struct ARNAUD_state_d*,ARNAUD_INT,ARNAUD_INT,double*,double*,double*,ARNAUD_INT,double*,ARNAUD_INT,ARNAUD_INT*,double*);
static void dnapps(ARNAUD_INT,ARNAUD_INT*,ARNAUD_INT,double*,double*,double*,ARNAUD_INT,double*,ARNAUD_INT,double*,double*,ARNAUD_INT,double*,double*);
static void dngets(struct ARNAUD_state_d*,ARNAUD_INT*,ARNAUD_INT*,double*,double*,double*);
static void dsortc(const enum ARNAUD_which w, const ARNAUD_INT apply, const ARNAUD_INT n, double* xreal, double* ximag, double* y);
static void dgetv0(struct ARNAUD_state_d *V, ARNAUD_INT initv, ARNAUD_INT n, ARNAUD_INT j, double* v, ARNAUD_INT ldv, double* resid, double* rnorm, ARNAUD_INT* ipntr, double* workd);
...
void
ARNAUD_dneupd(struct ARNAUD_state_d *V, ARNAUD_INT rvec, ARNAUD_INT howmny, ARNAUD_INT* select,
       double* dr, double* di, double* z, ARNAUD_INT ldz, double sigmar, double sigmai,
       double* workev, double* resid, double* v, ARNAUD_INT ldv, ARNAUD_INT* ipntr, double* workd,
       double* workl)
{
    ...
    if (rvec) {
        ...
        tmp_int = ldh*V->ncv;
        ARNAUD_BLAS(dcopy)(&tmp_int, &workl[ih], &int1, &workl[iuptri], &int1);
        ARNAUD_BLAS(dlaset)("A", &V->ncv, &V->ncv, &dbl0, &dbl1, &workl[invsub], &ldq);
        ARNAUD_BLAS(dlahqr)(&int1, &int1, &V->ncv, &int1, &V->ncv, &workl[iuptri], &ldh,
                &workl[iheigr], &workl[iheigi], &int1, &V->ncv, &workl[invsub],
                &ldq, &ierr);
        ARNAUD_BLAS(dcopy)(&V->ncv, &workl[invsub + V->ncv - 1], &ldq, &workl[ihbds], &int1);
        ...
        if (reord)
        {
            ARNAUD_BLAS(dtrsen)("N", "V", select, &V->ncv, &workl[iuptri], &ldh, &workl[invsub], &ldq,
        ...
        if (TYP == REGULAR) {
            ARNAUD_BLAS(dcopy)(&V->nconv, &workl[iheigr], &int1, dr, &int1);
            ARNAUD_BLAS(dcopy)(&V->nconv, &workl[iheigi], &int1, di, &int1);
        }
        ...
        ARNAUD_BLAS(dgeqr2)(&V->ncv, &V->nconv, &workl[invsub], &ldq, workev, &workev[V->ncv], &ierr);
        ...
        ARNAUD_BLAS(dorm2r)("R", "N", &V->n, &V->ncv, &V->nconv, &workl[invsub], &ldq, workev,
        ...
        for (j = 0; j < V->nconv; j++)
        {
            ...
            {
                ARNAUD_BLAS(dscal)(&V->nconv, &dblm1, &workl[iuptri + j], &ldq);
                ARNAUD_BLAS(dscal)(&V->nconv, &dblm1, &workl[iuptri + j*ldq], &int1);
            }
        }
        // 20
        ...
        if (howmny == 0)
        {
            ...
            ARNAUD_BLAS(dtrevc)("R", "S", select, &V->ncv, &workl[iuptri], &ldq, vl, &int1,
            ...
            for (j = 0; j < V->nconv; j++)
            {
                if (workl[iheigi + j] == 0.0)
                {
                    ...
                    temp = 1.0 / ARNAUD_BLAS(dnrm2)(&V->ncv, &workl[invsub + j*ldq], &int1);
                    ARNAUD_BLAS(dscal)(&V->ncv, &temp, &workl[invsub + j*ldq], &int1);
                    ...
                    if (iconj == 0)
                    {
                        temp = 1.0 / hypot(ARNAUD_BLAS(dnrm2)(&V->ncv, &workl[invsub + j*ldq], &int1),
                                           ARNAUD_BLAS(dnrm2)(&V->ncv, &workl[invsub + (j+1)*ldq], &int1));
                        ARNAUD_BLAS(dscal)(&V->ncv, &temp, &workl[invsub + j*ldq], &int1);
                        ARNAUD_BLAS(dscal)(&V->ncv, &temp, &workl[invsub + (j+1)*ldq], &int1);
                        iconj = 1;
                    } else {
                        iconj = 0;
                    }
            ...
            ARNAUD_BLAS(dgemv)("T", &V->ncv, &V->nconv, &dbl1, &workl[invsub], &ldq, &workl[ihbds], &int1, &dbl0, workev, &int1);
            ...
            ARNAUD_BLAS(dcopy)(&V->nconv, workev, &int1, &workl[ihbds], &int1);
            ...
            ARNAUD_BLAS(dorm2r)("R", "N", &V->n, &V->ncv, &V->nconv, &workl[invsub], &ldq,
                    workev, z, &ldz, &workd[V->n], &ierr);

            ARNAUD_BLAS(dtrmm)("R", "U", "N", "N", &V->n, &V->nconv, &dbl1, &workl[invsub], &ldq, z, &ldz);
        ...
        ARNAUD_BLAS(dcopy)(&V->nconv, &workl[ritzr], &int1, dr, &int1);
        ARNAUD_BLAS(dcopy)(&V->nconv, &workl[ritzi], &int1, di, &int1);
        ARNAUD_BLAS(dcopy)(&V->nconv, &workl[ritzr], &int1, &workl[iheigr], &int1);
        ARNAUD_BLAS(dcopy)(&V->nconv, &workl[ritzi], &int1, &workl[iheigi], &int1);
        ARNAUD_BLAS(dcopy)(&V->nconv, &workl[bounds], &int1, &workl[ihbds], &int1);
    }
    ...
    if (TYP == REGULAR)
    {
        if (rvec)
        {
            ARNAUD_BLAS(dscal)(&V->ncv, &rnorm, &workl[ihbds], &int1);
        }
    } else {
    ...
    if ((rvec) && (howmny == 0) && (TYP == SHIFTI))
    {
        ...
        ARNAUD_BLAS(dger)(&V->n, &V->nconv, &dbl1, resid, &int1, workev, &int1, z, &ldz);
    }
...
void
dnaup2(struct ARNAUD_state_d *V, double* resid, double* v, ARNAUD_INT ldv,
       double* h, ARNAUD_INT ldh, double* ritzr, double* ritzi, double* bounds,
...
LINE20:
    ...
    ARNAUD_BLAS(dcopy)(&V->aup2_kplusp, ritzr, &int1, &workl[tmp_int], &int1);
    tmp_int += V->aup2_kplusp;
    ARNAUD_BLAS(dcopy)(&V->aup2_kplusp, ritzi, &int1, &workl[tmp_int], &int1);
    tmp_int += V->aup2_kplusp;
    ARNAUD_BLAS(dcopy)(&V->aup2_kplusp, bounds, &int1, &workl[tmp_int], &int1);
    ...
    ARNAUD_BLAS(dcopy)(&V->aup2_nev, &bounds[V->np], &int1, &workl[2*V->np], &int1);
...
LINE50:
    ...
    if (V->shift == 0)
    {
        ...
        ARNAUD_BLAS(dcopy)(&V->np, workl, &int1, ritzr, &int1);
        ARNAUD_BLAS(dcopy)(&V->np, &workl[V->np], &int1, ritzi, &int1);
    }
...
LINE100:
    ...
    if (V->bmat)
    {
        V->aup2_rnorm = ARNAUD_BLAS(ddot)(&V->n, resid, &int1, workd, &int1);
...
void
dnconv(ARNAUD_INT n, double* ritzr, double* ritzi, double* bounds, const double tol, ARNAUD_INT* nconv)
{
    const double eps23 = pow(ulp, 2.0 / 3.0);
    double temp;
...
void
dneigh(double* rnorm, ARNAUD_INT n, double* h, ARNAUD_INT ldh, double* ritzr, double* ritzi,
       double* bounds, double* q, ARNAUD_INT ldq, double* workl, ARNAUD_INT* ierr)
{
    ...
    ARNAUD_BLAS(dlacpy)("A", &n, &n, h, &ldh, workl, &n);
    for (j = 0; j < n-1; j++)
    {
        bounds[j] = 0.0;
    }
    bounds[n-1] = 1.0;
    ARNAUD_BLAS(dlahqr)(&int1, &int1, &n, &int1, &n, workl, &n, ritzr, ritzi, &int1, &int1, bounds, &int1, ierr);
    ...
    ARNAUD_BLAS(dtrevc)("R", "A", select, &n, workl, &n, vl, &n, q, &ldq, &n, &n, &workl[n*n], ierr);
    if (*ierr != 0) { return; }
    ...
    for (i = 0; i < n; i++)
    {
        if (fabs(ritzi[i]) == 0.0)
        {
            ...
            temp = ARNAUD_BLAS(dnrm2)(&n, &q[ldq*i], &int1);
            tmp_dbl = 1.0 / temp;
            ARNAUD_BLAS(dscal)(&n, &tmp_dbl, &q[ldq*i], &int1);
            ...
            if (iconj == 0)
            {
                temp = hypot(ARNAUD_BLAS(dnrm2)(&n, &q[ldq*i], &int1),
                             ARNAUD_BLAS(dnrm2)(&n, &q[ldq*(i+1)], &int1));
                tmp_dbl = 1.0 / temp;
                ARNAUD_BLAS(dscal)(&n, &tmp_dbl, &q[ldq*i], &int1);
                ARNAUD_BLAS(dscal)(&n, &tmp_dbl, &q[ldq*(i+1)], &int1);
                iconj = 1;
            } else {
                iconj = 0;
            }
    ...
    ARNAUD_BLAS(dgemv)("T", &n, &n, &dbl1, q, &ldq, bounds, &int1, &dbl0, workl, &int1);
...
void
dnaitr(struct ARNAUD_state_d *V, ARNAUD_INT k, ARNAUD_INT np, double* resid, double* rnorm,
...
LINE60:
    ...
    tmp_int = V->aitr_j + 1;
    ARNAUD_BLAS(dgemv)("T", &n, &tmp_int, &dbl1, v, &ldv, &workd[ipj], &int1, &dbl0, &h[ldh*(V->aitr_j)], &int1);

    //  Orthogonalize r_{j} against V_{j}.
    //  RESID contains OP*v_{j}. See STEP 3.

    ARNAUD_BLAS(dgemv)("N", &n, &tmp_int, &dblm1, v, &ldv, &h[ldh*(V->aitr_j)], &int1, &dbl1, resid, &int1);
...
LINE80:
    ...
    ARNAUD_BLAS(daxpy)(&tmp_int, &dbl1, &workd[irj], &int1, &h[ldh*(V->aitr_j)], &int1);
...
LINE100:
    ...
    if (V->aitr_j >= k + np)
    {
        ...
        for (i = (k > 0 ? k-1 : k); i < k + np - 1; i++)
        {
            ...
            {
                tmp_int = k + np;
                tst1 = ARNAUD_BLAS(dlanhs)("1", &tmp_int, h, &ldh, &workd[n]);
            }
...
void
dnapps(ARNAUD_INT n, ARNAUD_INT* kev, ARNAUD_INT np, double* shiftr, double* shifti, double* v,
       ARNAUD_INT ldv, double* h, ARNAUD_INT ldh, double* resid, double* q, ARNAUD_INT ldq, double* workl,
       double* workd)
{
    ARNAUD_INT cconj;
    ...
    for (jj = 0; jj < np; jj++)
    {
        ...
        while (istart < kplusp - 1)
        {
            for (iend = istart; iend < kplusp - 1; iend++)
            {
                ...
                {
                    tmp_int = kplusp - jj;
                    tst1 = ARNAUD_BLAS(dlanhs)("1", &tmp_int, h, &ldh, workl);
                }
            ...
            if (fabs(sigmai) == 0.0)
            {
                ...
                for (i = istart; i < iend; i++)
                {
                    ...
                    tmp_int = kplusp - i;
                    ARNAUD_BLAS(drot)(&tmp_int, &h[i + ldh*i], &ldh, &h[i + 1 + ldh*i], &ldh, &c, &s);
                    tmp_int = (i+2 > iend ? iend : i + 2) + 1;
                    ARNAUD_BLAS(drot)(&tmp_int, &h[ldh*i], &int1, &h[ldh*(i+1)], &int1, &c, &s);
                ...
                for (i = istart; i < iend; i++)
                {
                    nr = iend - i + 1;
                    nr = (nr > 3? 3 : nr);
                    ARNAUD_BLAS(dlarfg)(&nr, &u[0], &u[1], &int1, &tau);
                    if (i > istart)
                    {
                        h[i + (i - 1) * ldh] = u[0];
                        h[i + 1 + (i - 1) * ldh] = 0.0;
                    ...
                    tmp_int = kplusp - i;
                    ARNAUD_BLAS(dlarf)("L", &nr, &tmp_int, u, &int1, &tau, &h[i + ldh*i], &ldh, workl);
                    ir = (i + 3 > iend ? iend : i + 3) + 1;
                    ARNAUD_BLAS(dlarf)("R", &ir, &nr, u, &int1, &tau, &h[ldh*i], &ldh, workl);
                    ARNAUD_BLAS(dlarf)("R", &kplusp, &nr, u, &int1, &tau, &q[ldq*i], &ldq, workl);
                    if (i < iend - 1)
                    {
                        u[0] = h[i+1 + i * ldh];
                        u[1] = h[i+2 + i * ldh];
    ...
    for (j = 0; j < *kev; j++)
    {
        ...
        {
            tmp_int = kplusp - j;
            ARNAUD_BLAS(dscal)(&tmp_int, &dblm1, &h[j+1 + ldh*j], &ldh);
            tmp_int = (j+3 > kplusp ? kplusp : j+3);
            ARNAUD_BLAS(dscal)(&tmp_int, &dblm1, &h[ldh*(j+1)], &int1);
            tmp_int = (j+np+2 > kplusp ? kplusp : j+np+2);
            ARNAUD_BLAS(dscal)(&tmp_int, &dblm1, &q[ldq*(j+1)], &int1);
        }
    }
    // 120
    ...
    for (i = 0; i < *kev; i++)
    {
        ...
        if (tst1 == 0.0)
        {
            tst1 = ARNAUD_BLAS(dlanhs)("1", kev, h, &ldh, workl);
        }
    ...
    {
        ARNAUD_BLAS(dgemv)("N", &n, &kplusp, &dbl1, v, &ldv, &q[(*kev)*ldq], &int1, &dbl0, &workd[n], &int1);
    }
    ...
    {
        tmp_int = kplusp - i;
        ARNAUD_BLAS(dgemv)("N", &n, &tmp_int, &dbl1, v, &ldv, &q[(*kev-i-1)*ldq], &int1, &dbl0, workd, &int1);
        ARNAUD_BLAS(dcopy)(&n, workd, &int1, &v[(kplusp-i-1)*ldv], &int1);
    }
    ...
    for (i = 0; i < *kev; i++)
    {
        ARNAUD_BLAS(dcopy)(&n, &v[(kplusp-*kev+i)*ldv], &int1, &v[i*ldv], &int1);
    }
    ...
    ARNAUD_BLAS(dscal)(&n, &q[kplusp-1 + ldq*(*kev-1)], resid, &int1);

    if (h[*kev + ldh*(*kev-1)] > 0.0)
    {
        ARNAUD_BLAS(daxpy)(&n, &h[*kev + ldh*(*kev-1)], &v[ldv*(*kev)], &int1, resid, &int1);
    }
...
void
dngets(struct ARNAUD_state_d *V, ARNAUD_INT* kev, ARNAUD_INT* np,
...
static void
dsortc(const enum ARNAUD_which w, const ARNAUD_INT apply, const ARNAUD_INT n, double* xreal, double* ximag, double* y)
{
