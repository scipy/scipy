...
static void snaup2(struct ARNAUD_state_s*, float*, float*, ARNAUD_INT, float*, ARNAUD_INT, float*, float*, float*, float*, ARNAUD_INT, float*, ARNAUD_INT*, float*);
static void snconv(ARNAUD_INT n, float* ritzr, float* ritzi, float* bounds, const float tol, ARNAUD_INT* nconv);
static void sneigh(float*,ARNAUD_INT,float*,ARNAUD_INT,float*,float*,float*,float*,ARNAUD_INT,float*,ARNAUD_INT*);
static void snaitr(struct ARNAUD_state_s*,ARNAUD_INT,ARNAUD_INT,float*,float*,float*,ARNAUD_INT,float*,ARNAUD_INT,ARNAUD_INT*,float*);
static void snapps(ARNAUD_INT,ARNAUD_INT*,ARNAUD_INT,float*,float*,float*,ARNAUD_INT,float*,ARNAUD_INT,float*,float*,ARNAUD_INT,float*,float*);
static void sngets(struct ARNAUD_state_s*,ARNAUD_INT*,ARNAUD_INT*,float*,float*,float*);
static void ssortc(const enum ARNAUD_which w, const ARNAUD_INT apply, const ARNAUD_INT n, float* xreal, float* ximag, float* y);
static void sgetv0(struct ARNAUD_state_s *V, ARNAUD_INT initv, ARNAUD_INT n, ARNAUD_INT j, float* v, ARNAUD_INT ldv, float* resid, float* rnorm, ARNAUD_INT* ipntr, float* workd);
...
void
ARNAUD_sneupd(struct ARNAUD_state_s *V, ARNAUD_INT rvec, ARNAUD_INT howmny, ARNAUD_INT* select,
       float* dr, float* di, float* z, ARNAUD_INT ldz, float sigmar, float sigmai,
       float* workev, float* resid, float* v, ARNAUD_INT ldv, ARNAUD_INT* ipntr, float* workd,
       float* workl)
{
    const float eps23 = powf(ulp, 2.0f / 3.0f);
    ARNAUD_INT ibd, iconj, ih, iheigr, iheigi, ihbds, iuptri, invsub, iri, irr, j, jj;
    ARNAUD_INT bounds, k, ldh, ldq, np, numcnv, reord, ritzr, ritzi;
    ARNAUD_INT iwork[1] = { 0 };
    ARNAUD_INT ierr = 0, int1 = 1, tmp_int = 0, nconv2 = 0, outncv;
    ...
    if (rvec) {
        ...
        tmp_int = ldh*V->ncv;
        ARNAUD_BLAS(scopy)(&tmp_int, &workl[ih], &int1, &workl[iuptri], &int1);
        ARNAUD_BLAS(slaset)("A", &V->ncv, &V->ncv, &dbl0, &dbl1, &workl[invsub], &ldq);
        ARNAUD_BLAS(slahqr)(&int1, &int1, &V->ncv, &int1, &V->ncv, &workl[iuptri], &ldh,
                &workl[iheigr], &workl[iheigi], &int1, &V->ncv, &workl[invsub],
                &ldq, &ierr);
        ARNAUD_BLAS(scopy)(&V->ncv, &workl[invsub + V->ncv - 1], &ldq, &workl[ihbds], &int1);
        ...
        if (reord)
        {
            ARNAUD_BLAS(strsen)("N", "V", select, &V->ncv, &workl[iuptri], &ldh, &workl[invsub], &ldq,
        ...
        if (TYP == REGULAR) {
            ARNAUD_BLAS(scopy)(&V->nconv, &workl[iheigr], &int1, dr, &int1);
            ARNAUD_BLAS(scopy)(&V->nconv, &workl[iheigi], &int1, di, &int1);
        }
        ...
        ARNAUD_BLAS(sgeqr2)(&V->ncv, &V->nconv, &workl[invsub], &ldq, workev, &workev[V->ncv], &ierr);
        ...
        ARNAUD_BLAS(sorm2r)("R", "N", &V->n, &V->ncv, &V->nconv, &workl[invsub], &ldq, workev,
        ...
        for (j = 0; j < V->nconv; j++)
        {
            ...
            {
                ARNAUD_BLAS(sscal)(&V->nconv, &dblm1, &workl[iuptri + j], &ldq);
                ARNAUD_BLAS(sscal)(&V->nconv, &dblm1, &workl[iuptri + j*ldq], &int1);
            }
        }
        // 20
        ...
        if (howmny == 0)
        {
            ...
            ARNAUD_BLAS(strevc)("R", "S", select, &V->ncv, &workl[iuptri], &ldq, vl, &int1,
            ...
            for (j = 0; j < V->nconv; j++)
            {
                if (workl[iheigi + j] == 0.0f)
                {
                    ...
                    temp = 1.0f / ARNAUD_BLAS(snrm2)(&V->ncv, &workl[invsub + j*ldq], &int1);
                    ARNAUD_BLAS(sscal)(&V->ncv, &temp, &workl[invsub + j*ldq], &int1);
                    ...
                    if (iconj == 0)
                    {
                        temp = 1.0f / hypotf(ARNAUD_BLAS(snrm2)(&V->ncv, &workl[invsub + j*ldq], &int1),
                                           ARNAUD_BLAS(snrm2)(&V->ncv, &workl[invsub + (j+1)*ldq], &int1));
                        ARNAUD_BLAS(sscal)(&V->ncv, &temp, &workl[invsub + j*ldq], &int1);
                        ARNAUD_BLAS(sscal)(&V->ncv, &temp, &workl[invsub + (j+1)*ldq], &int1);
                        iconj = 1;
                    } else {
                        iconj = 0;
                    }
            ...
            ARNAUD_BLAS(sgemv)("T", &V->ncv, &V->nconv, &dbl1, &workl[invsub], &ldq, &workl[ihbds], &int1, &dbl0, workev, &int1);
            ...
            ARNAUD_BLAS(scopy)(&V->nconv, workev, &int1, &workl[ihbds], &int1);
            ...
            ARNAUD_BLAS(sorm2r)("R", "N", &V->n, &V->ncv, &V->nconv, &workl[invsub], &ldq,
                    workev, z, &ldz, &workd[V->n], &ierr);

            ARNAUD_BLAS(strmm)("R", "U", "N", "N", &V->n, &V->nconv, &dbl1, &workl[invsub], &ldq, z, &ldz);
        ...
        ARNAUD_BLAS(scopy)(&V->nconv, &workl[ritzr], &int1, dr, &int1);
        ARNAUD_BLAS(scopy)(&V->nconv, &workl[ritzi], &int1, di, &int1);
        ARNAUD_BLAS(scopy)(&V->nconv, &workl[ritzr], &int1, &workl[iheigr], &int1);
        ARNAUD_BLAS(scopy)(&V->nconv, &workl[ritzi], &int1, &workl[iheigi], &int1);
        ARNAUD_BLAS(scopy)(&V->nconv, &workl[bounds], &int1, &workl[ihbds], &int1);
    }
    ...
    if (TYP == REGULAR)
    {
        if (rvec)
        {
            ARNAUD_BLAS(sscal)(&V->ncv, &rnorm, &workl[ihbds], &int1);
        }
    } else {
    ...
    if ((rvec) && (howmny == 0) && (TYP == SHIFTI))
    {
        ...
        ARNAUD_BLAS(sger)(&V->n, &V->nconv, &dbl1, resid, &int1, workev, &int1, z, &ldz);
    }
...
{
    ARNAUD_INT bounds, ih, iq, iw, j, ldh, ldq, next, iritzi, iritzr;
...
void
snaup2(struct ARNAUD_state_s *V, float* resid, float* v, ARNAUD_INT ldv,
       float* h, ARNAUD_INT ldh, float* ritzr, float* ritzi, float* bounds,
       float* q, ARNAUD_INT ldq, float* workl, ARNAUD_INT* ipntr, float* workd)
{
    enum ARNAUD_which temp_which;
    ARNAUD_INT int1 = 1, j, tmp_int;
    const float eps23 = powf(ulp, 2.0f / 3.0f);
    float temp = 0.0f;
...
LINE20:
    ...
    ARNAUD_BLAS(scopy)(&V->aup2_kplusp, ritzr, &int1, &workl[tmp_int], &int1);
    tmp_int += V->aup2_kplusp;
    ARNAUD_BLAS(scopy)(&V->aup2_kplusp, ritzi, &int1, &workl[tmp_int], &int1);
    tmp_int += V->aup2_kplusp;
    ARNAUD_BLAS(scopy)(&V->aup2_kplusp, bounds, &int1, &workl[tmp_int], &int1);
    ...
    ARNAUD_BLAS(scopy)(&V->aup2_nev, &bounds[V->np], &int1, &workl[2*V->np], &int1);
...
LINE50:
    ...
    if (V->shift == 0)
    {
        ...
        ARNAUD_BLAS(scopy)(&V->np, workl, &int1, ritzr, &int1);
        ARNAUD_BLAS(scopy)(&V->np, &workl[V->np], &int1, ritzi, &int1);
    }
...
LINE100:
    ...
    if (V->bmat)
    {
        V->aup2_rnorm = ARNAUD_BLAS(sdot)(&V->n, resid, &int1, workd, &int1);
...
void
snconv(ARNAUD_INT n, float* ritzr, float* ritzi, float* bounds, const float tol, ARNAUD_INT* nconv)
{
    const float eps23 = powf(ulp, 2.0f / 3.0f);
    float temp;
...
void
sneigh(float* rnorm, ARNAUD_INT n, float* h, ARNAUD_INT ldh, float* ritzr, float* ritzi,
       float* bounds, float* q, ARNAUD_INT ldq, float* workl, ARNAUD_INT* ierr)
{
    ARNAUD_INT select[1] = { 0 };
    ARNAUD_INT i, iconj, int1 = 1, j;
    ...
    ARNAUD_BLAS(slacpy)("A", &n, &n, h, &ldh, workl, &n);
    for (j = 0; j < n-1; j++)
    {
        bounds[j] = 0.0f;
    }
    bounds[n-1] = 1.0f;
    ARNAUD_BLAS(slahqr)(&int1, &int1, &n, &int1, &n, workl, &n, ritzr, ritzi, &int1, &int1, bounds, &int1, ierr);
    ...
    ARNAUD_BLAS(strevc)("R", "A", select, &n, workl, &n, vl, &n, q, &ldq, &n, &n, &workl[n*n], ierr);
    if (*ierr != 0) { return; }
    ...
    for (i = 0; i < n; i++)
    {
        if (fabsf(ritzi[i]) == 0.0f)
        {
            ...
            temp = ARNAUD_BLAS(snrm2)(&n, &q[ldq*i], &int1);
            tmp_dbl = 1.0f / temp;
            ARNAUD_BLAS(sscal)(&n, &tmp_dbl, &q[ldq*i], &int1);
            ...
            if (iconj == 0)
            {
                temp = hypotf(ARNAUD_BLAS(snrm2)(&n, &q[ldq*i], &int1),
                             ARNAUD_BLAS(snrm2)(&n, &q[ldq*(i+1)], &int1));
                tmp_dbl = 1.0f / temp;
                ARNAUD_BLAS(sscal)(&n, &tmp_dbl, &q[ldq*i], &int1);
                ARNAUD_BLAS(sscal)(&n, &tmp_dbl, &q[ldq*(i+1)], &int1);
                iconj = 1;
            } else {
                iconj = 0;
            }
    ...
    ARNAUD_BLAS(sgemv)("T", &n, &n, &dbl1, q, &ldq, bounds, &int1, &dbl0, workl, &int1);
...
void
snaitr(struct ARNAUD_state_s *V, ARNAUD_INT k, ARNAUD_INT np, float* resid, float* rnorm,
       float* v, ARNAUD_INT ldv, float* h, ARNAUD_INT ldh, ARNAUD_INT* ipntr, float* workd)
{
    ARNAUD_INT i = 0, infol, ipj, irj, ivj, jj, n, tmp_int;
...
LINE60:
    ...
    tmp_int = V->aitr_j + 1;
    ARNAUD_BLAS(sgemv)("T", &n, &tmp_int, &dbl1, v, &ldv, &workd[ipj], &int1, &dbl0, &h[ldh*(V->aitr_j)], &int1);

    //  Orthogonalize r_{j} against V_{j}.
    //  RESID contains OP*v_{j}. See STEP 3.

    ARNAUD_BLAS(sgemv)("N", &n, &tmp_int, &dblm1, v, &ldv, &h[ldh*(V->aitr_j)], &int1, &dbl1, resid, &int1);
...
LINE80:
    ...
    ARNAUD_BLAS(saxpy)(&tmp_int, &dbl1, &workd[irj], &int1, &h[ldh*(V->aitr_j)], &int1);
...
LINE100:
    ...
    if (V->aitr_j >= k + np)
    {
        ...
        for (i = (k > 0 ? k-1 : k); i < k + np - 1; i++)
        {
            ...
            {
                tmp_int = k + np;
                tst1 = ARNAUD_BLAS(slanhs)("1", &tmp_int, h, &ldh, &workd[n]);
            }
...
void
snapps(ARNAUD_INT n, ARNAUD_INT* kev, ARNAUD_INT np, float* shiftr, float* shifti, float* v,
       ARNAUD_INT ldv, float* h, ARNAUD_INT ldh, float* resid, float* q, ARNAUD_INT ldq, float* workl,
       float* workd)
{
    ARNAUD_INT cconj;
    ARNAUD_INT i, ir, j, jj, int1 = 1, istart, iend = 0, nr, tmp_int;
    ...
    for (jj = 0; jj < np; jj++)
    {
        ...
        while (istart < kplusp - 1)
        {
            for (iend = istart; iend < kplusp - 1; iend++)
            {
                ...
                {
                    tmp_int = kplusp - jj;
                    tst1 = ARNAUD_BLAS(slanhs)("1", &tmp_int, h, &ldh, workl);
                }
            ...
            if (fabsf(sigmai) == 0.0f)
            {
                ...
                for (i = istart; i < iend; i++)
                {
                    ...
                    tmp_int = kplusp - i;
                    ARNAUD_BLAS(srot)(&tmp_int, &h[i + ldh*i], &ldh, &h[i + 1 + ldh*i], &ldh, &c, &s);
                    tmp_int = (i+2 > iend ? iend : i + 2) + 1;
                    ARNAUD_BLAS(srot)(&tmp_int, &h[ldh*i], &int1, &h[ldh*(i+1)], &int1, &c, &s);
                ...
                for (i = istart; i < iend; i++)
                {
                    nr = iend - i + 1;
                    nr = (nr > 3? 3 : nr);
                    ARNAUD_BLAS(slarfg)(&nr, &u[0], &u[1], &int1, &tau);
                    if (i > istart)
                    {
                        h[i + (i - 1) * ldh] = u[0];
                        h[i + 1 + (i - 1) * ldh] = 0.0f;
                    ...
                    tmp_int = kplusp - i;
                    ARNAUD_BLAS(slarf)("L", &nr, &tmp_int, u, &int1, &tau, &h[i + ldh*i], &ldh, workl);
                    ir = (i + 3 > iend ? iend : i + 3) + 1;
                    ARNAUD_BLAS(slarf)("R", &ir, &nr, u, &int1, &tau, &h[ldh*i], &ldh, workl);
                    ARNAUD_BLAS(slarf)("R", &kplusp, &nr, u, &int1, &tau, &q[ldq*i], &ldq, workl);
                    if (i < iend - 1)
                    {
                        u[0] = h[i+1 + i * ldh];
                        u[1] = h[i+2 + i * ldh];
    ...
    for (j = 0; j < *kev; j++)
    {
        ...
        {
            tmp_int = kplusp - j;
            ARNAUD_BLAS(sscal)(&tmp_int, &dblm1, &h[j+1 + ldh*j], &ldh);
            tmp_int = (j+3 > kplusp ? kplusp : j+3);
            ARNAUD_BLAS(sscal)(&tmp_int, &dblm1, &h[ldh*(j+1)], &int1);
            tmp_int = (j+np+2 > kplusp ? kplusp : j+np+2);
            ARNAUD_BLAS(sscal)(&tmp_int, &dblm1, &q[ldq*(j+1)], &int1);
        }
    }
    // 120
    ...
    for (i = 0; i < *kev; i++)
    {
        ...
        if (tst1 == 0.0f)
        {
            tst1 = ARNAUD_BLAS(slanhs)("1", kev, h, &ldh, workl);
        }
    ...
    {
        ARNAUD_BLAS(sgemv)("N", &n, &kplusp, &dbl1, v, &ldv, &q[(*kev)*ldq], &int1, &dbl0, &workd[n], &int1);
    }
    ...
    {
        tmp_int = kplusp - i;
        ARNAUD_BLAS(sgemv)("N", &n, &tmp_int, &dbl1, v, &ldv, &q[(*kev-i-1)*ldq], &int1, &dbl0, workd, &int1);
        ARNAUD_BLAS(scopy)(&n, workd, &int1, &v[(kplusp-i-1)*ldv], &int1);
    }
    ...
    for (i = 0; i < *kev; i++)
    {
        ARNAUD_BLAS(scopy)(&n, &v[(kplusp-*kev+i)*ldv], &int1, &v[i*ldv], &int1);
    }
    ...
    ARNAUD_BLAS(sscal)(&n, &q[kplusp-1 + ldq*(*kev-1)], resid, &int1);

    if (h[*kev + ldh*(*kev-1)] > 0.0f)
    {
        ARNAUD_BLAS(saxpy)(&n, &h[*kev + ldh*(*kev-1)], &v[ldv*(*kev)], &int1, resid, &int1);
    }
...
void
sngets(struct ARNAUD_state_s *V, ARNAUD_INT* kev, ARNAUD_INT* np,
...
static void
ssortc(const enum ARNAUD_which w, const ARNAUD_INT apply, const ARNAUD_INT n, float* xreal, float* ximag, float* y)
{
