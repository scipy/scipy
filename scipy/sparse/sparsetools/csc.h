...
/*
 * Compute Y = alpha*A*X + beta*Y for CSC matrix A and dense vectors X,Y
 ...
 *   T  Ax[n_col]     - nonzeros
 *   T  alpha         - scalar factor multipying A*X
 *   T  Xx[n_col]     - input vector
 *   T  beta          - scalar factor multipying Y
 *
 * Output Arguments:
...
/*
 * Compute Y = alpha*A*X + beta*Y for CSC matrix A and dense block vectors X,Y
 * and scalars alpha and beta
 *
 *
 * Input Arguments:
    ...
    if (alpha == 0) {
        ...
        for(I j = 0; j < n_col; j++){
            for(I ii = Ap[j]; ii < Ap[j+1]; ii++){
                const I i = Ai[ii];
                const T a = alpha * Ax[ii];
                axpy(n_vecs, a, Xx + (npy_intp)n_vecs * j, Yx + (npy_intp)n_vecs * i);
            }
        }
    }
}
