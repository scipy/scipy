...
extern void
HYBRD(void *, int *, double *, double *, double *, int *, int *, int *, double *, double *, int *, double *, int *, int *, int *, double *, int *, double *, int *, double *, double *, double *, double *, double *);
extern void
HYBRJ(void *, int *, double *, double *, double *, int *, double *, int *, double *, int *, double *, int *, int *, int *, int *, double *, int *, double *, double *, double *, double *, double *);
extern void
LMDIF(void *, int *, int *, double *, double *, double *, double *, double *, int *, double *, double *, int *, double *, int *, int *, int *, double *, int *, int *, double *, double *, double *, double *, double *);
extern void
LMDER(void *, int *, int *, double *, double *, double *, int *, double *, double *, double *, int *, double *, int *, double *, int *, int *, int *, int *, int *, double *, double *, double *, double *, double *);
extern void
LMSTR(void *, int *, int *, double *, double *, double *, int *, double *, double *, double *, int *, double *, int *, double *, int *, int *, int *, int *, int *, double *, double *, double *, double *, double *);

/* We only use ccallback with Python functions right now */
static ccallback_signature_t call_signatures[] = {{NULL}};
...
static int release_callback(ccallback_t *callback) { return ccallback_release(callback) != 0; }

static int init_jac_callback(
    ccallback_t *callback, jac_callback_info_t *jac_callback_info, PyObject *fcn, PyObject *Dfun, PyObject *extra_args,
    int col_deriv
) {
    int ret;
    int flags = CCALLBACK_OBTAIN;
...
int raw_multipack_calling_function(int *n, double *x, double *fvec, int *iflag) {
    ...
    */
    ...
    result_array = (PyArrayObject *) call_python_function(
        multipack_python_function, *n, x, multipack_extra_arguments, 1, minpack_error, *n
    );
...
int jac_multipack_calling_function(int *n, double *x, double *fvec, double *fjac, int *ldfjac, int *iflag) {
    ...
    */
    ...
    if (*iflag == 1) {
        ...
        result_array = (PyArrayObject *) call_python_function(
            multipack_python_jacobian, *n, x, multipack_extra_arguments, 2, minpack_error, (*n) * (*ldfjac)
        );
        if (result_array == NULL) {
            *iflag = -1;
            return -1;
...
int raw_multipack_lm_function(int *m, int *n, double *x, double *fvec, int *iflag) {
    ...
    */
    ...
    result_array = (PyArrayObject *) call_python_function(
        multipack_python_function, *n, x, multipack_extra_arguments, 1, minpack_error, *m
    );
    if (result_array == NULL) {
        *iflag = -1;
        return -1;
...
static PyObject *minpack_hybrd(PyObject *dummy, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(
            args, "OO|OidiiiddO", &fcn, &x0, &extra_args, &full_output, &xtol, &maxfev, &ml, &mu, &epsfcn, &factor,
            &o_diag
        ))
        return NULL;
    ...
    if (ap_r == NULL || ap_qtf == NULL || ap_fjac == NULL)
        goto fail;
    ...
    n_int = n;
    lr_int = lr; /* cast/store/pass into HYBRD */
    HYBRD(
        raw_multipack_calling_function, &n_int, x, fvec, &xtol, &maxfev, &ml, &mu, &epsfcn, diag, &mode, &factor,
        &nprint, &info, &nfev, fjac, &ldfjac, r, &lr_int, qtf, wa, wa + n, wa + 2 * n, wa + 3 * n
    );

    RESTORE_FUNC();

    if (info < 0)
        goto fail; /* Python Terminated */
    ...
    if (full_output) {
        return Py_BuildValue(
            "N{s:N,s:i,s:N,s:N,s:N}i", PyArray_Return(ap_x), "fvec", PyArray_Return(ap_fvec), "nfev", nfev, "fjac",
            PyArray_Return(ap_fjac), "r", PyArray_Return(ap_r), "qtf", PyArray_Return(ap_qtf), info
        );
    } else {
        Py_DECREF(ap_fvec);
        Py_DECREF(ap_fjac);
...
static PyObject *minpack_hybrj(PyObject *dummy, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(
            args, "OOO|OiididO", &fcn, &Dfun, &x0, &extra_args, &full_output, &col_deriv, &xtol, &maxfev, &factor,
            &o_diag
        ))
        return NULL;
    ...
    /* Call the underlying FORTRAN routines. */
    n_int = n;
    lr_int = lr; /* cast/store/pass into HYBRJ */
    HYBRJ(
        jac_multipack_calling_function, &n_int, x, fvec, fjac, &ldfjac, &xtol, &maxfev, diag, &mode, &factor, &nprint,
        &info, &nfev, &njev, r, &lr_int, qtf, wa, wa + n, wa + 2 * n, wa + 3 * n
    );
    ...
    if (full_output) {
        return Py_BuildValue(
            "N{s:N,s:i,s:i,s:N,s:N,s:N}i", PyArray_Return(ap_x), "fvec", PyArray_Return(ap_fvec), "nfev", nfev, "njev",
            njev, "fjac", PyArray_Return(ap_fjac), "r", PyArray_Return(ap_r), "qtf", PyArray_Return(ap_qtf), info
        );
    } else {
        Py_DECREF(ap_fvec);
        Py_DECREF(ap_fjac);
...
static PyObject *minpack_lmdif(PyObject *dummy, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(
            args, "OO|OidddiddO", &fcn, &x0, &extra_args, &full_output, &ftol, &xtol, &gtol, &maxfev, &epsfcn, &factor,
            &o_diag
        ))
        return NULL;
    ...
    if (ap_ipvt == NULL || ap_qtf == NULL || ap_fjac == NULL)
        goto fail;
    ...
    LMDIF(
        raw_multipack_lm_function, &m, &n_int, x, fvec, &ftol, &xtol, &gtol, &maxfev, &epsfcn, diag, &mode, &factor,
        &nprint, &info, &nfev, fjac, &ldfjac, ipvt, qtf, wa, wa + n, wa + 2 * n, wa + 3 * n
    );

    RESTORE_FUNC();

    if (info < 0)
        goto fail; /* Python error */
    ...
    if (full_output) {
        return Py_BuildValue(
            "N{s:N,s:i,s:N,s:N,s:N}i", PyArray_Return(ap_x), "fvec", PyArray_Return(ap_fvec), "nfev", nfev, "fjac",
            PyArray_Return(ap_fjac), "ipvt", PyArray_Return(ap_ipvt), "qtf", PyArray_Return(ap_qtf), info
        );
    } else {
        Py_DECREF(ap_fvec);
        Py_DECREF(ap_fjac);
...
static PyObject *minpack_lmder(PyObject *dummy, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(
            args, "OOO|OiidddidO", &fcn, &Dfun, &x0, &extra_args, &full_output, &col_deriv, &ftol, &xtol, &gtol,
    ...
    n_int = n;
    LMDER(
        jac_multipack_lm_function, &m, &n_int, x, fvec, fjac, &ldfjac, &ftol, &xtol, &gtol, &maxfev, diag, &mode,
        &factor, &nprint, &info, &nfev, &njev, ipvt, qtf, wa, wa + n, wa + 2 * n, wa + 3 * n
    );
    ...
    if (full_output) {
        return Py_BuildValue(
            "N{s:N,s:i,s:i,s:N,s:N,s:N}i", PyArray_Return(ap_x), "fvec", PyArray_Return(ap_fvec), "nfev", nfev, "njev",
            njev, "fjac", PyArray_Return(ap_fjac), "ipvt", PyArray_Return(ap_ipvt), "qtf", PyArray_Return(ap_qtf), info
        );
    } else {
        Py_DECREF(ap_fvec);
        Py_DECREF(ap_fjac);
...
static PyObject *minpack_chkder(PyObject *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(
            args, "iiOOOiO!OiO!", &m, &n, &o_x, &o_fvec, &o_fjac, &ldfjac, &PyArray_Type, (PyObject **) &ap_xp,
            &o_fvecp, &mode, &PyArray_Type, (PyObject **) &ap_err
        ))
        return NULL;
    ...
    if (mode == 1) {
        ...
        if (ap_fvec == NULL || ap_fjac == NULL || ap_fvecp == NULL)
            goto fail;
