...
#define STORE_VARS()                                                                                                   \
    ...
    int callback_inited = 0;                                                                                           \
    jac_callback_info_t jac_callback_info;
#define STORE_VARS_NO_INFO()                                                                                           \
    ccallback_t callback;                                                                                              \
    int callback_inited = 0;

#define INIT_FUNC(fun, arg, errobj)                                                                                    \
    do { /* Get extra arguments or set to zero length tuple */                                                         \
        if (arg == NULL) {                                                                                             \
            if ((arg = PyTuple_New(0)) == NULL)                                                                        \
                goto fail_free;                                                                                        \
        } else                                                                                                         \
...
#define SET_DIAG(ap_diag, o_diag, mode)                                                                                \
    { /* Set the diag vector from input */                                                                             \
...
static PyObject *call_python_function(
    PyObject *func, npy_intp n, double *x, PyObject *args, int dim, PyObject *error_obj, npy_intp out_size
) {
    /*
    ...
    */
    ...
    if (sequence == NULL)
        PYERR2(
            error_obj,
            "Internal failure to make an array of doubles out of first\n                 argument to function call."
        );
