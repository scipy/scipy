...
__all__ = ['newton', 'bisect', 'bisect_exact', 'ridder', 'brentq', 'brenth', 'toms748',
           'RootResults']
...
def bisect_exact(f, a, b, args=(),
    ...
    if rtol < _rtol:
        raise ValueError(f"rtol too small ({rtol:g} < {_rtol:g})")
    f = _wrap_nan_raise(f)
    r = _zeros._bisect_exact(f, a, b, xtol, rtol, maxiter, args, full_output, disp)
    return results_c(full_output, r, "bisect_exact")
