...
def _linprog_highs(lp, solver, time_limit=None, presolve=True,
    ...
    res = _highs_wrapper(
        c=c, indptr=A.indptr, indices=A.indices, data=A.data,
        lhs=lhs, rhs=rhs,
        lb=lb, ub=ub,
    ...
    sol = {
        ...
        'lower': OptimizeResult({
            'residual': None if res['x'] is None else res['x'] - lb,
            'marginals': marg_lower,
        }),
        'upper': OptimizeResult({
            'residual': None if res['x'] is None else ub - res['x'],
    ...
    if np.any(res['x']) and integrality is not None:
        sol.update({
