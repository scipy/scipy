...
def TRLIBQuadraticSubproblem(tol_rel_i=-2.0, tol_rel_b=-3.0):

    class SP(BaseQuadraticSubproblem):
        ...
        def __init__(self, x, fun, jac, hess, hessp):
            ...
            itmax = self.itmax
            ctrlib.trlib_krylov_memory_size(itmax,
                    &iwork_size, &fwork_size, &h_pointer)
            self.h_pointer = h_pointer
    
            self.fwork = np.empty([fwork_size])
            cdef double [:] fwork_view = self.fwork
            ctrlib.trlib_krylov_prepare_memory(itmax,
                    &fwork_view[0] if self.fwork.shape[0] > 0 else NULL)
        ...
        def solve(self, double trust_radius):
            ...
            while True:
                ret = ctrlib.trlib_krylov_min(init, trust_radius, equality,
                    ...
                    &iwork_view[0] if self.iwork.shape[0] > 0 else NULL,
                    &fwork_view[0] if self.fwork.shape[0] > 0 else NULL,
                    refine, verbose, unicode, prefix,
                    <libc.stdio.FILE*> libc.stdio.stdout,
                    &timing_view[0] if self.timing.shape[0] > 0 else NULL,
