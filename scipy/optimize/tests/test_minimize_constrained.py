...
def test_powell_domain_assignments():
    ...
    class CallbackMinimize:
        ...
        def __call__(self, intermediate_result):
            ...
            self.fl_vals.append(intermediate_result.funl)
            self.x_vals.append(intermediate_result.x[0])
            self.xl_vals.append(intermediate_result.xl[0])
    ...
    # For a bijective function, check for consecutive points in the linesearch:
    # if both points have different x-values, they must have different y-values,
    # if both points have same x-values, they must have the same y-values.
    # In case only x- or(!) y-values differ, elements were assigned incorrectly.
    domain_codomain_comparison = (
        (~np.isclose(call_fun.x_vals[0], call_fun.xl_vals[1], rtol=1e-20) and
         ~np.isclose(call_fun.f_vals[0], call_fun.fl_vals[1], rtol=1e-20)) or
        (np.isclose(call_fun.x_vals[0], call_fun.xl_vals[1], rtol=1e-20) and
         np.isclose(call_fun.f_vals[0], call_fun.fl_vals[1], rtol=1e-20))
    )
    assert domain_codomain_comparison, \
        ('Element of function codomain was assigned to the wrong element of '
         'function domain.')
