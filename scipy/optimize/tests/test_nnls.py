...
class TestNNLS:
    ...
    def test_nnls_max_iterations(self):
        ...
        n = np.array([3, 2, 0, 1, 1, 1, 3, 8, 14, 16, 29, 23, 41,
        ...
        k = np.array([0., 0., 0., 0., 0., 0.,
                      ...
                      0.])
        # The following code sets up a system of equations such that $k_i - p_i * n_i$ is minimized for $p_i$ with
        # weights $n_i$ and monotonicity constraints on $p_i$. This translates to a system of equations of the form
        # $k_i - (d_1 + ... + d_i) * n_i$ and non-negativity constraints on the $d_i$. If $n_i$ is zero the system is
        # modified such that $d_i - d_{i+1}$ is then minimized.
        N = len(n)
        ...
        # Small perturbations can already make the infinity loop go away (just uncomment the next line)
