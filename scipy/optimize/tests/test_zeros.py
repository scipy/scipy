...
class TestBracketMethods(TestScalarRootFinders):
    ...
    @pytest.mark.parametrize(
        "method", [zeros.bisect, zeros.ridder, zeros.toms748, zeros.chandrupatla]
    )
...
def test_gh_5555():
    ...
    methods = [zeros.bisect, zeros.ridder, zeros.chandrupatla]
    xtol = rtol = TOL
...
@pytest.mark.parametrize(
    "solver_name", ["brentq", "brenth", "bisect", "ridder", "toms748", "chandrupatla"]
)
...
@pytest.mark.parametrize(
    "method", ["brentq", "brenth", "bisect", "ridder", "toms748", "chandrupatla"]
)
...
@pytest.mark.parametrize(
    "method",
    [zeros.bisect, zeros.ridder, zeros.brentq, zeros.brenth, zeros.chandrupatla],
)
