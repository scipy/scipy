...
class CanonicalConstraint:
    ...
    def __init__(self, n_ineq, constr_ineq, jac_ineq,
                 n_eq, constr_eq, jac_eq, hess, feasible_constr):
        self.n_ineq = n_ineq
        ...
        self.jac_eq = jac_eq
        self.hess = hess
        self.feasible_constr = feasible_constr
...
class NonlinearConstraint:
    ...
    feasible_constr : {list of boolean, boolean}, optional
        ...
        is False.
    """
    def __init__(self, fun, jac, hess, kind, feasible_constr=False):
        self.fun = fun
        self.jac = jac
        self.hess = hess
        self.kind = kind
        _, _, _, _, _, fun_len = parse_constraint(self.kind)
        if isinstance(feasible_constr, bool):
            self.feasible_constr = np.full(fun_len,
                                           feasible_constr,
                                           dtype=bool)
        else:
            self.feasible_constr = np.array(feasible_constr,
                                            dtype=bool)
    ...
    def to_canonical(self, sparse_jacobian=None):
        ...
        if n_ineq == 0:
            ...
            feasible_constr = self.feasible_constr[ineq]
...
class LinearConstraint:
    ...
    feasible_constr : {list of boolean, boolean}, optional
        ...
        is False.
    """
    def __init__(self, A, kind, feasible_constr=False):
        self.A = A
        self.kind = kind
        _, _, _, _, _, fun_len = parse_constraint(self.kind)
        if isinstance(feasible_constr, bool):
...
class BoxConstraint:
    ...
    feasible_constr : {list of boolean, boolean}, optional
        ...
        is False.
    """
    def __init__(self, kind, feasible_constr=False):
        self.kind = kind
        _, _, _, _, _, fun_len = parse_constraint(self.kind)
        if isinstance(feasible_constr, bool):
            self.feasible_constr = np.full(fun_len,
    ...
    def to_linear(self, sparse_jacobian=None):
        ...
        if sparse_jacobian:
            ...
            A = np.eye(fun_len)
        # Return Constraint
        return LinearConstraint(A, self.kind, self.feasible_constr)
...
def concatenate_canonical_constraints(constraints, sparse_jacobian=None):
    ...
    feasible_constr_list = [constr.feasible_constr for constr in constraints]
    feasible_constr = np.hstack(feasible_constr_list)
