...
def _minimize_cobyqa(fun, x0, args=(), bounds=None, constraints=(),
    ...
    Notes
    -----
    **Thread-safety:** The COBYQA implementation uses internal synchronization
    (a module-level lock) to serialize access to the underlying C++ library.
    Multiple threads can call this function, but only one thread will execute
    the optimization at a time. The objective function and constraint functions
    must be thread-safe if they access shared state. See :ref:`scipy_thread_safety`
    for more details.
