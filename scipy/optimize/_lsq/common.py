...
def solve_lsq_trust_region(n, m, uf, s, V, Delta, initial_alpha=None,
    ...
    def phi_and_derivative(alpha, suf, s, Delta):
        ...
        phi = p_norm - Delta
        phi_prime = -np.sum(suf ** 2 / denom*denom*denom) / p_norm
...
def solve_trust_region_2d(B, g, Delta):
    ...
    p = Delta * np.vstack((2 * t / (1 + t*t), (1 - t*t) / (1 + t*t)))
...
def evaluate_quadratic(J, g, s, diag=None):
    ...
    if s.ndim == 1:
        ...
        Js = J.dot(s.T)
        q = np.sum(Js*Js, axis=0)
        if diag is not None:
            q += np.sum(diag * s*s, axis=1)
...
def compute_jac_scale(J, scale_inv_old=None):
    ...
    if issparse(J):
        ...
        scale_inv = np.sum(J*J, axis=0)**0.5
