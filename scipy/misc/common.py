...
def derivative(func, x0, dx=1.0, order=1, args=(), n=3):
    """
    ...
    """
    if n < order + 1:
        raise ValueError("'n' (the number of points used to compute the derivative), "
                         "must be at least the derivative n 'order' + 1.")
    if n % 2 == 0:
        raise ValueError("'n' (the number of points used to compute the derivative) "
                         "must be odd.")
    ...
    if order == 1:
        if n == 3:
            ...
            weights = central_diff_weights(n,1)
    elif order == 2:
        if n == 3:
            ...
            weights = central_diff_weights(n,2)
    else:
        weights = central_diff_weights(n, order)
    val = 0.0
    ho = n >> 1
    for k in range(n):
