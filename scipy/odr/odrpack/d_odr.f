...
C  ESTIMATE CURVATURE BY SECOND DERIVATIVE OF MODEL
      ...
      ABSCRV = ABS(PVPCRV)+ABS(PVMCRV)+TWO*ABS(PV)
      CURVE = CURVE + ETA*ABSCRV / (STPCRV*STPCRV)
...
C                TRANS(WRK4)*WRK4 = E = (D*D + ALPHA*TT*TT)
...
C  TREAT THE IDF GREATER THAN OR EQUAL TO 3 CASE
         ...
         D1 = PPFN
         D3 = PPFN*PPFN*PPFN
         D5 = PPFN*PPFN*PPFN*PPFN*PPFN
         D7 = PPFN*PPFN*PPFN*PPFN*PPFN*PPFN*PPFN
         D9 = PPFN*PPFN*PPFN*PPFN*PPFN*PPFN*PPFN*PPFN*PPFN
         DF2 = DF*DF
         DF3 = DF*DF*DF
         DF4 = DF*DF*DF*DF
         TERM1 = D1
         TERM2 = (ONE/B21)*(D3+D1)/DF
         TERM3 = (ONE/B31)*(B32*D5+B33*D3+B34*D1)/DF2
         TERM4 = (ONE/B41)*(B42*D7+B43*D5+B44*D3+B45*D1)/DF3
         TERM5 = (ONE/B51)*(B52*D9+B53*D7+B54*D5+B55*D3+B56*D1)/DF4
...
C  AUGMENT THE RESULTS FOR THE IDF = 4 CASE
            ...
            DO 90 IPASS=1,MAXIT
               S = SIN(Z)
               C = COS(Z)
               Z = Z - ((ONE+HALF*C*C)*S-CON)/((ONE+HALF)*C*C*C)
   90       CONTINUE
            DPPT = SQRT(DF)*S/C
...
C  AUGMENT THE RESULTS FOR THE IDF = 6 CASE
            ...
            DO 130 IPASS=1,MAXIT
               S = SIN(Z)
               C = COS(Z)
               Z = Z - ((ONE+HALF*C*C + (THREE/EIGHT)*C*C*C*C)*S-CON)/
     +                 ((FIFTN/EIGHT)*C*C*C*C*C)
  130       CONTINUE
            DPPT = SQRT(DF)*S/C
         END IF
      END IF
