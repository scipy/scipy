...
static PyObject *FIRsepsym2d(PyObject *NPY_UNUSED(dummy), PyObject *args) {
    ...
    case NPY_FLOAT:
        ret = S_separable_2Dconvolve_mirror(
            (float *) PyArray_DATA(a_image), (float *) PyArray_DATA(out), M, N, (float *) PyArray_DATA(a_hrow),
            (float *) PyArray_DATA(a_hcol), PyArray_DIMS(a_hrow)[0], PyArray_DIMS(a_hcol)[0], instrides, outstrides
        );
        break;
    case NPY_DOUBLE:
        ret = D_separable_2Dconvolve_mirror(
            (double *) PyArray_DATA(a_image), (double *) PyArray_DATA(out), M, N, (double *) PyArray_DATA(a_hrow),
            (double *) PyArray_DATA(a_hcol), PyArray_DIMS(a_hrow)[0], PyArray_DIMS(a_hcol)[0], instrides, outstrides
        );
        break;
#ifdef __GNUC__
    case NPY_CFLOAT:
        ret = C_separable_2Dconvolve_mirror(
            (__complex__ float *) PyArray_DATA(a_image), (__complex__ float *) PyArray_DATA(out), M, N,
            (__complex__ float *) PyArray_DATA(a_hrow), (__complex__ float *) PyArray_DATA(a_hcol),
            PyArray_DIMS(a_hrow)[0], PyArray_DIMS(a_hcol)[0], instrides, outstrides
        );
        break;
    case NPY_CDOUBLE:
        ret = Z_separable_2Dconvolve_mirror(
            (__complex__ double *) PyArray_DATA(a_image), (__complex__ double *) PyArray_DATA(out), M, N,
            (__complex__ double *) PyArray_DATA(a_hrow), (__complex__ double *) PyArray_DATA(a_hcol),
            PyArray_DIMS(a_hrow)[0], PyArray_DIMS(a_hcol)[0], instrides, outstrides
        );
        break;
#endif
...
static PyObject *IIRsymorder1_ic(PyObject *NPY_UNUSED(dummy), PyObject *args) {
    ...
    case NPY_FLOAT: {
        ...
        if ((precision <= 0.0) || (precision > 1.0))
            precision = 1e-6;
        ret = S_SYM_IIR1_initial(
            rz1, (float *) PyArray_DATA(a_sig), (float *) PyArray_DATA(out), M, N, (float) precision
        );
    } break;
    case NPY_DOUBLE: {
        double rz1 = z1.real;
        ...
        if ((precision <= 0.0) || (precision > 1.0))
            precision = 1e-11;
        ret = D_SYM_IIR1_initial(rz1, (double *) PyArray_DATA(a_sig), (double *) PyArray_DATA(out), M, N, precision);
    } break;
#ifdef __GNUC__
    case NPY_CFLOAT: {
        ...
        if ((precision <= 0.0) || (precision > 1.0))
            precision = 1e-6;
        ret = C_SYM_IIR1_initial(
            zz1, (__complex__ float *) PyArray_DATA(a_sig), (__complex__ float *) PyArray_DATA(out), M, N,
            (float) precision
        );
    } break;
    case NPY_CDOUBLE: {
        ...
        if ((precision <= 0.0) || (precision > 1.0))
            precision = 1e-11;
        ret = Z_SYM_IIR1_initial(
            zz1, (__complex__ double *) PyArray_DATA(a_sig), (__complex__ double *) PyArray_DATA(out), M, N, precision
        );
    } break;
#endif
    default:
...
static char doc_IIRsymorder2_ic_fwd[] =
...
static PyObject *IIRsymorder2_ic_fwd(PyObject *NPY_UNUSED(dummy), PyObject *args) {
    ...
    case NPY_FLOAT: {
        if ((precision <= 0.0) || (precision > 1.0))
            precision = 1e-6;
        ret = S_SYM_IIR2_initial_fwd(
            r, omega, (float *) PyArray_DATA(a_sig), (float *) PyArray_DATA(out), M, N, (float) precision
        );
    } break;
    case NPY_DOUBLE: {
        if ((precision <= 0.0) || (precision > 1.0))
            precision = 1e-11;
        ret = D_SYM_IIR2_initial_fwd(
            r, omega, (double *) PyArray_DATA(a_sig), (double *) PyArray_DATA(out), M, N, precision
        );
    } break;
    default:
        PYERR("Incorrect type.");
...
static char doc_IIRsymorder2_ic_bwd[] =
...
static struct PyModuleDef moduledef = {
    PyModuleDef_HEAD_INIT, "_spline", NULL, -1, toolbox_module_methods, NULL, NULL, NULL, NULL
};
