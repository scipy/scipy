...
def hurst_dfa(x, s_min, s_max):
    ...
    V : ndarray
        Covariance matrix of the fitting error.
        For more details, see np.linalg.lstsq
    ...
    Generate brown noise with ``alpha = 2``.

    >>> brown_noise = signal.coloured_noise(alpha=2, size=10000)

    Plot the signal and the power spectral density

    >>> psd_brown, freq_brown = plt.psd(brown_noise, NFFT=2**13)
    >>> plt.show()

    Calculate the Hurst exponents. For pink noise theoretically
    ``H = (1+alpha)/2 = 1.5``.

    >>> hurst_brown, cov_err = signal.hurst_dfa(brown_noise, s_min=5, s_max=50)
    
    Raises
    -------
    ValueError
        If 0 < ``s_min`` <= ``s_max`` <= ``len(x)`` is not satisfied.
    ...
    Detrended Fluctuation Analysis (DFA) is one of a variety of methods by
    which this alpha exponent can be measured. This is done by calculating
    the Hurst exponent, :math:`H` .
    ...
    wide variety of seemingly purely random signals, such as in
    music performances, penmanship, and the financial markets.
    ...
    is then fit along each "window", and the curve fit values, :math:`y_s`,
    recorded.
    ...
    See Also
    ---------
    coloured_noise : Generate data with defined Hurst exponent
    numpy.linalg.lstsq : Description of the "full" parameter
    ...
    if s_max <= s_min:
        raise ValueError("s_max must be smaller than s_min")
    if s_min == 1:
    ...
    for s in range(s_min, s_max + 1):
        ...
        f_k_substep = np.zeros(len(delta_y_s) // s)
        s_axis = np.arange(1, s + 1)

        # Sum diff for i=1 to N and j=1 to i and ensure integer num of windows
    ...
    c, cov_error = np.polyfit(log_s_axis, log_f_s, 1, cov=True)
    hurst = c[0]

    return hurst, cov_error
