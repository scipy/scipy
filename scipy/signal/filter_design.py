...
def iircomb(w0, Q, N, ftype='notch', fs=2.0):
    """
    Design Nth-order IIR notching or peaking digital comb filter.
    ...
    ----------
    w0 : float
        Frequency to remove or pass from a signal. If `fs` is specified, this
        is in the same units as `fs`. By default, it is a normalized scalar
        that must satisfy  ``0 < w0 < 1``, with ``w0 = 1`` corresponding to
    ...
    Q : float
        ...
        frequency, ``Q = w0/bw``.
    N : int
        The order of the filter. Must be between 0 and 24.
    ftype : {'notch', 'peak'}
        The type of comb filter generated by the function. Default is 'notch'.
    fs : float, optional
    ...
    ValueError
        ...
        ``fs/2``, if `ftype` is not 'notch' or 'peak', if `N` is less than
        or equal to 0 or greater than 24.
    ...
    Examples
    --------
    Design and plot 10th order notching comb filter at 60 Hz for a
    signal sampled at 200 Hz, using a quality factor Q = 30
    ...
    >>> b, a = signal.iircomb(f0, Q, 10, ftype='notch', fs=fs)
    ...
    Design and plot 10th order peaking comb filter at 300 Hz for a
    signal sampled at 1000 Hz, using a quality factor Q = 30

    >>> fs = 1000.0  # Sample frequency (Hz)
    >>> f0 = 300.0  # Frequency to be retained (Hz)
    >>> Q = 30.0  # Quality factor
    >>> # Design peaking filter
    >>> b, a = signal.iircomb(f0, Q, 10, ftype='peak', fs=fs)
    ...
    # Convert w0, Q, and fs to float, N to int
    w0 = float(w0)
    Q = float(Q)
    N = operator.index(N)
    fs = float(fs)
    ...
    if not 0 < N <= 24:
        raise ValueError("N must be > 0 and <= 24.")
