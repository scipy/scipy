...
def _eqtflength(b, a):
    ...
    zeros and removing any common trailing zeros.

    This is equivalent to converting a transfer function from negative powers
    form to positive powers form.
    ...
    Examples
    --------
    Convert negative powers :math:`\frac{1 + 2z^{-1}}{3 + 4z^{-1} + 5z^{-2}}`
    to positive powers :math:`\frac{1z^2 + 2z + 0}{3z^2 + 4z + 5}`:
    ...
    # Get index of last non-zero elements
    if np.count_nonzero(b) == 0:
        newlen = 1 + np.flatnonzero(a)[-1]
    else:
        newlen = 1 + max(np.flatnonzero(a)[-1], np.flatnonzero(b)[-1])

    if newlen > len(a):
        a = append(a, zeros(newlen - len(a), dtype=a.dtype))
    elif newlen < len(a):
        a = a[:newlen]

    if newlen > len(b):
        b = append(b, zeros(newlen - len(b), dtype=b.dtype))
    elif newlen < len(b):
        b = b[:newlen]
...
def tf2zpk(b, a, negative_powers=False):
    ...
    (If `M` and `N` are equal, which is true for all filters generated by the
    bilinear transform, then both forms happen to be equivalent, but this is
...
def normalize(b, a):
    ...
    Any leading zeros in `a` or `b` are stripped.
