...
def freqz(b, a=1, worN=512, whole=False, plot=None, fs=2*pi, nyquist=False):
    """
    ...
    worN : {None, int, array_like}, optional
        ...
        N=512). This is a convenient alternative to::
            ...
            np.linspace(0, fs if whole else fs/2, N, endpoint=nyquist)
    ...
    nyquist : bool, optional
        If `nyquist` is True, `freqz` includes last frequency (Nyquist frequency) 
    ...
    if _is_int_type(worN):
        ...
        # if nyquist is true, w should include end point
        w = np.linspace(0, lastpoint, N, endpoint=nyquist)
