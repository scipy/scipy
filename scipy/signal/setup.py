...
def configuration(parent_package='', top_path=None):
    ...
    config.add_extension('_spectral', sources=['_spectral.c'],extra_link_args = ['-lpython2.7'])
    config.add_extension('_max_len_seq_inner', sources=['_max_len_seq_inner.c'],extra_link_args = ['-lpython2.7'])

    spline_src = ['splinemodule.c', 'S_bspline_util.c', 'D_bspline_util.c',
                  'C_bspline_util.c', 'Z_bspline_util.c', 'bspline_util.c']
    config.add_extension('spline', sources=spline_src,extra_info={'extra_link_args' : ['-lpython2.7']}, **numpy_nodepr_api)
