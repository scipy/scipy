...
#define MAKE_C_ONEMULTADD(fname, type)                                                                                 \
    static void fname##_onemultadd2(char *sum, char *term1, char *term2) {                                             \
        ((type *) sum)[0] += ((type *) term1)[0] * ((type *) term2)[0] - ((type *) term1)[1] * ((type *) term2)[1];    \
        ((type *) sum)[1] += ((type *) term1)[0] * ((type *) term2)[1] + ((type *) term1)[1] * ((type *) term2)[0];    \
        return;                                                                                                        \
    }
...
static OneMultAddFunction *OneMultAdd[] = {
    NULL,
    BYTE_onemultadd,
    UBYTE_onemultadd,
    SHORT_onemultadd,
...
//
static int64_t reflect_symm_index(int64_t j, int64_t m) {
...
//
static int64_t circular_wrap_index(int64_t j, int64_t m) {
...
) /* fill value */
{
    ...
    if (mult_and_add == NULL)
        return -5; /* Not available for this type */

    if (type_num < 0 || type_num > MAXTYPES)
    ...
    if (outsize == FULL) {
        Os[0] = Ns[0] + Nwin[0] - 1;
        Os[1] = Ns[1] + Nwin[1] - 1;
    } else if (outsize == SAME) {
        Os[0] = Ns[0];
        Os[1] = Ns[1];
    } else if (outsize == VALID) {
        Os[0] = Ns[0] - Nwin[0] + 1;
        Os[1] = Ns[1] - Nwin[1] + 1;
    } else
        return -1; /* Invalid output flag */
    ...
    for (int64_t m = 0; m < Os[0]; m++) {
        ...
        int64_t new_m;
        if (outsize == FULL)
            new_m = convolve ? m : (m - Nwin[0] + 1);
        else if (outsize == SAME)
            new_m = convolve ? (m + ((Nwin[0] - 1) >> 1)) : (m - ((Nwin[0] - 1) >> 1));
        else
            new_m = convolve ? (m + Nwin[0] - 1) : m; /* VALID */
        ...
        for (int64_t n = 0; n < Os[1]; n++) { /* loop over columns */
            ...
            int64_t new_n;
            if (outsize == FULL)
                new_n = convolve ? n : (n - Nwin[1] + 1);
            else if (outsize == SAME)
                new_n = convolve ? (n + ((Nwin[1] - 1) >> 1)) : (n - ((Nwin[1] - 1) >> 1));
            else
                new_n = convolve ? (n + Nwin[1] - 1) : n;
            ...
            for (int64_t j = 0; j < Nwin[0]; j++) {
                ...
                if ((ind0 < 0) || (ind0 >= Ns[0])) {
                    if (boundary == REFLECT)
                        ind0 = reflect_symm_index(ind0, Ns[0]);
                    else if (boundary == CIRCULAR)
                        ind0 = circular_wrap_index(ind0, Ns[0]);
                    else
                        bounds_pad_flag = true;
                }
                ...
                if (bounds_pad_flag) {
                    ...
                    for (int64_t k = 0; k < Nwin[1]; k++) {
                        ...
                        if ((ind1 < 0) || (ind1 >= Ns[1])) {
                            if (boundary == REFLECT)
                                ind1 = reflect_symm_index(ind1, Ns[1]);
                            else if (boundary == CIRCULAR)
                                ind1 = circular_wrap_index(ind1, Ns[1]);
                            else
                                bounds_pad_flag = true;
                        }
