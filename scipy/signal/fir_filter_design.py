    ...
    if isinstance(pass_zero, str):
        if pass_zero in ("bandstop", "lowpass"):
            if pass_zero == "lowpass":
                if cutoff.size != 1:
                    raise ValueError(
                        'cutoff must have one element if pass_zero=="lowpass", got %s'
                        % (cutoff.shape,)
                    )
            elif cutoff.size <= 1:
                raise ValueError(
            ...
            if pass_zero == "highpass":
                if cutoff.size != 1:
                    raise ValueError(
                        'cutoff must have one element if pass_zero=="highpass", got %s'
                        % (cutoff.shape,)
                    )
            elif cutoff.size <= 1:
                raise ValueError(
...
def firls(numtaps, bands, desired, weight=None, nyq=None, fs=None):
    ...
    if len(weight) != len(desired):
        raise ValueError(
            "weight must be the same size as the number of band pairs (%s)."
            % (len(bands),)
        )
    if (weight < 0).any():
...
def minimum_phase(h, method="homomorphic", n_fft=None):
    ...
    if method == "hilbert":
        ...
        dp = max(H) - 1
        ds = 0 - min(H)
        S = 4.0 / (np.sqrt(1 + dp + ds) + np.sqrt(1 - dp + ds)) ** 2
        H += ds
        H *= S
        H = np.sqrt(H, out=H)
