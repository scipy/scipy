...
#: Allowed values for property `ShortTimeFFT.fft_typ`:
FFT_TYP_TYPE = Literal['twosided', 'centered', 'onesided', 'onesided2X']
...
class ShortTimeFFT:
    ...
    When assembling sequential signal parts together, the overlap needs to be
    considered:
    ...
    _phase_shift: Optional[int]  # amount to shift phase of FFT in samples
    ...
    def __init__(self, win: NDArray, hop: int, T: float,
        ...
        if scale_to is not None:  # needs to be set before fft_typ
    ...
    def fft_typ(self) -> FFT_TYP_TYPE:
        """Type of utilized FFT ('twosided', 'centered', 'onesided' or
        'onesided2X').
        ...
        'onesided2X':
            ...
            set to 'psd'. If `scaling` is ``None``, setting `fft_typ` to
    ...
    @fft_typ.setter
    def fft_typ(self, t_: FFT_TYP_TYPE):
        """Set type of FFT.
    ...
    @property
    def scaling(self) -> Optional[Literal['magnitude', 'psd']]:
    ...
    @property
    def phase_shift(self) -> Optional[int]:
    ...
    @phase_shift.setter
    def phase_shift(self, v: Optional[int]):
    ...
    def stft(self, x: NDArray, p0: Optional[int] = None, p1: Optional[int] = None,
        ...
        axis
            The axis over which to compute the inverse STFT.
    ...
    def spectrogram(self, x: NDArray, y: Optional[NDArray] = None,
        ...
        This a convenience function for calling `stft` / `stft_detrend`, hence
        all parameters are discussed there. If `y` is not ``None`` it needs to
        ...
        # Cross-spectrogram:
        Sy = self.stft_detrend(y, detr, p0, p1, k_offset, padding, axis)
        return Sx * Sy.conj()
    ...
    def dual_win(self) -> NDArray:
        ...
        the one with the minimal energy (i.e., L_2 norm).
    ...
    def istft(self, S: NDArray[np.complex_], k0: int = 0, k1: Optional[int] = None,
        ...
        ----------
        S
            A complex valued arrays where `f_axis` denote the frequency
