...
class TemplateError(Exception):
    ...
    def __str__(self):
        msg = ' '.join(self.args)
        if self.position:
            msg = f'{msg} at line {self.position[0]} column {self.position[0]}'
        if self.name:
            msg += f' in {self.name}'
        return msg
...
class Template:
    ...
    def __repr__(self):
        ...
        f' {hex(id(self))[2:]} name={self.name}>'
        ...
        if args:
            ...
            if not hasattr(args[0], 'items'):
                raise TypeError(
                    ...
                    ' a dictionary-like object (with a .items() method); you'
                    f' gave {args[0]}')
            kw = args[0]
        ns = kw
            ...
            if self._unicode and isinstance(value, bytes):
                if not self.default_encoding:
                    raise UnicodeDecodeError(
                        f'Cannot decode bytes value {value} into unicode '
                ...
                try:
                    ...
                    raise UnicodeDecodeError(
                        ...
                        e.start,
                        e.end,
                        e.reason + f' in string {value}')
            elif not self._unicode and isinstance(value, str):
                if not self.default_encoding:
                    raise UnicodeEncodeError(
                        f'Cannot encode unicode value {value} into bytes '
                        '(no default_encoding provided)')
                value = value.encode(self.default_encoding)
            return value
...
class bunch(dict):
    ...
    def __repr__(self):
        return f'<{self.__class__.__name__} '
        f'{" ".join(["{k}={v}" for k, v in sorted(self.items())])}>'
...
class TemplateDef:
    ...
    def __repr__(self):
        return f'<tempita function {self._func_name}({self._func_signature})'
        f'at {self._template.name}:{self._pos}>'
...
class TemplateObjectGetter:
    ...
    def __repr__(self):
        return f'<{self.__class__.__name__} around {self.__template_obj}>'
...
def lex(s, name=None, trim_whitespace=True, line_offset=0, delimiters=None):
    ...
    token_re = re.compile(r'f"{re.escape(delimiters[0]}|{re.escape(delimiters[1]}"')
...
def parse_one_cond(tokens, name, context):
    ...
    if first.startswith('if '):
        ...
        assert 0, f'Unexpected token {first} at {pos}'
    while 1:
        if not tokens:
            raise TemplateError(
                'No {{endif}}',
...
def parse_for(tokens, name, context):
    ...
    if not match:
        raise TemplateError(
            f'Bad for (no "in") in {first}',
            position=pos, name=name)
    vars = first[:match.start()]
    if '(' in vars:
        raise TemplateError(
            f'You cannot have () in the variable section of a for loop {vars}',
            position=pos, name=name)
    vars = tuple([
...
def parse_default(tokens, name, context):
    ...
    if len(parts) == 1:
        raise TemplateError(
            f'Expression must be {{default var=value}}; no = found in {first}',
            position=pos, name=name)
    var = parts[0].strip()
    if ',' in var:
        raise TemplateError(
    ...
    if not var_re.search(var):
        raise TemplateError(
            f'Not a valid variable name for {{default}}: {var}',
            position=pos, name=name)
    expr = parts[1].strip()
