...
static inline int PyArray_ImportNumPyAPI() {
...
#define NPY_RAVEL_AXIS (PyArray_RUNTIME_VERSION >= NPY_2_0_API_VERSION ? -1 : 32)
#define NPY_MAXARGS (PyArray_RUNTIME_VERSION >= NPY_2_0_API_VERSION ? 64 : 32)
#endif
...
static inline void PyDataType_SET_ELSIZE(PyArray_Descr *dtype, npy_intp size) { dtype->elsize = size; }

static inline npy_uint64 PyDataType_FLAGS(const PyArray_Descr *dtype) {
#if NPY_FEATURE_VERSION >= NPY_2_0_API_VERSION
    return dtype->flags;
#else
...
#define DESCR_ACCESSOR(FIELD, field, type, legacy_only)                                                                \
    static inline type PyDataType_##FIELD(const PyArray_Descr *dtype) {                                                \
...
static inline void PyDataType_SET_ELSIZE(PyArray_Descr *dtype, npy_intp size) {
...
#if NPY_FEATURE_VERSION >= NPY_2_0_API_VERSION
static inline PyArray_ArrFuncs *PyDataType_GetArrFuncs(PyArray_Descr *descr) { return _PyDataType_GetArrFuncs(descr); }
#elif NPY_ABI_VERSION < 0x02000000
static inline PyArray_ArrFuncs *PyDataType_GetArrFuncs(PyArray_Descr *descr) { return descr->f; }
#else
