...
static ckdtree_intp_t build(
    ckdtree *self, ckdtree_intp_t start_idx, intptr_t end_idx, double *maxes, double *mins, const int _median,
    const int _compact
) {
    ...
    if (end_idx - start_idx <= self->leafsize) {
        ...
        auto partition_pivot = [=](ckdtree_intp_t *first, ckdtree_intp_t *last, double pivot) {
            const auto partition_ptr =
                std::partition(first, last, [&](ckdtree_intp_t a) { return !(data[a * m + d] >= pivot); });
...
static double add_weights(ckdtree *self, double *node_weights, ckdtree_intp_t node_index, double *weights) {
