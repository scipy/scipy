...
static void traverse_no_checking(
    const ckdtree *self, std::vector<ordered_pair> *results, const ckdtreenode *node1, const ckdtreenode *node2
) {
...
static void traverse_checking(
    ...
    if (tracker->min_distance > tracker->upper_bound * tracker->epsfac)
        ...
        if (node2->split_dim == -1) { /* 1 & 2 are leaves */
            ...
            for (i = start1; i < end1; ++i) {
                ...
                for (j = min_j; j < end2; ++j) {
                    ...
                    d = MinMaxDist::point_point_p(self, data + indices[i] * m, data + indices[j] * m, p, m, tub);
        ...
        if (node2->split_dim == -1) { /* 1 is an inner node, 2 is a leaf node */
            ...
            tracker->push_greater_of(2, node2);
            traverse_checking(self, results, node1->less, node2->greater, tracker);
            tracker->pop();
            tracker->pop();
            ...
            if (node1 != node2) {
                 ...
                 */
                tracker->push_less_of(2, node2);
                traverse_checking(self, results, node1->greater, node2->less, tracker);
                tracker->pop();
            }
            tracker->push_greater_of(2, node2);
            traverse_checking(self, results, node1->greater, node2->greater, tracker);
            tracker->pop();
            tracker->pop();
        }
    }
...
#define HANDLE(cond, kls)                                                                                              \
    if (cond) {                                                                                                        \
        ...
        traverse_checking(self, results, self->ctree, self->ctree, &tracker);                                          \
    } else
