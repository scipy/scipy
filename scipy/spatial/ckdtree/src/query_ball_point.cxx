...
static void traverse_no_checking(
    const ckdtree *self, const int return_length, std::vector<ckdtree_intp_t> &results, const ckdtreenode *node
) {
...
#define HANDLE(cond, kls)                                                                                              \
    ...
    for (ckdtree_intp_t i = 0; i < n_queries; ++i) {
        ...
        if (CKDTREE_LIKELY(self->raw_boxsize_data == NULL)) {
            ...
            int j;
            for (j = 0; j < m; ++j) {
                point.maxes()[j] = point.mins()[j] =
                    BoxDist1D::wrap_position(point.mins()[j], self->raw_boxsize_data[j]);
            }
