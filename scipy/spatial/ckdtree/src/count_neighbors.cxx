...
template <typename MinMaxDist, typename WeightType, typename ResultType>
static void traverse(
    RectRectDistanceTracker<MinMaxDist> *tracker, const CNBParams *params, double *start, double *end,
    const ckdtreenode *node1, const ckdtreenode *node2
) {
    static void (*const next)(
        RectRectDistanceTracker<MinMaxDist> *tracker, const CNBParams *params, double *start, double *end,
        const ckdtreenode *node1, const ckdtreenode *node2
    ) = traverse<MinMaxDist, WeightType, ResultType>;
    ...
    if (params->cumulative) {
        ...
        if (new_end != end) {
            ResultType nn =
                WeightType::get_weight(&params->self, node1) * WeightType::get_weight(&params->other, node2);
            ...
            for (i = start1; i < end1; ++i) {
                ...
                for (j = start2; j < end2; ++j) {
                    ...
                    double d = MinMaxDist::point_point_p(
                        params->self.tree, sdata + sindices[i] * m, odata + oindices[j] * m, p, m, tmd
                    );
...
template <typename WeightType, typename ResultType>
void count_neighbors(struct CNBParams *params, ckdtree_intp_t n_queries, const double p) {
...
#define HANDLE(cond, kls)                                                                                              \
    if (cond) {                                                                                                        \
        ...
        traverse<kls, WeightType, ResultType>(                                                                         \
            &tracker, params, params->r, params->r + n_queries, self->ctree, other->ctree                              \
        );                                                                                                             \
    } else
...
struct Unweighted {
    ...
    static inline ckdtree_intp_t get_weight(const WeightedTree *wt, const ckdtreenode *node) { return node->children; }
    static inline ckdtree_intp_t get_weight(const WeightedTree *wt, const ckdtree_intp_t i) { return 1; }
};

int count_neighbors_unweighted(
    const ckdtree *self, const ckdtree *other, ckdtree_intp_t n_queries, double *real_r, intptr_t *results,
...
struct Weighted {
    ...
    static inline double get_weight(const WeightedTree *wt, const ckdtreenode *node) {
        return (wt->weights != NULL) ? wt->node_weights[node - wt->tree->ctree] : node->children;
    }
    static inline double get_weight(const WeightedTree *wt, const ckdtree_intp_t i) {
        return (wt->weights != NULL) ? wt->weights[i] : 1;
    }
};

int count_neighbors_weighted(
    const ckdtree *self, const ckdtree *other, double *self_weights, double *other_weights, double *self_node_weights,
    double *other_node_weights, ckdtree_intp_t n_queries, double *real_r, double *results, const double p,
    int cumulative
) {
