...
static void traverse(
    const ckdtree *self, const ckdtree *other, std::vector<coo_entry> *results, const ckdtreenode *node1,
    const ckdtreenode *node2, RectRectDistanceTracker<MinMaxDist> *tracker
) {
        ...
        if (node2->split_dim == -1) { /* 1 & 2 are leaves */
            ...
            for (ckdtree_intp_t i = start1; i < end1; ++i) {
                ...
                for (ckdtree_intp_t j = start2; j < end2; ++j) {
                    ...
                    double d =
                        MinMaxDist::point_point_p(self, sdata + sindices[i] * m, odata + oindices[j] * m, p, m, tub);
