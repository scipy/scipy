...
template <
    int ilp_factor = 4, typename T, typename TransformFunc, typename ProjectFunc = Identity, typename ReduceFunc = Plus>
void transform_reduce_2d_(
    StridedView2D<T> out, StridedView2D<const T> x, StridedView2D<const T> y, const TransformFunc &map,
    const ProjectFunc &project = Identity{}, const ReduceFunc &reduce = Plus{}
) {
    // Result type of calling map
    using AccumulateType = typename std::decay<decltype(map(std::declval<T>(), std::declval<T>()))>::type;
    ...
    if (xs == 1 && ys == 1) {
        for (; i + (ilp_factor - 1) < x.shape[0]; i += ilp_factor) {
            ...
            ForceUnroll<ilp_factor>{}([&](int k) { out(i + k, 0) = project(dist[k]); });
        }
    } else {
        for (; i + (ilp_factor - 1) < x.shape[0]; i += ilp_factor) {
            const T *x_rows[ilp_factor];
...
template <
    int ilp_factor = 2, typename T, typename TransformFunc, typename ProjectFunc = Identity, typename ReduceFunc = Plus>
void transform_reduce_2d_(
    StridedView2D<T> out, StridedView2D<const T> x, StridedView2D<const T> y, StridedView2D<const T> w,
    const TransformFunc &map, const ProjectFunc &project = Identity{}, const ReduceFunc &reduce = Plus{}
) {
    intptr_t i = 0;
    intptr_t xs = x.strides[1], ys = y.strides[1], ws = w.strides[1];
    // Result type of calling map
    using AccumulateType =
        typename std::decay<decltype(map(std::declval<T>(), std::declval<T>(), std::declval<T>()))>::type;
...
struct MinkowskiDistance {
    ...
    template <typename T>
    void operator()(StridedView2D<T> out, StridedView2D<const T> x, StridedView2D<const T> y, StridedView2D<const T> w)
        const {
        const T p = static_cast<T>(p_);
        const T invp = static_cast<T>(1.0 / p_);
        transform_reduce_2d_(
            out, x, y, w,
            [p](T x, T y, T w) INLINE_LAMBDA {
                auto diff = std::abs(x - y);
...
struct ChebyshevDistance {
    ...
    void operator()(StridedView2D<T> out, StridedView2D<const T> x, StridedView2D<const T> y) const {
        transform_reduce_2d_(
            out, x, y, [](T x, T y) INLINE_LAMBDA { return std::abs(x - y); }, Identity{},
...
struct CityBlockDistance {
    template <typename T>
    void operator()(StridedView2D<T> out, StridedView2D<const T> x, StridedView2D<const T> y) const {
        transform_reduce_2d_<2>(out, x, y, [](T x, T y) INLINE_LAMBDA { return std::abs(x - y); });
    }

    template <typename T>
    void operator()(StridedView2D<T> out, StridedView2D<const T> x, StridedView2D<const T> y, StridedView2D<const T> w)
        const {
        transform_reduce_2d_(out, x, y, w, [](T x, T y, T w) INLINE_LAMBDA { return w * std::abs(x - y); });
    }
};
...
struct BraycurtisDistance {
    ...
    void operator()(StridedView2D<T> out, StridedView2D<const T> x, StridedView2D<const T> y) const {
        ...
        transform_reduce_2d_<2>(
            ...
            [](const Acc<T> &acc) INLINE_LAMBDA { return acc.diff / acc.sum; },
...
struct HammingDistance {
    ...
    void operator()(StridedView2D<T> out, StridedView2D<const T> x, StridedView2D<const T> y) const {
        transform_reduce_2d_<4>(
            ...
            [](const Acc<T> &acc) INLINE_LAMBDA { return acc.nonmatches / acc.total; },
            [](const Acc<T> &a, const Acc<T> &b) INLINE_LAMBDA {
                Acc<T> acc;
...
struct DiceDistance {
    ...
    void operator()(StridedView2D<T> out, StridedView2D<const T> x, StridedView2D<const T> y) const {
        transform_reduce_2d_<2>(
            ...
            [](const Acc<T> &acc) INLINE_LAMBDA { return acc.nonmatches / (2 * acc.tt_matches + acc.nonmatches); },
            [](const Acc<T> &a, const Acc<T> &b) INLINE_LAMBDA {
                Acc<T> acc;
                acc.nonmatches = a.nonmatches + b.nonmatches;
...
struct JaccardDistance {
    ...
    void operator()(StridedView2D<T> out, StridedView2D<const T> x, StridedView2D<const T> y) const {
        transform_reduce_2d_<4>(
            ...
            [](const Acc<T> &acc)
                INLINE_LAMBDA { return (acc.denom != 0) * (acc.num / (1 * (acc.denom == 0) + acc.denom)); },
            [](const Acc<T> &a, const Acc<T> &b) INLINE_LAMBDA {
                Acc<T> acc;
                acc.num = a.num + b.num;
...
struct RogerstanimotoDistance {
    ...
    void operator()(StridedView2D<T> out, StridedView2D<const T> x, StridedView2D<const T> y) const {
        transform_reduce_2d_<4>(
            ...
            [](const Acc<T> &acc) INLINE_LAMBDA { return (2 * acc.ndiff) / (acc.n + acc.ndiff); },
            [](const Acc<T> &a, const Acc<T> &b) INLINE_LAMBDA {
                Acc<T> acc;
...
struct Kulczynski1Distance {
    ...
    void operator()(StridedView2D<T> out, StridedView2D<const T> x, StridedView2D<const T> y) const {
        transform_reduce_2d_<4>(
            ...
            [](const Acc<T> &acc) INLINE_LAMBDA { return acc.ntt / acc.ndiff; },
            [](const Acc<T> &a, const Acc<T> &b) INLINE_LAMBDA {
                Acc<T> acc;
                acc.ntt = a.ntt + b.ntt;
                acc.ndiff = a.ndiff + b.ndiff;
...
struct RussellRaoDistance {
    ...
    void operator()(StridedView2D<T> out, StridedView2D<const T> x, StridedView2D<const T> y) const {
        transform_reduce_2d_<4>(
            ...
            [](const Acc<T> &acc) INLINE_LAMBDA { return (acc.n - acc.ntt) / acc.n; },
            [](const Acc<T> &a, const Acc<T> &b) INLINE_LAMBDA {
                Acc<T> acc;
                acc.ntt = a.ntt + b.ntt;
                acc.n = a.n + b.n;
...
struct SokalmichenerDistance {
    ...
    void operator()(StridedView2D<T> out, StridedView2D<const T> x, StridedView2D<const T> y) const {
        transform_reduce_2d_<4>(
            ...
            [](const Acc<T> &acc) INLINE_LAMBDA { return (2 * acc.ndiff) / (acc.ndiff + acc.n); },
            [](const Acc<T> &a, const Acc<T> &b) INLINE_LAMBDA {
                Acc<T> acc;
                acc.ntt = a.ntt + b.ntt;
                acc.ndiff = a.ndiff + b.ndiff;
...
struct SokalsneathDistance {
    ...
    void operator()(StridedView2D<T> out, StridedView2D<const T> x, StridedView2D<const T> y) const {
        transform_reduce_2d_<4>(
            ...
            [](const Acc<T> &acc) INLINE_LAMBDA { return (2 * acc.ndiff) / (2 * acc.ndiff + acc.ntt); },
            [](const Acc<T> &a, const Acc<T> &b) INLINE_LAMBDA {
                Acc<T> acc;
                acc.ntt = a.ntt + b.ntt;
                acc.ndiff = a.ndiff + b.ndiff;
