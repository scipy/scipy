...
class RotationSpline(object):
    ...
    The angular acceleration is continuous, but not smooth. Also you can see
    that the angular acceleration is not a piecewise-linear function, because
    it is different from the second derivative of the rotation vector (which
    is the linear function analogous to the cubic spline).
