    ...
    if n <= 1:
        ...
        for i in range(1, len(y) - 1):
            if i % 2 == 0:
                ...
                y2 = y2 + y[i]
        result = dx*(y[0]+2*y1+4*y2+y[n-1])/3
        ...
        for i in range(1, len(y) - 1):
            if i % 3 == 0:
                ...
                y2 = y2 + y[i]
        result = 3*dx*(y[0]+2*y1+3*y2+y[n-1])/8
        ...
        for i in range(1, len(y) - 4):
            if(i % 2 == 0):
                ...
                y2 = y2 + y[i]
        result = (dx*(y[0]+2*y1+4*y2+y[n-4])/3 +
                  3*dx*(y[n-4]+3*y[n-3]+3*y[n-2]+y[n-1])/8)
    return result
