...
def qmc_quad(func, a, b, *, n_estimates=8, n_points=1024, qrng=None,
    ...
    func : callable
        ...
        shape ``(n, n_points)``, where ``n`` is the number of variables (i.e.
        the dimensionality of the function domain) and `n_points` is the number
        of quadrature points, and return the an array of shape ``(n_points,)``,
    ...
    qrng : `~scipy.stats.qmc.QMCEngine`, optional
        ...
        The QMCEngine must be initialized to a number of dimensions ``n``
        corresponding with the number of variables ``x1, ..., xn`` passed to
        `func`.
