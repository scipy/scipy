...
def test_tbound_respected_small_interval(method):
    ...
    #           undefined otherwise 
    def f(t,y):
        if t>SMALL:raise ValueError("Function was evaluated outside interval")
        return 2*y
...
def test_tbound_respected_larger_interval(method):
    ...
    def func(t, p):
        if t<-17 or t>2: raise ValueError("Function was evaluated outside interval")
        P = p[0]
        Q = p[1]
        r = np.exp(t)
        dPdr = r * Q
    ...
    result = solve_ivp(
        func, (-17, 2),
        y0=np.array([1, -11]), max_step=0.03, vectorized=False,
        t_eval=None,
        atol=1e-8, rtol=1e-5)
    assert_(result.success)
...
def test_tbound_respected_oscillator(method):
    ...
    def reactions_func (t, y):
        if (t>205): raise ValueError("Called outside interval")
    ...
    def run_sim2(t_end, n_timepoints=10, shortest_delay_line=10000000):
        ...
        t0=100.0
        t1=200.0
        return solve_ivp (reactions_func, (t0,t1),
                                init_state.copy(), dense_output=True, max_step=t1-t0)
    ...
    for (t0, t_bound) in ((5,9),(5,1)):
        for method_order in [RK23.error_estimator_order,
            ...
            step_no_max = select_initial_step(fun_rational,t0,y0,t_bound,
                                            np.inf,
                                            fun_rational(t0,y0),np.sign(t_bound-t0),
                                            method_order,rtol,atol)
