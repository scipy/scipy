...
def parse_constants(d,firstcol,secondcol,thirdcol):
    ...
    for line in d.split('\n'):
        ...
        val = line[firstcol:secondcol].replace(' ', '').replace('...', '')
        val = float(val)
        uncert = line[secondcol:thirdcol].replace(' ', '').replace('(exact)', '0')
        uncert = float(uncert)
        units = line[thirdcol:].rstrip()
        constants[name] = (val, units, uncert)
    return constants


_physical_constants_2002 = parse_constants(txt2002, 55, 77, 99)
_physical_constants_2006 = parse_constants(txt2006, 55, 77, 99)
_physical_constants_2010 = parse_constants(txt2010, 55, 77, 99)
_physical_constants_2014 = parse_constants(txt2014, 59, 84, 109)
