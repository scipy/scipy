...
def fht(a, dln, mu, offset=0.0, bias=0.0):
    ...
    defined using :math:`k dr` and :math:`r dk`, respectively 
    (see for example [2]_). The DHT makes clever use of the convolution
    theorem for a function of argument  :math:`r`
...
def _fhtq(a, u, inverse=False):
    ...
    if np.isinf(u[0]) and not inverse:
        warn('singular transform; consider changing the bias', stacklevel=1)
        # fix coefficient to obtain (potentially correct) transform anyway
        u = u.copy()
        u[0] = 0
    elif u[0] == 0 and inverse:
        warn('singular inverse transform; consider changing the bias', stacklevel=1)
