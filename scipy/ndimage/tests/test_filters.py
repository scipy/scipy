...
class TestNdimageFilters1:
    ...
    @xfail_xp_backends('cupy', reasons=['cupy/cupy#8403'])
...
@xfail_xp_backends('cupy', reasons=["cupy/cupy#8339"])
class TestFilter:
...
class TestNdimageFilters2:
    ...
    @xfail_xp_backends("cupy", reasons=["cupy/cupy#8430"])
    @skip_xp_backends(np_only=True,
                      reasons=["output=dtype is numpy-specific"],
                      exceptions=['cupy'],)
    @pytest.mark.parametrize('dtype_array', types)
    ...
    @xfail_xp_backends("cupy", reasons=["cupy/cupy#8406"])
    @skip_xp_backends("jax.numpy",
        reasons=["assignment destination is read-only"],
        cpu_only=True, exceptions=['cupy', 'jax.numpy'],
    )
    ...
    @xfail_xp_backends("cupy", reasons=["CuPy does not support extra_arguments"])
...
@xfail_xp_backends("cupy", reasons=["CuPy raises a TypeError."])
...
@xfail_xp_backends("cupy", reasons=["cupy/cupy#8402"])
...
@xfail_xp_backends("cupy",
                   reasons=["XXX thread exception; cannot repro outside of pytest"])
@skip_xp_backends("jax.numpy", reasons=["output array is read-only"],
                  cpu_only=True, exceptions=['cupy', 'jax.numpy'],)
class TestThreading:
...
@xfail_xp_backends('cupy', reasons=['cupy/cupy#8401'])
...
@xfail_xp_backends('cupy', reasons=['CuPy does not raise'])
