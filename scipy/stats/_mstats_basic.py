...
def kurtosistest(a, axis=0, alternative='two-sided'):
    ...
    n = a.count(axis=axis)
    if np.min(n) < 5:
        raise ValueError(f"kurtosistest requires at least 5 observations; {np.min(n)} observations were given.")
    if np.min(n) < 20:
        warnings.warn(f"kurtosistest only valid for n>=20 ... continuing anyway, n={np.min(n)}", stacklevel=2)
