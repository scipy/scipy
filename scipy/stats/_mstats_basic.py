...
def winsorize(a, limits=None, inclusive=(True, True), inplace=False,
    ...
    # ``a`` may be a list or a masked array; ``_contains_nan`` requires a
    # plain ndarray (a list has no ``shape`` and a masked array is rejected by
    # the array-API namespace), so check for NaNs via ``np.asarray(a)``.
    contains_nan = _contains_nan(np.asarray(a), nan_policy)
