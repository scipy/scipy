...
double g_alpha_ne_one(struct nolan_precanned *sp, double theta) {
    if (theta == -sp->xi) {
        if (sp->alpha < 1) {
            return 0;
        } else {
    ...
    return (
        sp->zeta_prefactor * pow(cos_theta / sin(sp->alpha_xi + sp->alpha * theta) * sp->zeta_offset, sp->alpha_exp) *
        cos(sp->alpha_xi + (sp->alpha - 1) * theta) / cos_theta
    );
}

double g_alpha_eq_one(struct nolan_precanned *sp, double theta) {
    if (theta == -sp->xi) {
        return 0;
    }
    ...
    return (
        (1 + theta * sp->two_beta_div_pi) * exp((sp->pi_div_two_beta + theta) * tan(theta) - sp->x0_div_term) /
        cos(theta)
    );
}

struct nolan_precanned *nolan_precan(double alpha, double beta, double x0) {
    ...
    if (alpha != 1.) {
        ...
        sp->zeta_prefactor = pow(pow(sp->zeta, 2.) + 1., -1. / (2. * (alpha - 1.)));
