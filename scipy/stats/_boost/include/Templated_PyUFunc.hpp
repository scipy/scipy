...
RealType callfunc(void* func, RealType **inputs) {
  switch (NINPUTS) {
  case 1: return (reinterpret_cast<RealType(*)(RealType)>(func))(*inputs[0]);
  case 2: return (reinterpret_cast<RealType(*)(RealType, RealType)>(func))(*inputs[0], *inputs[1]);
  case 3: return (reinterpret_cast<RealType(*)(RealType, RealType, RealType)>(func))(*inputs[0], *inputs[1], *inputs[2]);
  case 4: return (reinterpret_cast<RealType(*)(RealType, RealType, RealType, RealType)>(func))(*inputs[0], *inputs[1], *inputs[2], *inputs[3]);
  // add more cases if/when you need them
  default: throw std::runtime_error("callfunc() in C++11 compatibility mode can only handle 4 args!");
  }
}
#endif // USE_CPP14
