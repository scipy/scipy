...
from gen_func_defs_pxd import _gen_func_defs_pxd  # type: ignore
from _info import _x_funcs, _no_x_funcs, _klass_mapper  # type: ignore
...
def _ufunc_gen(
    ...
    methods = [
        _MethodDef(
            ...
            boost_func_name=x_func
            if boost_dist != "beta_distribution"
            else "pdf_beta"
            if x_func == "pdf"
            else x_func,
        )
    ...
    with open(filename, "w") as fp:
        ...
        if has_NPY_FLOAT16:
            warn(
                "Boost stats NPY_FLOAT16 ufunc generation not currently not supported!"
            )
...
if __name__ == "__main__":
    ...
    _gen_func_defs_pxd(
        f"{src_dir}/func_defs.pxd", x_funcs=_x_funcs, no_x_funcs=_no_x_funcs
    )
