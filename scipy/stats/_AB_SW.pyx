...
def gscale(int test, int other):
    ...
    cdef int LL = (test * other) // 2 + 1
    cdef int ind, n2b1, n2b2, loop_m = 3, part_no, ks
    cdef int len1 = 0, len2 = 0, len3 = 0  # Nonzero entry lengths

    cdef cnp.ndarray a1 = cnp.PyArray_ZEROS(1, [LL], cnp.NPY_FLOAT32, 0)
    cdef cnp.ndarray a2 = cnp.PyArray_ZEROS(1, [LL], cnp.NPY_FLOAT32, 0)
    cdef cnp.ndarray a3 = cnp.PyArray_ZEROS(1, [LL], cnp.NPY_FLOAT32, 0)
    cdef float[::1] a1v = a1  # memview of a1
    cdef float[::1] a2v = a2  # memview of a2
    cdef float[::1] a3v = a3  # memview of a3
...
def swilk(double[::1] x, double[::1] a, bint init=False, int n1=-1):
    ...
    """
    cdef int n = len(x), n2 = len(a)
    cdef int ncens, nn2, ind1, i1, ind2
    cdef bint upper = True

    cdef double[6] c1 = [0., 0.221157, -0.147981, -0.207119e1, 0.4434685e1,
                         -0.2706056e1]
    cdef double[6] c2 = [0., 0.42981e-1, -0.293762, -0.1752461e1, 0.5682633e1,
                         -0.3582633e1]
    cdef double[4] c3 = [0.5440, -0.39978, 0.25054e-1, -0.6714e-3]
    cdef double[4] c4 = [0.13822e1, -0.77857, 0.62767e-1, -0.20322e-2]
    cdef double[4] c5 = [-0.15861e1, -0.31082, -0.83751e-1, 0.38915e-2]
    cdef double[3] c6 = [-0.4803, -0.82676e-1, 0.30302e-2]
    cdef double[2] c7 = [0.164, 0.533]
    cdef double[2] c8 = [0.1736, 0.315]
    cdef double[2] c9 = [0.256, -0.635e-2]
    cdef double[2] g = [-0.2273e1, 0.459]
    ...
    cdef double Z90F, Z95F, Z99F, ZFM, ZSD, ZBAR
