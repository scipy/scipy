...
def cases_test_fit():
    # These fail default test; check separately
    skip_basic_fit = {'argus', 'foldnorm', 'truncweibull_min', 'beta'}
    ...
    for dist in dict(distdiscrete + distcont):
        ...
        elif dist in too_slow_basic_fit:
            reason = "far too slow (never waited for it to finish)"
            yield pytest.param(dist, marks=pytest.mark.skip(reason=reason))
        else:
            yield dist
...
class TestFit:
    ...
    def test_analytical_fit(self):
        ...
        # test with no parameters fixed
        ref = stats.fit(stats.laplace, data, [(-10, 10), (0, 10)])
        ref2 = stats.laplace.fit(data)
        assert ref.message == "Analytical MLEs calculated."
        assert_allclose(ref.params, ref2)

        # test with a parameter fixed
        ref = stats.fit(stats.laplace, data, [(0.5, 0.5), (0, 10)])
        ref2 = stats.laplace.fit(data, floc=0.5)
        assert ref.message == "Analytical MLEs calculated."
        assert_allclose(ref.params, ref2)

        # test with analytical fit out of bounds
        message = "Analytical MLEs were available, but they were not within"
        with pytest.warns(RuntimeWarning, match=message):
            res = stats.fit(stats.laplace, data, [(-1, 0), (10, 12)],
                            optimizer=self.opt)
