...
axis_nan_policy_cases = [
    ...
    (stats.alexandergovern, tuple(), {}, 2, 2, False,
     lambda res: (res.statistic, res.pvalue)),
    (stats.combine_pvalues, tuple(), dict(), 1, 2, False, None),
    (stats.combine_pvalues, tuple(), {'method': 'mudholkar_george'}, 1, 2, False, None),
]
...
empty_special_case_funcs = {stats.entropy, stats.combine_pvalues}
        ...
        for axis in range(-max_axis, max_axis):
            ...
            try:
                ...
                with np.testing.suppress_warnings() as sup:
                    ...
                    expected = [expected.copy() for _ in range(n_outputs)]

                    if hypotest in empty_special_case_funcs:
                        empty_val = unpacker(
                            hypotest(*([[]]*len(samples)), *args, **kwds)
                        )
                        for i in range(n_outputs):
                            mask = np.isnan(expected[i])
