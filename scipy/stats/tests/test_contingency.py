...
def test_cramersv():
    ...
    assert_array_almost_equal(np.asarray(c), correctc)
    assert_array_almost_equal(np.asarray(cx), correctcx)
...
def test_c():
    ...
    correctc = np.array([[0.31443609, 0.42354437],
                         [0.21905398, 0.12438315]])
    assert_array_almost_equal(np.asarray(c), correctc)
