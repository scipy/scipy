...
def _combine_bounds(name, user_bounds, shape_domain, integral):
    ...
    # Only if there is no analytical fit are we concerned about infinite bounds
    message = None
...
def fit(dist, data, bounds=None, *, guess=None,
    ...
    for i in range(n_params):
        ...
        combined, msg = _combine_bounds(name, user_bound,
                                        param_domain, integral)
        validated_bounds.append(combined)
        if msg is not None:
            delayed_error.append(msg)
    ...
    ignore_override = {'erlang', 'pearson3'}  # override not useful
    if (hasattr(dist, 'fit') and dist.name not in ignore_override
            and dist.fit != super(type(dist), dist).fit):
        i_fixed = (bounds[:, 0] == bounds[:, 1])
        f_names = np.array(['f' + name for name in param_list])
        override_fixed = dict(zip(f_names[i_fixed], bounds[i_fixed, 0]))
        try:
            params = dist.fit(data, **override_fixed)
            if ((params >= bounds[:, 0]).all()
                and (params <= bounds[:, 1]).all()):
                ...
                message = ("Analytical MLEs were available, but they "
                           "were not within the provided bounds.")
                warnings.warn(message, RuntimeWarning, stacklevel=2)
        except ValueError:
            pass

    if delayed_error:
        raise ValueError(delayed_error[0])
