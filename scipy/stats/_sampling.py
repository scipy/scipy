...
class FastGeneratorInversion:
    ...
    def rvs(self, size=None):
        ...
        `ppf_fast` computed by `NumericalInversePolynomial` when the class
    ...
    def evaluate_error(self, size=100000, random_state=None, x_error=False):
        ...
        Notes
        -----
        The numerical precision of the inverse CDF `ppf_fast` is controlled by
        ...
        sample. Note that if `ppf_fast` was exact, the u-error would be zero.

        The x-error measures the direct distance between the exact PPF
        (`ppf`) and `ppf_fast`. If ``x_error`` is set to ``True`, it is
