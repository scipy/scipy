...
class FastGeneratorInversion:
    ...
    the PPF using `ppf_fast` are very fast,
    ...
    Compare `ppf` and `ppf_fast`:

    >>> trunc_norm.ppf(0.5)
    3.1984741317619285
    >>> trunc_norm.ppf_fast(0.5)
    3.198474131740042

    In this particular, case, `scipy.stats.truncnorm` could also be used to
    generate truncated normal random variates. We can verify that
    the CDFs coincide up to a small numerical error:

    >>> trunc_norm.cdf(3.5) - stats.truncnorm(3, 4).cdf(3.5)
        ...
        if "check_pinv_params" in cfg:
            if not self._ignore_shape_range:
                if not cfg["check_pinv_params"](*args):
                    ...
                    msg += f"{args}. Use ignore_shape_range to proceed "
                    msg += "with the selected values."
    ...
    def rvs(self, size=None, random_state=None):
        ...
        Notes
        -----
        Note that the default ``rvs`` method of the rv_continuous class is
    ...
    def ppf_fast(self, q):
        ...
        results against the values computed by the method `ppf`.
    ...
    def evaluate_error(self, size=100000, random_state=None, x_error=False):
        ...
        ``np.nan``.
        The u-error is computed as follows:
        ``max |u - CDF(PPF_fast(u))|`` where the max is taken `size` random
        points in the interval [0,1]. `random_state` determines the random
        sample.
        If ``x_error`` is set to ``True`, the x-error is computed as the
        maximum of the minimum of the relative and absolute x-error:
        ...
        Note that the evaluation of the x-error will be very slow if the
        implementation of the PPF is slow.
