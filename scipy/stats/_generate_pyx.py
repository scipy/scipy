...
Generate Ufunc definition source files for scipy.stats. Produces
...
Cython API for stats functions
================================

Scalar, typed versions of many of the functions in ``scipy.stats``
...
The module is usable from Cython via::
    ...
    cimport scipy.stats.cython_stats

Error handling
--------------

Functions can indicate an error by returning ``nan``; however they
cannot emit warnings like their counterparts in ``scipy.stats``.
...
Some functions in ``scipy.stats`` which are not ufuncs have custom
Cython wrappers.
...
def generate_doc(name, specs):
    tab = " "*4
    doc = [f"- :py:func:`~scipy.stats.{name}`::\n"]
    for spec in specs:
...
class Ufunc(Func):
    """
    Ufunc signature, restricted format suitable for stats functions.
...
class FusedFunc(Func):
    """
    Generate code for a fused-type stats function that can be
    cimported in Cython.

    """
    def __init__(self, name, signatures):
        super().__init__(name, signatures)
        self.doc = "See the documentation for scipy.stats." + self.name
    ...
    def _get_common(self, signum, sig):
        ...
        if cpp:
            ...
            func_name = f"scipy.stats._ufuncs_cxx._export_{func_name}"
        ...
        for c_name, c_proto, cy_proto, header in cfuncs:
            if header.endswith('++'):
                ...
                ufunc.function_name_overrides[c_name] = (
                    "scipy.stats._ufuncs_cxx._export_" + var_name
                )
            else:
                # usual case
...
def main(outdir):
    ...
    if all_newer(src_files, dst_files):
        print("scipy/stats/_generate_pyx.py: all files up-to-date")
        return
