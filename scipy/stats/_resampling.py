...
def monte_carlo_test(sample, rvs, statistic, *, vectorized=None,
    ...
    The null hypothesis is that the provided `data` was drawn at random from
    the distributions for which the elements of `rvs` generates random
    variates. The value of `statistic` for the given `data` is compared against
    a Monte Carlo null distribution: the value of the statistic for each of
    `n_resamples` sets of samples generated by `rvs`. This gives the p-value,
    the probability of observing such an extreme value of the test statistic
    ...
    rvs : callable or tuple of callables
        A callable or sequence of callabls that generates random variates
