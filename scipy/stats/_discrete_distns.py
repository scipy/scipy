...
class betanbinom_gen(rv_discrete):
    ...
    def _stats(self, n, a, b, moments='mv'):
        ...
        def mean(n, a, b):
            return n * b / (a - 1.)
        mu = xpx.apply_where(a > 1, mean, (n, a, b), fill_value=np.inf)
        def var(n, a, b):
            return (n * b * (n + a - 1.) * (a + b - 1.)
                    / ((a - 2.) * (a - 1.)**2.))
        var = xpx.apply_where(a > 2, var, (n, a, b), fill_value=np.inf)
        g1, g2 = None, None
        def skew(n, a, b):
            return ((2 * n + a - 1.) * (2 * b + a - 1.)
                    ...
                    / (a - 2.)))
        if 's' in moments:
            g1 = xpx.apply_where(a > 3, skew, (n, a, b), fill_value=np.inf)
        def kurtosis(n, a, b):
            term = (a - 2.)
        ...
        if 'k' in moments:
            g2 = xpx.apply_where(a > 4, kurtosis, (n, a, b), fill_value=np.inf)
        return mu, var, g1, g2
...
class poisson_gen(rv_discrete):
    ...
    def _stats(self, mu):
        ...
        tmp = np.asarray(mu)
        mu_nonzero = tmp > 0
        g1 = xpx.apply_where(mu_nonzero, lambda x: sqrt(1.0/x), tmp, fill_value=np.inf)
        g2 = xpx.apply_where(mu_nonzero, lambda x: 1.0/x, tmp, fill_value=np.inf)
        return mu, var, g1, g2
...
def _gen_harmonic(n, a):
    ...
    return xpx.apply_where(a > 1, _gen_harmonic_gt1, _gen_harmonic_leq1, (n, a))
...
class dlaplace_gen(rv_discrete):
    ...
    def _cdf(self, x, a):
        ...
        return xpx.apply_where(k >= 0, f1, f2, (k, a))
