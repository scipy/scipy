...
tab_t *ptr(tab_t *m, int nr, int nc, int ir, int ic) { return m + nc * ir + ic; }
...
*/
void rcont1(
    tab_t *table, int nr, const tab_t *r, int nc, const tab_t *c, const tab_t ntot, tab_t *work, bitgen_t *rstate
) {
    // nothing to do
    if (ntot == 0)
        return;
...
*/
void rcont2(tab_t *table, int nr, const tab_t *r, int nc, const tab_t *c, const tab_t ntot, bitgen_t *rstate) {
    // nothing to do
    if (ntot == 0)
        return;
    ...
    for (int l = 0; l < nr - 1; ++l) {
        ...
        l131:
            ...
            double x =
                exp(logfactorial(ia) + logfactorial(ib) + logfactorial(ic) + logfactorial(id) - logfactorial(ie) -
                    logfactorial(nlm) - logfactorial(id - nlm) - logfactorial(ia - nlm) - logfactorial(ii + nlm));
            if (x >= z)
                goto l160;
            double sumprb = x;
            double y = x;
