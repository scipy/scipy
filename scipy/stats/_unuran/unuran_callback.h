...
void error_handler(
    const char *objid, const char *file, int line, const char *errortype, int unur_errno, const char *reason
) {
    if (unur_errno != UNUR_SUCCESS) {
        ...
        if (strcmp(errortype, "error") == 0) {
            ...
            PyErr_WarnFormat(
                PyExc_RuntimeWarning, 1, "[objid: %s] %d : %s => %s", objid_, unur_errno, reason_, errno_msg
            );
        }
    }
}

static ccallback_signature_t unuran_call_signatures[] = {{NULL}};
...
double pmf_thunk(int x, const struct unur_distr *distr) { UNURAN_THUNK(PyLong_FromLong, "pmf", 3); }

double pdf_thunk(double x, const struct unur_distr *distr) { UNURAN_THUNK(PyFloat_FromDouble, "pdf", 3); }

double dpdf_thunk(double x, const struct unur_distr *distr) { UNURAN_THUNK(PyFloat_FromDouble, "dpdf", 4); }

double logpdf_thunk(double x, const struct unur_distr *distr) { UNURAN_THUNK(PyFloat_FromDouble, "logpdf", 6); }

double cont_cdf_thunk(double x, const struct unur_distr *distr) { UNURAN_THUNK(PyFloat_FromDouble, "cdf", 3); }

double discr_cdf_thunk(int x, const struct unur_distr *distr) { UNURAN_THUNK(PyLong_FromLong, "cdf", 3); }
