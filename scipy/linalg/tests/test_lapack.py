...
@pytest.mark.parametrize('dtype', DTYPES)
def test_pptrs_pptrf_ppsv(dtype):
    seed(1234)
    ...
    ap = a[inds]
    ppsv, pptrf, pptrs = get_lapack_funcs(('ppsv', 'pptrf', 'pptrs'),
                                          dtype=dtype)

    ul, info = pptrf(n, ap)
    aul = cholesky(a)[inds]
    assert_allclose(ul, aul, rtol=np.finfo(dtype).eps*50)

    x, info = pptrs(n, ul, b)
    bx = solve(a, b)
    assert_allclose(x, bx, rtol=np.finfo(dtype).eps*50)

    xv, info = ppsv(n, ap, b)
    assert_allclose(xv, bx, rtol=np.finfo(dtype).eps*50)
