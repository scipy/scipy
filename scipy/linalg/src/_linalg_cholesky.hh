template<typename T>
int
_cholesky(PyArrayObject *ap_Am, PyArrayObject *ap_Cm, int lower, int overwrite_a, SliceStatusVec &vec_status) {
    char uplo = lower ? 'L' : 'U';
    ...
    for (npy_intp idx = 0; idx < outer_size; idx++) {
        ...
        zero_other_triangle(uplo, data_a, intn);

        if (!overwrite_a) {
            copy_slice_F_to_C(&ret_data[idx * n * n], data_a, n, n);
        } // NB. if `overwrite_a` is enabled the result is already in place
