...
class MatFile5Reader(MatFileReader):
    ...
    def read_var_header(self):
        ...
        if not mdtype == miMATRIX:
            raise TypeError('Expecting miMATRIX type here, got {}'.format(mdtype))
...
class VarWriter5:
    ...
    def _write_items(self, arr):
        ...
        if length > max_length:
            raise ValueError("Field names are restricted to {} characters".format(max_length - 1))
        self.write_element(np.array([length], dtype='i4'))
        self.write_element(np.array(fieldnames, dtype='S{}'.format(length)), mdtype=miINT8)
        A = np.atleast_2d(arr).flatten('F')
        for el in A:
            for f in fieldnames:
                self.write(el[f])
