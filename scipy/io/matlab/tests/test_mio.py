...
def _check_level(label, expected, actual):
    ...
    if SP.issparse(expected):  # allow different types of sparse matrices
        ...
        assert_array_almost_equal(
            actual.todense(), expected.todense(), err_msg=label, decimal=5
        )
        return
    # Check types are as expected
    assert_(
        types_compatible(expected, actual),
        "Expected type %s, got %s at %s" % (type(expected), type(actual), label),
    )
    ...
    assert_(
        ...
        msg="Expected shape %s, got %s at %s" % (expected.shape, actual.shape, label),
    )
    ...
    if ex_dtype.fields:  # probably recarray
        ...
        return
    if ex_dtype.type in (str, np.unicode_, np.bool_):  # string or bool
...
def test_load():
    for case in case_table4 + case_table5:
        ...
        files = glob(filt)
        assert_(len(files) > 0, "No files for test %s using filter %s" % (name, filt))
...
def test_gzip_simple():
    ...
    assert_array_almost_equal(
        actual["x"].todense(), expected["x"].todense(), err_msg=repr(actual)
    )
