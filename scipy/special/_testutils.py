...
def check_version(module, min_ver):
    ...
    return pytest.mark.skipif(
        LooseVersion(module.__version__) < LooseVersion(min_ver),
    ...
    def __init__(
        ...
        if result_columns is not None:
            ...
            if result_func is not None:
                raise ValueError(
                    "Only result_func or result_columns should be provided"
                )
    ...
    def check(self, data=None, dtype=None, dtypes=None):
        ...
        for output_num, (x, y) in enumerate(zip(got, wanted)):
            ...
            if np.any(bad_j):
                ...
                msg.append(
                    "Bad results (%d out of %d) for the following points (in output"
                    " %d):"
                    % (
                        np.sum(bad_j),
                        point_count,
