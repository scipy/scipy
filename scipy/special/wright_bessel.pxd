...
cdef inline double _wb_asymptotic(double a, double b, double x) nogil:
    cdef:
        double A[13]  # powers of a
        double B[15]  # powers of b
        double Ap1[8]  # powers of (1+a)
        double C[8]  # coefficients of asymptotic series a_k
        double Z, Zp, res
        int k
...
cdef inline double wright_bessel_scalar(double a, double b, double x) nogil:
    ...
    elif (a * x >= pow(15. * fmax(1., b), 1 + a)):
        # asymptotic expansion in Z = (a*x)^(1/(1+a)) >= 15 * max(1, b)
