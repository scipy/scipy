...
namespace special {
    ...
    constexpr double SMALLX = 7;
    constexpr double SMALLY = 7;
    constexpr double HLOG2PI = 0.918938533204672742; // log(2*pi)/2
    constexpr double LOGPI = 1.1447298858494001741434262; // log(pi)
    constexpr double TAYLOR_RADIUS = 0.2;
	...
	return (z - 0.5)*std::log(z) - z + HLOG2PI + rz*cevalpoly(coeffs, 7, rzz);
    }
		

    std::complex<double> loggamma_recurrence(std::complex<double> z) {
	...
	return loggamma_stirling(z) - std::log(shiftprod) - signflips*2*M_PI*1i;
    }

    std::complex<double> loggamma_taylor(std::complex<double> z) {
	...
	if (z.real() <= 0 and z == std::floor(z.real())) {
	    sf_error("loggamma", SF_ERROR_SINGULAR, NULL);
	    return std::complex<double>(numeric_limits<double>::quiet_NaN(),
					numeric_limits<double>::quiet_NaN());
	}
	if (z.real() > SMALLX || std::fabs(z.imag()) > SMALLY) {
	    return loggamma_stirling(z);
	}
	if (std::abs(z - 1.0) < TAYLOR_RADIUS) {
	...
	if (z.real() < 0.1) {
	    ...
	    return std::complex<double>(LOGPI, tmp) - std::log(csinpi(z))
		- loggamma(1.0 - z);
	}
	...
	if (z.real() <= 0 && z == std::floor(z.real())) {
	    // Poles
	    sf_error("gamma", SF_ERROR_SINGULAR, NULL);
	    return std::complex<double>(numeric_limits<double>::quiet_NaN(),
					numeric_limits<double>::quiet_NaN());
	}
    ...
    inline std::complex<double> crgamma(std::complex<double> z) {
