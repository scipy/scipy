...
namespace special {
	...
	std::complex<double> loggamma_stirling(std::complex<double> z) {
	    ...
	    return (z - 0.5)*std::log(z) - z + loggamma_HLOG2PI
		+ rz*cevalpoly(coeffs, 7, rzz);
	}
	...
	if (z.real() > detail::loggamma_SMALLX
	    || std::fabs(z.imag()) > detail::loggamma_SMALLY) {
	...
	if (z.real() < 0.1) {
	    ...
	    return std::complex<double>(detail::loggamma_LOGPI, tmp) - std::log(sinpi(z))
		- loggamma(1.0 - z);
	}
    ...
    inline std::complex<double> rgamma(std::complex<double> z) {
