...
class FusedFunc(Func):
    ...
    def _get_common(self, signum, sig):
        ...
        if cpp:
            ...
            func_name = "scipy.special._cython_special_cxx._export_{}".format(func_name)
            func_name = "(<{}(*)({}) nogil>{})"\
...
def generate_ufuncs(fn_prefix, cxx_fn_prefix, ufuncs, fused_modname):
    ...
    # Also define a unique version of the cxx module for the Cython
    # API so that it can be compiled with the macro CYTHON_SPECIAL
    # defined. This is necessary for correct error handling.
    with open("_{}_cxx.pyx".format(fused_modname), 'w') as f:
        f.write(CYTHON_SPECIAL_CXX)
        f.write("\n".join(cxx_defs))
        f.write("\n# distutils: language = c++\n")

    with open("_{}_cxx.pxd".format(fused_modname), 'w') as f:
        cxx_pxd_defs.remove("cdef int _set_errprint(int flag) nogil")
        f.write("\n".join(cxx_pxd_defs))
...
def main():
    ...
    generate_ufuncs("_ufuncs", "_ufuncs_cxx", ufuncs, "cython_special")
