...
def uniform_random_points():
    random_state = np.random.RandomState(1729)
...
class TestNdtriExp(object):
    ...
    def test_very_small_arg(self, test_input, uniform_random_points):
        ...
        assert_func_equal(log_ndtr_ndtri_exp, lambda y: y, points,
                        rtol=1e-14)

    @pytest.mark.parametrize('interval,expected_rtol',
                             [((-10, -2), 1e-14),
                              ((-2, -0.14542), 1e-12),
                              ((-0.14542, -1e-6), 1e-10),
                              ((-1e-6, 0), 1e-6)])
    def test_in_interval(self, interval, expected_rtol,
    ...
    def test_asymptotes(self):
        assert_equal(ndtri_exp([-np.inf, 0.0]),
                     [-np.inf,  np.inf])
