...
class TestNctdtr:
    ...
    def test_nctdtr_gh19896(self):
        ...
        resarr = [0.9999279987514815, 0.9999528361700505, 0.9999908823016942,
                  ...
                  0.00012416481147589105, 0.035388035775454095, 0.7954826975430583,
                  3.02106943e-316, 6.069970616996603e-309, 2.591995360483094e-97,
                  0.011927265886910935, 0.9999280776192786, 0.9999599410685442,
                  0.9999997432394788, 0.9999999999999984, 0.3525155979107491,
                  0.40763120140379194, 0.8476794017024651, 0.9999999297116268,
                  7.277620328149153e-05, 0.00013024802220900652, 0.013477432800072933,
                  0.999850151230648, 3.05066095e-316, 1.79065514676e-313,
                  2.0935940165900746e-249, 2.252076291604796e-09, 0.9999280875149109,
                  0.9999608250170452, 0.9999999304757682, 1.0, 0.35252817848596313,
                  0.40890253001794846, 0.8664672830006552, 1.0, 7.278609891281275e-05,
                  0.0001310318674827004, 0.010990879189991727, 0.9999999999999989,
                  3.05437385e-316, 9.1668336166e-314, 1.8085884236563926e-288,
                  2.7740532792035907e-50, 0.9999280885188965, 0.9999609144559273,
                  0.9999999410050979, 1.0, 0.3525294548792812, 0.4090315324657382,
                  0.8684247068517293, 1.0, 7.278710289828983e-05,
                  0.00013111131667906573, 0.010750678886113882, 1.0, 3.0547506e-316,
                  8.6191646313e-314, 1.1824454111413493e-291, 5.407535300713606e-105]
        actarr = []
        for df, p, t in itertools.product(dfarr, pnoncarr, tarr):
            actarr += [sp.nctdtr(df, p, t)]

        # for nctdtr(980, 38, 1.5), boost computes a tiny negative value
        # we set this to nan in the wrapper
        # ignore this value in the comparison for now by setting the 
        # corresponding entry in the reference array to nan
        actarr = np.asarray(actarr)
        nan_entry = np.isnan(actarr)
        resarr = np.asarray(resarr)
        resarr[nan_entry] = np.nan

        # ignore subnormal non-zero values, so set atol to 1e-303
        assert_allclose(actarr, resarr, rtol=1e-13, atol=1e-303)
