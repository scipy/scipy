...
class TestCephes(object):
    ...
    def test_smirnovp(self):
        assert_equal(_smirnovp(1, .1), -1)
        assert_equal(_smirnovp(2, 0.75), -2*(0.25)**(2-1))
        assert_equal(_smirnovp(3, 0.75), -3*(0.25)**(3-1))
        assert_(np.isnan(_smirnovp(1, np.nan)))

    def test_smirnovc(self):
        assert_equal(_smirnovc(1,.1),0.1)
        assert_(np.isnan(_smirnovc(1,np.nan)))
        x10 = np.linspace(0, 1, 11, endpoint=True)
        assert_almost_equal(_smirnovc(3, x10), 1-cephes.smirnov(3, x10))
        x4 = np.linspace(0, 1, 5, endpoint=True)
        assert_almost_equal(_smirnovc(4, x4), 1-cephes.smirnov(4, x4))
    ...
    def test_smirnovci(self):
        assert_almost_equal(_smirnovc(1,_smirnovci(1,0.4)),0.4)
        assert_almost_equal(_smirnovc(1,_smirnovci(1,0.6)),0.6)
        assert_(np.isnan(_smirnovci(1,np.nan)))
