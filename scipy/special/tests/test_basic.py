...
class TestCephes:
    ...
    def test_beta(self):
        ...
        assert_allclose(
            cephes.beta(0.0342, 171), 24.070498359873497, rtol=1e-13, atol=0
        )
    ...
    def test_betaln(self):
        ...
        assert_allclose(
            cephes.betaln(0.0342, 170), 3.1811881124242447, rtol=1e-14, atol=0
        )

    def test_betaincinv(self):
        assert_equal(cephes.betaincinv(1, 1, 1), 1.0)
        assert_allclose(
            cephes.betaincinv(0.0342, 171, 0.25),
    ...
    def test_mathieu_cem(self):
        ...
        def ce_smallq(m, q, z):
            z *= np.pi / 180
            if m == 0:
                return 2 ** (-0.5) * (1 - 0.5 * q * cos(2 * z))  # + O(q^2)
            elif m == 1:
...
class TestAiry:
    def test_airy(self):
        ...
        x = special.airy(0.99)
        assert_array_almost_equal(
            x, array([0.13689066, -0.16050153, 1.19815925, 0.92046818]), 8
        )
        x = special.airy(0.41)
        assert_array_almost_equal(
            x, array([0.25238916, -0.23480512, 0.80686202, 0.51053919]), 8
        )
        x = special.airy(-0.36)
        assert_array_almost_equal(
            x, array([0.44508477, -0.23186773, 0.44939534, 0.48105354]), 8
        )
...
class TestBeta:
    ...
    def test_betainc(self):
        btinc = special.betainc(1, 1, 0.2)
        assert_almost_equal(btinc, 0.2, 8)

    def test_betaincinv(self):
        y = special.betaincinv(2, 4, 0.5)
        comp = special.betainc(2, 4, y)
        assert_almost_equal(comp, 0.5, 5)
...
class TestCombinatorics:
    ...
    def test_comb_with_np_int64(self):
        ...
        np_k = np.int64(k)
        assert_equal(
            special.comb(np_n, np_k, exact=True), special.comb(n, k, exact=True)
        )
...
class TestFactorialFunctions:
    def test_factorial(self):
        ...
        assert_array_almost_equal(
            [6.0, 24.0, 120.0], special.factorial([3, 4, 5], exact=False)
        )
        assert_array_almost_equal(
            special.factorial([[5, 3], [4, 3]]), [[120, 6], [24, 6]]
        )
        ...
        assert_equal(
            special.factorial([7, 4, 15, 10], exact=True),
            [5040, 24, 1307674368000, 3628800],
        )

        assert_equal(special.factorial([[5, 3], [4, 3]], True), [[120, 6], [24, 6]])

        # object arrays
        assert_equal(
            special.factorial(np.arange(-3, 22), True),
            special.factorial(np.arange(-3, 22), False),
        )

        # int64 array
        assert_equal(
            special.factorial(np.arange(-3, 15), True),
            special.factorial(np.arange(-3, 15), False),
        )

        # int32 array
        assert_equal(
            special.factorial(np.arange(-3, 5), True),
        ...
        for exact in (True, False):
            ...
            assert_array_equal([1, 2, 0, 0], special.factorial([1, 2, -5, -4], exact))
        ...
        for n in range(0, 22):
            ...
            assert_array_equal(special.factorial(n, True), special.factorial(n, False))

            assert_array_equal(
                special.factorial([n], True), special.factorial([n], False)
            )
    ...
    def test_factorial2(self):
        assert_array_almost_equal(
            [105.0, 384.0, 945.0], special.factorial2([7, 8, 9], exact=False)
        )
    ...
    # single NaN.
    @pytest.mark.skipif(
        sys.version_info >= (3, 10), reason="Python 3.10+ math.factorial() requires int"
    )
...
class TestFresnel:
    def test_fresnel(self):
        frs = array(special.fresnel(0.5))
        assert_array_almost_equal(
            frs, array([0.064732432859999287, 0.49234422587144644]), 8
        )
...
class TestHyper:
    ...
    def test_hyp0f1(self):
        ...
        assert_raises(ValueError, special.hyp0f1, np.row_stack([x1] * 3), [0, 1])
...
class TestBessel:
    def test_itj0y0(self):
        ...
        assert_array_almost_equal(
            it0, array([0.19933433254006822, -0.34570883800412566]), 8
        )

    def test_it2j0y0(self):
        it2 = array(special.it2j0y0(0.2))
        assert_array_almost_equal(
            it2, array([0.0049937546274601858, -0.43423067011231614]), 8
        )
    ...
    def test_kvp_n2(self):
        v = 3.0
        z = 2.2
        xc = (z ** 2 + v ** 2 - v) / z ** 2 * special.kv(v, z) + special.kv(
            v + 1, z
        ) / z
    ...
    def test_y1p_zeros(self):
        ...
        assert_array_almost_equal(
            y1p, (array([0.5768 + 0.904j]), array([-0.7635 + 0.5892j])), 3
        )
    ...
    def test_iti0k0(self):
        ...
        assert_array_almost_equal(
            iti0, array([31.848667776169801, 1.5673873907283657]), 5
        )

    def test_it2i0k0(self):
        it2k = special.it2i0k0(0.1)
        assert_array_almost_equal(
            it2k, array([0.0012503906973464409, 3.3309450354686687]), 6
        )
...
class TestLegendreFunctions:
    ...
    def test_clpmn_close_to_real_2(self):
        ...
        assert_array_almost_equal(
            array([clp_plus, clp_minus]),
            array([special.lpmv(m, n, x), special.lpmv(m, n, x)]),
            7,
        )
        ...
        for type in [2, 3]:
            for z in zvals:
                for h in [1e-3, 1e-3j]:
                    ...
                    assert_allclose(
                        special.clpmn(m, n, z, type)[1], approx_derivative, rtol=1e-4
                    )
    ...
    def test_lpmn(self):
        ...
        assert_array_almost_equal(
            lp,
            (
                array([[1.00000, 0.50000, -0.12500]]),
                array([[0.00000, 1.00000, 1.50000]]),
            ),
            4,
        )
    ...
    def test_lpn(self):
        ...
        assert_array_almost_equal(
            lpnf,
            (array([1.00000, 0.50000, -0.12500]), array([0.00000, 1.00000, 1.50000])),
            4,
        )
    ...
    def test_lqn(self):
        lqf = special.lqn(2, 0.5)
        assert_array_almost_equal(
            lqf, (array([0.5493, -0.7253, -0.8187]), array([1.3333, 1.216, -0.8427])), 4
        )
...
class TestPolygamma:
    ...
    def test_polygamma(self):
        ...
        assert_almost_equal(special.polygamma(n, np.row_stack([x] * 2)), expected)
        assert_almost_equal(special.polygamma(np.row_stack([n] * 2), x), expected)
...
def test_sph_harm():
    ...
    assert_array_almost_equal(
        sh(-2, 2, 0.0, pi / 4), 0.25 * sqrt(15.0 / (2.0 * pi)) * (sin(pi / 4)) ** 2.0
    )
    assert_array_almost_equal(sh(-2, 2, 0.0, pi / 2), 0.25 * sqrt(15.0 / (2.0 * pi)))
    assert_array_almost_equal(
        sh(2, 2, pi, pi / 2),
        0.25 * sqrt(15 / (2.0 * pi)) * exp(0 + 2.0 * pi * 1j) * sin(pi / 2.0) ** 2.0,
    )
    assert_array_almost_equal(
