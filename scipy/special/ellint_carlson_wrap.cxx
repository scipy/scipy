...
{
    sf_error_t status;
    std::complex<double> xx{npymath_creal(x), npymath_cimag(x)};
    std::complex<double> yy{npymath_creal(y), npymath_cimag(y)};
    std::complex<double> res;

    status = static_cast<sf_error_t>(ellint_carlson::rc(xx, yy,
                                                        ellip_rerr, res));
    sf_error("elliprc (complex)", status, NULL);
    return npymath_cpack(res.real(), res.imag());
}
...
npy_cdouble cellint_RD(npy_cdouble x, npy_cdouble y, npy_cdouble z)
{
    ...
    std::complex<double> xx{npymath_creal(x), npymath_cimag(x)};
    std::complex<double> yy{npymath_creal(y), npymath_cimag(y)};
    std::complex<double> zz{npymath_creal(z), npymath_cimag(z)};
    std::complex<double> res;
...
npy_cdouble cellint_RJ(npy_cdouble x, npy_cdouble y, npy_cdouble z, npy_cdouble p)
{
    ...
    std::complex<double> yy{npymath_creal(y), npymath_cimag(y)};
    std::complex<double> zz{npymath_creal(z), npymath_cimag(z)};
    std::complex<double> pp{npymath_creal(p), npymath_cimag(p)};
    std::complex<double> res;
