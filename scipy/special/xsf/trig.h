...
inline double radian(double d, double m, double s) {
    return cephes::radian(d, m, s);
}
...
inline double cosm1(double x) {
    return cephes::cosm1(x);
}

inline float cosm1(float x) {
    return cosm1(static_cast<double>(x));
}
