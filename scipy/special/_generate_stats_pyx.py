"""
python _generate_stats_pyx.py

Generate Ufunc definition source files for scipy.special. Produces
files '_stats_ufuncs.c' and '_stats_ufuncs_cxx.c' by first producing Cython.

This will generate both calls to PyUFunc_FromFuncAndData and the
required ufunc inner loops.

The functions signatures are contained in 'functions_stats.json', the syntax
...
The C++ module
--------------
In addition to ``_stats_ufuncs`` module, a second module ``_stats_ufuncs_cxx`` is
generated. This module only exports function pointers that are to be
used when constructing some of the ufuncs in ``_stats_ufuncs``. The function
...
# This file is automatically generated by _generate_stats_pyx.py.
...
UFUNCS_EXTRA_CODE = """\
include "_stats_ufuncs_extra_code.pxi"
"""

UFUNCS_EXTRA_CODE_BOTTOM = f"""\

from ._special_ufuncs import ({', '.join(special_stats_ufuncs)})
"""
...
include "_cython_special_stats.pxi"
...
def generate_bench(name, codes):
    ...
    pytemplate = "\n".join(top + end)
    cytemplate = "\n".join(top + middle + end)
    pybench = pytemplate.format(pyfunc, "_stats_ufuncs." + name, ", ".join(inargs))
...
class FusedFunc(Func):
    ...
    def _get_common(self, signum, sig):
        ...
        if cpp:
            # Functions from _stats_ufuncs_cxx are exported as a void*
            # pointers; cast them to the correct types
            func_name = f"scipy.special._stats_ufuncs_cxx._export_{func_name}"
...
def generate_stats_ufuncs(fn_prefix, cxx_fn_prefix, ufuncs):
        ...
        for c_name, c_proto, cy_proto, header in cfuncs:
            if header.endswith('++'):
                ...
                ufunc.function_name_overrides[c_name] = (
                    "scipy.special._stats_ufuncs_cxx._export_" + var_name
                )
            else:
                # usual case
    ...
    all_stats_ufuncs = (
        ...
        [
            f"'{name}'"
            for name in special_stats_ufuncs if not name.startswith('_')
        ]
    )
    module_all = '__all__ = [{}]'.format(', '.join(all_stats_ufuncs))
...
def generate_stats_ufuncs_type_stubs(module_name: str, ufunc_names: list[str]):
    stubs, module_all = [], []
...
def main(outdir):
    ...
    src_files = (os.path.abspath(__file__),
                 os.path.abspath(os.path.join(pwd, 'functions_stats.json')),
                 os.path.abspath(os.path.join(pwd, '_add_newdocs.py')))
    dst_files = ('_stats_ufuncs.pyx',
                 '_stats_ufuncs_defs.h',
                 '_stats_ufuncs_cxx.pyx',
                 '_stats_ufuncs_cxx.pxd',
                 '_stats_ufuncs_cxx_defs.h',
                 '_stats_ufuncs.pyi',
                 'stats_cython_special.pyx',
                 'stats_cython_special.pxd')
    dst_files = (os.path.join(outdir, f) for f in dst_files)

    os.chdir(BASE_DIR)

    if all_newer(src_files, dst_files):
        print("scipy/special/_generate_stats_pyx.py: all files up-to-date")
        return

    ufuncs, ufuncs_type_stubs, fused_funcs = [], [], []
    with open('functions_stats.json') as data:
        functions = json.load(data)
    for f, sig in functions.items():
        if (f not in special_stats_ufuncs):
            ufuncs.append(Ufunc(f, sig))
        ufuncs_type_stubs.append(f)
        fused_funcs.append(FusedFunc(f, sig))
    generate_stats_ufuncs(os.path.join(outdir, "_stats_ufuncs"),
                    os.path.join(outdir, "_stats_ufuncs_cxx"),
                    ufuncs)
    generate_stats_ufuncs_type_stubs(os.path.join(outdir, "_stats_ufuncs"),
                               ufuncs_type_stubs)
    generate_fused_funcs(os.path.join(outdir, "stats_cython_special"),
                         os.path.join(outdir, "_stats_ufuncs"),
                         fused_funcs)
...
if __name__ == "__main__":
    ...
    if not args.outdir:
        raise ValueError("Missing `--outdir` argument to _generate_stats_pyx.py")
    else:
