...
namespace arithmetic {
    namespace aux {
        ...
        inline typing::real_only<RT, RT> next_power_two(const RT &p) {
        ...
        /* Algorithm 3.2 */
        template <typename RT>
        inline typing::real_only<RT, void> extract_scalar(const RT &sigma, RT &p, RT &q) {
            q = (sigma + p) - sigma;
            p -= q;
        }
        ...
        inline typing::real_only<RT, void> extract_vector(const RT &sigma, RT (&p)[LEN], bool (&mask)[LEN], RT &tau) {
            RT q;
            tau = (RT) 0.0;
         ...
         * values. */
        template <typename RT, std::size_t LEN>
        inline typing::real_only<RT, void> make(const RT (&buf)[LEN], bool (&mask)[LEN]) {
            for (std::size_t i = 0; i < LEN; ++i) {
                mask[i] = (buf[i] != 0.0);
            }
        }
        ...
        inline bool none(const bool (&mask)[LEN]) {
            for (std::size_t i = 0; i < LEN; ++i) {
                if (mask[i]) {
                    return false;
                }
        ...
        template <std::size_t LEN>
        inline std::size_t count(const bool (&mask)[LEN]) {
            std::size_t r = 0;
            for (std::size_t i = 0; i < LEN; ++i) {
                if (mask[i]) {
                    ++r;
        ...
        template <typename RT, std::size_t LEN>
        inline typing::real_only<RT, RT> vmax(const RT (&p)[LEN], const bool (&mask)[LEN]) {
            RT v(0.0);
            for (std::size_t i = 0; i < LEN; ++i) {
                if (mask[i]) {
        ...
        template <typename RT, std::size_t LEN>
        inline typing::real_only<RT, RT> masked_sum(const RT (&p)[LEN], const bool (&mask)[LEN]) {
            RT v(0.0);
            for (std::size_t i = 0; i < LEN; ++i) {
                if (mask[i]) {
                    v += p[i];
        ...
        /* Algorithm 6.1 */
        template <typename RT, std::size_t LEN>
        inline typing::real_only<RT, RT> acc_sum(RT (&p)[LEN], bool (&mask)[LEN]) {
            if ((LEN == 0) || none(mask)) {
                return (RT) 0.0;
            }
            ...
            while (true) {
                ...
                tau1 = t + tau;
                if ((std::abs(tau1) >= factor * sigma) || (sigma <= std::numeric_limits<RT>::min())) {
