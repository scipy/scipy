...
namespace arithmetic {
    namespace aux {
        ...
        template <typename T>
        static inline typing::real_only<T, void> rg_dot2_acc(const T &fac, const T &term, T &acc, T &cor) {
...
template <typename T>
static ExitStatus rg0(const T &x, const T &y, const double &rerr, T &res) {
...
template <typename T>
ExitStatus rg(const T &x, const T &y, const T &z, const double &rerr, T &res) {
    typedef typing::decplx_t<T> RT;
    ...
    if ((argcheck::isinf(cct[0]) || argcheck::isinf(cct[1]) || argcheck::isinf(cct[2])) && argcheck::ph_good(cct[0]) &&
        argcheck::ph_good(cct[1]) && argcheck::ph_good(cct[2])) {
        res = typing::huge<T>();
