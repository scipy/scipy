...
template <typename T>
ExitStatus rc(const T &x, const T &y, const double &rerr, T &res) {
    ...
    if (argcheck::too_small(std::imag(y)) && (std::real(y) < 0.0)) {
        T tmpres;
    ...
    sm /= Am;
    /* Eq. (20) of Ref[2] */
    res = arithmetic::comp_horner(sm, constants::RC_C) / (std::sqrt(Am) * (RT) (constants::RC_C[0]));
    return status;
}
