...
namespace arithmetic {
    ...
    inline typing::real_or_cplx<FPT, void> eft_sum(const FPT &x, const FPT &y, FPT &s, FPT &corr) {
        s = x + y;
        FPT z = s - x;
        corr = (x - (s - z)) + (y - z);
    }
    ...
    template <typename FPT>
    inline typing::real_or_cplx<FPT, void> sum2_acc(const FPT &summand, FPT &acc, FPT &corr) {
        FPT tmp_s, tmp_e;
    ...
    inline auto nsum2(const IterableT &x, std::size_t n) -> JUST_ELEM(std::begin(x)) {
        auto it = std::begin(x);
        typedef JUST_ELEM(it) FPT;
        static_assert(
            typing::is_real_or_complex_fp<FPT>::value, "the sum2 function only works with real or complex "
    ...
    inline typing::real_or_cplx<FPT, FPT> sum2(const FPT (&arr)[N]) {
        return nsum2(arr, N);
    }
    ...
    inline typing::real_only<RT, void> eft_prod(const RT &x, const RT &y, RT &prod, typing::corrbuf<RT> &corr) {
        prod = x * y;
    ...
    inline typing::cplx_only<CT, void> eft_prod(const CT &x, const CT &y, CT &prod, typing::corrbuf<CT> &corr) {
    ...
    template <typename RT>
    inline typing::real_only<RT, void> fdot2_acc(const RT &x, const RT &y, RT &acc, RT &corr) {
        RT h, t, q, r;
    ...
    inline auto ndot2(const Iterable &x, const Iterable &y, std::size_t n) ->
        typename std::enable_if<
            std::is_floating_point<JUST_ELEM(std::begin(x))>::value, JUST_ELEM(std::begin(x))>::type {
        auto itx = std::begin(x);
        auto ity = std::begin(y);
        typedef JUST_ELEM(itx) RT;
        RT p(0.0), s(0.0);

        for (std::size_t i = 0; ((itx != std::end(x)) && (ity != std::end(y)) && (i < n)); ++itx, ++ity, ++i) {
    ...
    template <typename Iterable>
    inline auto ndot2(const Iterable &x, const Iterable &y, std::size_t n) ->
        typename std::enable_if<typing::is_complex<JUST_ELEM(std::begin(y))>::value, JUST_ELEM(std::begin(x))>::type {
        auto itx = std::begin(x);
        auto ity = std::begin(y);
    ...
    inline FPT dot2(const FPT (&x)[M], const FPT (&y)[N]) {
     ...
     * Ref. [3] */
    template <typename RT>
    inline typing::real_only<RT, RT> dcomp_horner(const RT &x, const RT *poly, std::size_t degree) {
    ...
    inline typename std::enable_if<
        typing::is_complex<T0>::value || typing::is_complex<T1>::value, typing::Promote<T0, T1>>::type
    dcomp_horner(const T0(&x), const T1 *poly, std::size_t degree) {
        ...
        for (std::ptrdiff_t i = (std::ptrdiff_t) degree - 1; i >= 0; --i) {
            ...
            r = r * x + CT(aux::acc_sum(ws.value_real, mask_r), aux::acc_sum(ws.value_imag, mask_i));
        }
    ...
    inline typing::Promote<T, U> comp_horner(const T &x, const U (&poly)[N]) {
        return dcomp_horner(x, poly, N - 1);
    }

    /* Small integer roots for real types. */
    template <typename RT>
    inline typing::real_only<RT, RT> ocrt(const RT &x) /* 8th (octic) root */
    {
        return std::sqrt(std::sqrt(std::sqrt(x)));
    }

    template <typename RT>
    inline typing::real_only<RT, RT> sxrt(const RT &x) /* 6th (sextic) root */
    {
