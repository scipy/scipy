...
template <typename T>
ExitStatus rj(const T &x, const T &y, const T &z, const T &p, const double &rerr, T &res, bool noasymp = false);
...
namespace rjimpl {
    ...
    template <typename T>
    static inline constexpr typing::real_only<T, bool> asymp_zero(const T &r) {
        return ((0.0 < r) && (r <= config::asym_zero_ul));
    }

    template <typename T>
    static inline constexpr typing::real_only<T, bool> abs_close_zero(const T &r) {
     ...
     */
    template <typename T>
    static bool good_args(const T &x, const T &y, const T &z, const T &p, ArgCases &classify) {
        typedef typing::decplx_t<T> RT;

        if ((classify.hit_pole =
                 (argcheck::too_small(x) && argcheck::too_small(y) && argcheck::ph_good(z) && !argcheck::too_small(p))
            )) {
            return false;
        }
        if ((classify.good_infinity =
                 ((argcheck::isinf(x) || argcheck::isinf(y) || argcheck::isinf(z) || argcheck::isinf(p)) &&
                  (argcheck::ph_good(x) && argcheck::ph_good(y) && argcheck::ph_good(z))))) {
            return false;
        }
        RT xr = std::real(x);
        RT xi = std::imag(x);
        ...
        bool xyzreal_nonneg_atmost1z =
            (argcheck::too_small(xi) && argcheck::too_small(yi) && argcheck::too_small(zi) && (xr >= 0.0) && (yr > 0.0)
            );
        ...
        if ((!argcheck::too_small(p)) && argcheck::ph_good(p)) {
            ...
            if (argcheck::r1conj2(x, y, z) || argcheck::r1conj2(y, z, x) || argcheck::r1conj2(z, x, y)) {
                flag ^= 1u;
            }
            return (bool) flag;
        }
    ...
    static ExitStatus rj_cpv_dispatch(const T &x, const T &y, const T &z, const T &p, const double &rerr, Tres &res) {
    ...
    rj_asym_conf(const T &x, const T &y, const T &z, const T &p, AsymConfig<T> &conf) {
        T t;
        ...
        */
        ...
        if (((0.0 < y) && (y <= 1e-26)) || asymp_zero(t)) {
            ...
            conf.a = (T) 0.5 * (x + y);
            conf.g = std::sqrt(x * y);
            if ((conf.a / z + conf.a / p) * std::abs(std::log(p / conf.a)) <= 1.0) {
    ...
    template <typename T>
    static inline typing::cplx_only<T, T> safe_atan_sqrt_div(T z) {
    ...
    template <typename T>
    static inline typing::real_only<T, T> safe_atan_sqrt_div(T x) {
...
template <typename T>
ExitStatus rj(const T &x, const T &y, const T &z, const T &p, const double &rerr, T &res, bool noasymp) {
    typedef typing::decplx_t<T> RT;
    ...
    if (classify.maybe_asymp && !noasymp) {
        ...
        case rjimpl::AsymFlag::tinyp: {
            ...
            status = rj(xr + lamt, yr + lamt, zr + lamt, pr + lamt, r, xct2[1]);
        ...
        case rjimpl::AsymFlag::tinyy: {
            ...
            tmpres = std::log((RT) 8.0 * zr / (config.a + config.g)) - (RT) 2.0 * tx;
            tx = std::log((RT) 2.0 * pr / (config.a + config.g)) / pr;
            RT rt_h = tx * config.a * ((RT) 1.0 + (RT) 0.5 * pr / zr) / ((RT) 1.0 - config.a / pr);
        ...
        case rjimpl::AsymFlag::hugez: {
            ...
            RT rt_h = (RT) 0.75 * ((RT) 2.0 * pr * tmpres / zr - tt / (zr - config.h));
            tt = (RT) 3.0 / std::sqrt(zr);
            tmpres *= tt;
            /* Also try if direct computation gives reasonable approx. */
            RT tmp_direct;
            ExitStatus status_direct = rj(xr, yr, zr, pr, rerr * (RT) 0.5, tmp_direct, true);
            if ((tmp_direct > tmpres) || (tmp_direct < tmpres + rt_h * tt)) {
    ...
    T yym = Am - ym;
    T zzm = Am - zm;
    RT fterm = std::max({std::abs(xxm), std::abs(yym), std::abs(zzm), std::abs(Am - p)}) / arithmetic::ocrt(rerr / 5.0);
    ...
    while ((aAm = std::abs(Am)) <= fterm ||
           aAm <= std::max({std::abs(xxm), std::abs(yym), std::abs(zzm), std::abs(Am - pm)})) {
        if (m > config::max_iter) {
            status = ExitStatus::n_iter;
            break;
        }
        T rm = sm * (std::sqrt((delta * d4m) / (sm * sm) + (RT) 1.0) + (RT) 1.0);
        ELLINT_RJ_CALC();
