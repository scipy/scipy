...
namespace typing {
    ...
    struct is_complex<std::complex<RT>> : std::enable_if<std::is_floating_point<RT>::value, std::true_type>::type {
        typedef RT rtype;
    ...
    struct is_real_or_complex_fp
        : std::integral_constant<bool, std::is_floating_point<T>::value || is_complex<T>::value> {};
    ...
    using real_only = typename std::enable_if<std::is_floating_point<arg_t>::value, ret_t>::type;

    template <typename arg_t, typename ret_t>
    using cplx_only = typename std::enable_if<is_complex<arg_t>::value, ret_t>::type;

    template <typename arg_t, typename ret_t>
    using real_or_cplx = typename std::enable_if<is_real_or_complex_fp<arg_t>::value, ret_t>::type;
     ...
     * std::numeric_limit<T>. */
    template <typename T>
    constexpr real_only<T, T> nan() {
        return std::numeric_limits<T>::quiet_NaN();
    }

    template <typename T>
    constexpr real_only<T, T> huge() {
        return std::numeric_limits<T>::infinity();
    }

    template <typename T>
    constexpr real_only<T, T> half_epsilon() {
     ...
     * return values. */
    template <typename T>
    constexpr cplx_only<T, T> nan() {
        typedef decplx_t<T> RT;
        return T{nan<RT>(), nan<RT>()};
    }

    template <typename T>
    constexpr cplx_only<T, T> huge() {
        typedef decplx_t<T> RT;
...
#define PIERCE_ITER_REF(ITEXPR) typename std::remove_reference<decltype(*ITEXPR)>::type

#define JUST_ELEM(ITEXPR) typename std::remove_cv<PIERCE_ITER_REF(ITEXPR)>::type
