...
template <typename T>
ExitStatus rd(const T &x, const T &y, const T &z, const double &rerr, T &res) {
    ...
    T xxm = Am - xm;
    T yym = Am - ym;
    RT fterm = std::max({std::abs(xxm), std::abs(yym), std::abs(Am - z)}) / arithmetic::ocrt(rerr / 5.0);
    RT d4m(1.0);
    T adt(0.0), ade(0.0);
    ...
    RT aAm;
    T tmp;
    while ((aAm = std::abs(Am)) <= fterm || aAm <= std::max({std::abs(xxm), std::abs(yym), std::abs(Am - zm)})) {
