...
Real ibeta_inva_wrap(Real p, Real b, Real x)
{
    ...
    if ((b <= 0) || (x <= 0) || (p < 0) || (p > 1)) {
        sf_error("betaincinva", SF_ERROR_DOMAIN, NULL);
        return NAN;
    }
    try {
        ...
        sf_error("betaincinva", SF_ERROR_OVERFLOW, NULL);
        y = INFINITY;
    } catch (const std::underflow_error& e) {
        sf_error("betaincinva", SF_ERROR_UNDERFLOW, NULL);
        y = 0;
    } catch (...) {
        sf_error("betaincinva", SF_ERROR_OTHER, NULL);
        y = NAN;
    }
    return y;
}
