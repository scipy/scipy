...
namespace cephes {
    ...
    namespace detail {
         ...
         constexpr double owens_t_HRANGE[] = {0.02, 0.06, 0.09, 0.125, 0.26, 0.4, 0.6, 1.6,
             1.7, 2.33, 2.4, 3.36, 3.4, 4.8};

         constexpr double owens_t_ARANGE[] = {0.025, 0.09, 0.15, 0.36, 0.5, 0.9, 0.99999};

         constexpr double owens_t_ORD[] = {2, 3, 4, 5, 7, 10, 12, 18, 10, 20, 30, 0, 4, 7,
    8, 20, 0, 0};

         constexpr int owens_t_METHODS[] = {1, 1, 1, 1, 1, 1, 1, 1, 2, 2, 2, 3, 4, 4, 4, 4,
             5, 6};
         ...
         SPECFUN_HOST_DEVICE inline double owens_t_norm1(double x) {
             return special::cephes::erf(x / std::sqrt(2)) / 2;
         }
        ...
        if (fabs_a == std::numeric_limits<double>::infinity()) {
            ...
            if (fabs_ah <= 0.67) {
                ...
                result = 0.25 - normh * normah -
                    detail::owens_t_dispatch(fabs_ah, (1 / fabs_a), h);
            }
            else {
