...
template <typename T>
void legendre_p_init(int n, T z, T (&res)[3]) {
    res[0] = 1;
    res[1] = z;
}

template <typename T>
void legendre_p_init(int n, T z, T (&res)[3], T (&res_jac)[3]) {
...
template <typename T>
void legendre_p_init(int n, T z, T (&res)[3], T (&res_jac)[3], T (&res_hess)[3]) {
...
template <typename T>
void assoc_legendre_p_diag_recur(int m, int type, T z, T (&res)[3]) {
    int m_abs = std::abs(m);

    assoc_legendre_p_diag_recurrence<T> r{std::signbit(m), type, z};
...
template <typename T>
void assoc_legendre_p_diag_recur(int m, int type, T z, T (&res)[3], T (&res_jac)[3], T (&res_hess)[3]) {
    int m_abs = std::abs(m);
...
void assoc_legendre_p_init(int m, int type, T z, T (&res)[3]) {
    ...
    if (true) {
        assoc_legendre_p_diag_init(std::signbit(m), type, z, res);
        assoc_legendre_p_diag_recur(m, type, z, res);
...
void assoc_legendre_p_init(int m, int type, T z, T (&res)[3], T (&res_jac)[3], T (&res_hess)[3]) {
    ...
    if (true) {
        assoc_legendre_p_diag_init(std::signbit(m), type, z, res, res_jac, res_hess);
        assoc_legendre_p_diag_recur(m, type, z, res, res_jac, res_hess);
...
void assoc_legendre_p_all(int type, T z, OutputMat1 res, OutputMat2 res_jac) {
    ...
    assoc_legendre_p_diag_recur(
        -m, type, z, p_diag, p_diag_jac,
        [res, res_jac, n](int i_abs, auto re, const T(&p_diag)[3], const T(&p_diag_jac)[3]) {
            int i = -i_abs;
            ...
            assoc_legendre_p_recur(
                ...
                [res, res_jac](int j, const auto &re, const T(&p)[3], const T(&p_jac)[3]) {
                    if (re.m != 0) {
                        res(re.m + int(res.extent(0)), j) = p[2];
                        res_jac(re.m + int(res.extent(0)), j) = p_jac[2];
                    }
                }
            );
        }
