...
namespace detail {
    ...
    class grad_tuple_impl;

    template <typename T, size_t... I>
    class grad_tuple_impl<T, std::index_sequence<I...>> : public grad_tuple_leaf<T, I>... {
      public:
        grad_tuple_impl() = default;

        template <typename... Args>
        explicit grad_tuple_impl(Args &&...args) : grad_tuple_leaf<T, I>(std::forward<Args>(args))... {}

        std::tuple<typename grad_tuple_leaf<T, I>::value_type &...> refs() {
            return std::forward_as_tuple(static_cast<grad_tuple_leaf<T, I> *>(this)->value...);
        }
...
template <typename T, size_t N>
class grad_tuple : public detail::grad_tuple_impl<T, std::make_index_sequence<N + 1>> {
  public:
    using detail::grad_tuple_impl<T, std::make_index_sequence<N + 1>>::grad_tuple_impl;
};
