...
from numpy.testing import (assert_, assert_equal, assert_almost_equal,
        ...
        dec, TestCase, run_module_suite, assert_allclose, assert_array_less)
...
class TestInterp1D(object):
    ...
    def test_regression_3868_original(self):
        # https://github.com/scipy/scipy/issues/3868
        xs = [
                0, 31, 39, 48, 58, 75, 91, 108, 127, 141, 158, 175, 194, 208,
                224, 241, 258, 291, 310, 343, 379, 427, 444, 459, 493, 526,
                560, 580, 626, 659, 693, 726, 759, 793, 826, 859, 893, 926,
                958, 993, 1025, 1059, 1093, 1125, 1159, 1193, 1224, 1257,
                1278, 1310, 1343, 1379, 1410, 1443, 1478, 1512, 1558, 1662,
                1815, 1823, 1831, 1845, 1860, 1876]
        ys = [
                0.7072243346007605, 0.6996197718631179, 0.6844106463878327,
                0.6730038022813688, 0.6577946768060836, 0.6159695817490495,
                0.5665399239543726, 0.5019011406844106, 0.43346007604562736,
                0.3840304182509506, 0.3041825095057034, 0.2623574144486692,
                0.23574144486692014, 0.20532319391634982, 0.155893536121673,
                0.11406844106463879, 0.07984790874524715, 0.026615969581749048,
                0.0076045627376425855, 0.0, 0.0, 0.0038022813688212928,
                0.022813688212927757, 0.053231939163498096,
                0.12927756653992395, 0.17870722433460076, 0.22433460076045628,
                0.24334600760456274, 0.30798479087452474, 0.33840304182509506,
                0.376425855513308, 0.3840304182509506, 0.376425855513308,
                0.3574144486692015, 0.3041825095057034, 0.2509505703422053,
                0.21292775665399238, 0.1520912547528517, 0.12167300380228137,
                0.09885931558935361, 0.09125475285171103, 0.09125475285171103,
                0.11787072243346007, 0.1596958174904943, 0.20152091254752852,
                0.24714828897338403, 0.28517110266159695, 0.3155893536121673,
                0.33840304182509506, 0.3688212927756654, 0.39543726235741444,
                0.44106463878326996, 0.4828897338403042, 0.5057034220532319,
                0.5247148288973384, 0.5285171102661597, 0.532319391634981,
                0.5361216730038023, 0.5475285171102662, 0.5627376425855514,
                0.5779467680608364, 0.5893536121673004, 0.6007604562737643,
                0.6083650190114068]
        assert_equal(len(xs), 64)
        assert_equal(len(ys), 64)
        assert_array_less(xs[:-1], xs[1:])
        xs = np.array(xs)
        fx = interp1d(xs, ys, kind='cubic')
        assert_allclose(fx(xs), ys, atol=1e-12)

    def test_regression_3868_minimal(self):
        # https://github.com/scipy/scipy/issues/3868
        xs = [
                0, 31, 39, 48, 58, 75, 91, 108, 127, 141, 158, 175, 194, 208,
                224, 241, 258, 291, 310, 343, 379, 427, 444, 459, 493, 526,
                ...
                1815, 1823, 1831, 1845, 1860, 1876]
        ys = [
                70, 69, 68, 67, 65, 61, 56, 50, 43, 38, 30, 26, 23, 20, 15,
                11, 7, 2, 0, 0, 0, 0, 2, 5, 12, 17, 22, 24, 30, 33, 37, 38,
                37, 35, 30, 25, 21, 15, 12, 9, 9, 9, 11, 15, 20, 24, 28, 31,
                33, 36, 39, 44, 48, 50, 52, 52, 53, 53, 54, 56, 57, 58, 60, 60]
        assert_equal(len(xs), 64)
        assert_equal(len(ys), 64)
        assert_array_less(xs[:-1], xs[1:])
        xs = np.array(xs)
