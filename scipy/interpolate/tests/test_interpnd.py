...
class TestEstimateGradients2DGlobal:
    def test_smoketest(self):
        ...
        for j, (func, grad) in enumerate(funcs):
            ...
            assert dz.shape == (6, 2)
            xp_assert_close(dz, np.array(grad)[None,:] + 0*dz, rtol=1e-5, atol=1e-5, err_msg=f"item {j}")
...
class TestCloughTocher2DInterpolator:
    ...
    def test_linear_smoketest(self):
        ...
        for j, func in enumerate(funcs):
            self._check_accuracy(func, tol=1e-13, atol=1e-7, rtol=1e-7, err_msg=f"Function {j}")
            self._check_accuracy(func, tol=1e-13, atol=1e-7, rtol=1e-7, alternate=True, err_msg=f"Function (alternate) {j}")
            # check rescaling
            self._check_accuracy(func, tol=1e-13, atol=1e-7, rtol=1e-7, err_msg=f"Function (rescaled) {j}", rescale=True)
            self._check_accuracy(func, tol=1e-13, atol=1e-7, rtol=1e-7, alternate=True, rescale=True, err_msg=f"Function (alternate, rescaled) {j}")
    ...
    def test_quadratic_smoketest(self):
        ...
        for j, func in enumerate(funcs):
            self._check_accuracy(func, tol=1e-9, atol=0.22, rtol=0, err_msg=f"Function {j}")
            self._check_accuracy(func, tol=1e-9, atol=0.22, rtol=0, err_msg=f"Function {j}", rescale=True)
    ...
    def test_dense(self):
        ...
        for j, func in enumerate(funcs):
            self._check_accuracy(func, x=grid, tol=1e-9, atol=5e-3, rtol=1e-2, err_msg=f"Function {j}")
            self._check_accuracy(func, x=grid, tol=1e-9, atol=5e-3, rtol=1e-2, err_msg=f"Function {j}", rescale=True)
