...
class TestLinearNDInterpolation:
    ...
    def test_square(self):
        ...
        values = np.array([1.0, 2.0, -3.0, 5.0], dtype=np.double)
    ...
    def test_smoketest_rescale(self):
        # Test at single points
        x = np.array([(0, 0), (-5, -5), (-5, 5), (5, 5), (2.5, 3)], dtype=np.double)
        y = np.arange(x.shape[0], dtype=np.double)
...
class TestCloughTocher2DInterpolator:
    ...
    def test_linear_smoketest(self):
        ...
        for j, func in enumerate(funcs):
            self._check_accuracy(
                func, tol=1e-13, atol=1e-7, rtol=1e-7, err_msg="Function %d" % j
            )
            self._check_accuracy(
                func,
                tol=1e-13,
    ...
    def test_quadratic_smoketest(self):
        ...
        for j, func in enumerate(funcs):
            self._check_accuracy(
                func, tol=1e-9, atol=0.22, rtol=0, err_msg="Function %d" % j
            )
            self._check_accuracy(
                func,
                tol=1e-9,
    ...
    def test_dense(self):
        ...
        for j, func in enumerate(funcs):
            self._check_accuracy(
                func, x=grid, tol=1e-9, atol=5e-3, rtol=1e-2, err_msg="Function %d" % j
            )
            self._check_accuracy(
                func,
                x=grid,
