...
def Highs_call(colcost, collower, colupper, rowlower, rowupper,
    ...
    retcode = highslib.Highs_call(
            ...
            dbl_array_type_col(*colcost), dbl_array_type_col(*collower),
            dbl_array_type_col(*colupper), dbl_array_type_row(*rowlower),
            dbl_array_type_row(*rowupper), int_array_type_astart(*astart),
            int_array_type_aindex(*aindex), dbl_array_type_avalue(*avalue),
            col_value, col_dual,
            row_value, row_dual,
            col_basis, row_basis, ctypes.byref(ctypes.c_int(return_val)))
    return (retcode, list(col_value), list(col_dual), list(row_value),
            list(row_dual), list(col_basis), list(row_basis))
