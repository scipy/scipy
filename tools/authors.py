...
def main():
    ...
    def analyze_line(line, names, disp=False):
        ...
        m = re.search(
            r"([Tt]hanks to|[Cc]ourtesy of) ([A-Z][A-Za-z]*? [A-Z][A-Za-z]*?"
            r" [A-Z][A-Za-z]*|[A-Z][A-Za-z]*? [A-Z]\. [A-Z][A-Za-z]*|[A-Z][A-Za-z ]*?"
            r" [A-Z][A-Za-z]*|[a-z0-9]+)($|\.| )",
            line,
        )
        if m:
    ...
    for line in git.pipe("log", "--pretty=@@@%an@@@%n@@@%cn@@@%n%b", "%s" % (rev1,)):
        analyze_line(line, all_authors)

    # Find authors in the named range
    for line in git.pipe(
        "log", "--pretty=@@@%an@@@%n@@@%cn@@@%n%b", "%s..%s" % (rev1, rev2)
    ):
