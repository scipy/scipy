...
Generate Ufunc definition source files.  Produces files '_ufuncs.c'
and '_ufuncs_cxx.c' by first producing Cython.
...
# This file is automatically generated by generate_pyx.py.
...
class Ufunc(Func):
    ...
    """
    def __init__(self, name, signatures, doc):
...
def generate_ufuncs(fn_prefix, cxx_fn_prefix, ufuncs, module_path):
    ...
    with open(os.path.join(module_path, proto_h_filename), 'w') as f:
        f.write("#ifndef UFUNCS_PROTO_H\n#define UFUNCS_PROTO_H 1\n")
        f.write("\n".join(defs_h))
        f.write("\n#endif\n")

    cxx_defs_h = unique(cxx_defs_h)
    with open(os.path.join(module_path, cxx_proto_h_filename), 'w') as f:
        f.write("#ifndef UFUNCS_PROTO_H\n#define UFUNCS_PROTO_H 1\n")
        f.write("\n".join(cxx_defs_h))
        f.write("\n#endif\n")

    with open(os.path.join(module_path, cxx_pyx_filename), 'w') as f:
        f.write(UFUNCS_EXTRA_CODE_COMMON)
        f.write("\n")
        f.write("\n".join(cxx_defs))
        f.write("\n# distutils: language = c++\n")

    with open(os.path.join(module_path, cxx_pxd_filename), 'w') as f:
        f.write("\n".join(cxx_pxd_defs))


def generate_fused_funcs(modname, ufunc_fn_prefix, fused_funcs, module_path):
    pxdfile = modname + ".pxd"
    pyxfile = modname + ".pyx"
    ...
    with open(os.path.join(module_path, pxdfile), 'w') as f:
        f.write(CYTHON_SPECIAL_PXD)
        f.write("\n")
        f.write("\n\n".join(fused_types))
        f.write("\n\n")
        f.write("\n".join(declarations))
    with open(os.path.join(module_path, pyxfile), 'w') as f:
...
def main():
    parser = argparse.ArgumentParser(description='Generate ufuncs')
    parser.add_argument('config_file')
    args = parser.parse_args()

    with open(args.config_file) as config_data:
        config = json.load(config_data)
    module_path = os.path.dirname(args.config_file)
    functions_file = os.path.join(
        module_path, config['function_declarations']
    )
    docs_module = os.path.join(
        module_path, config['docs_module']
    )
    cython_api = config.get('cython_api')

    src_files = (functions_file, docs_module)

    with open(functions_file) as data:
        functions = json.load(data)
    docs_module = SourceFileLoader(
        os.path.join(module_path, 'docs_module'), docs_module
    ).load_module()
    ...
    ufunc_dst_files = [os.path.join(module_path, f) for f in ufunc_dst_files]
    if not all_newer(src_files, ufunc_dst_files):
        ufuncs = [
            Ufunc(f, sig, docs_module.get(f)) for f, sig in functions.items()
        ]
        generate_ufuncs('_ufuncs', '_ufuncs_cxx', ufuncs, module_path)
    else:
        print('generate_pyx.py: not generating ufuncs because all '
    ...
    cython_dst_files = [
        '{}.pyx'.format(cython_api),
        '{}.pxd'.format(cython_api)
    ]
    cython_dst_files = (
        ufunc_dst_files
        + [os.path.join(module_path, f) for f in ufunc_dst_files]
    )
    if not all_newer(src_files, cython_dst_files):
        fused_funcs = [FusedFunc(f, sig) for f, sig in functions.items()]
        generate_fused_funcs(
            cython_api, '_ufuncs', fused_funcs, module_path
        )
    else:
        print('generate_pyx.py: not generating Cython functions '
              'because all source files are up to date')
