...
def dfovec(m, n, x, nprob):
    ...
    c29 = 2.9e1
    c45 = 4.5e1
    v = [4.0e0, 2.0e0, 1.0e0, 5.0e-1, 2.5e-1, 1.67e-1, 1.25e-1, 1.0e-1,
         8.33e-2, 7.14e-2, 6.25e-2]
    y1 = [1.4e-1, 1.8e-1,2.2e-1, 2.5e-1, 2.9e-1, 3.2e-1, 3.5e-1, 3.9e-1,
          3.7e-1, 5.8e-1, 7.3e-1, 9.6e-1, 1.34e0, 2.1e0, 4.39e0]
    y2 = [1.957e-1, 1.947e-1, 1.735e-1, 1.6e-1, 8.44e-2, 6.27e-2, 4.56e-2,
          3.42e-2, 3.23e-2, 2.35e-2, 2.46e-2]
    y3 = [3.478e4, 2.861e4, 2.365e4, 1.963e4, 1.637e4, 1.372e4, 1.154e4,
          9.744e3, 8.261e3, 7.03e3, 6.005e3, 5.147e3, 4.427e3, 3.82e3,
          3.307e3, 2.872e3]
    y4 = [8.44e-1, 9.08e-1, 9.32e-1, 9.36e-1, 9.25e-1, 9.08e-1, 8.81e-1,
    ...
    y5 = [1.366e0, 1.191e0, 1.112e0, 1.013e0, 9.91e-1, 8.85e-1, 8.31e-1,
    ...
    if nprob == 1:  # Linear function - full rank.
        ...
        for i in range(m):
            temp = i+1
            tmp1 = temp / 10
            fvec[i] = np.exp(-tmp1*x[0]) - np.exp(-tmp1*x[1]) \
        ...
        for i in range(n-4):
            fvec[i] = (-4*x[i] + 3.0)
            fvec[n-4+i] = x[i]**2 + 2*x[i+1]**2 + 3*x[i+2]**2 + 4*x[i+3]**2 \
                + 5*x[n-1]**2
    elif nprob == 20:  # Cube
        # n = 2, m = n
        fvec[1] = x[0] - 1.0
        ...
        fvec[6] = x[0]*x[4]*(x[4]**2 - 3.0*x[6]**2) + x[2]*x[6]*(x[6]**2
            - 3.0*x[4]**2) + x[1]*x[5]*(x[5]**2 - 3.0*x[7]**2) \
            + x[3]*x[7]*(x[7]**2 - 3.0*x[5]**2) + 12.6
        fvec[7] = x[2]*x[4]*(x[4]**2 - 3.0*x[6]**2) - x[0]*x[6]*(x[6]**2
            - 3.0*x[4]**2) + x[3]*x[5]*(x[5]**2 - 3.0*x[7]**2) \
            - x[1]*x[7]*(x[7]**2 - 3.0*x[6]**2) - 9.48
    else:
