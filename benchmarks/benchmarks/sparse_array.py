...
with safe_import():
    ...
    from scipy.sparse import (coo_array, dia_array, lil_array,
                              dok_array, random_array, SparseEfficiencyWarning)
...
def random_sparse(m, n, nnz_per_row):
    ...
    return coo_array((vals, (rows, cols)), (m, n)).tocsr()


def poisson2d(N, dtype='d', format=None):
    """
    Return a sparse array for the 2D Poisson problem
    with standard 5-point finite difference stencil on a
    square N-by-N grid.
    """
    if N == 1:
        diags = asarray([[4]], dtype=dtype)
        return dia_array((diags, [0]), shape=(1, 1)).asformat(format)
    ...
    return dia_array((diags, offsets), shape=(N**2, N**2)).asformat(format)
...
class Matvec(Benchmark):
    ...
    param_names = ['array', 'format']

    def setup(self, array, format):
        if array == 'Identity':
            if format in ('lil', 'dok'):
                raise NotImplementedError()
            self.A = sparse.eye(10000, 10000, format=format)
        elif array == 'Poisson5pt':
    ...
    def time_matvec(self, array, format):
        self.A @ self.x
...
class Matmul(Benchmark):
    def setup(self):
        ...
        data = rng.random(C1)
        self.array1 = coo_array((data, (i, j)), shape=(H1, W1)).tocsr()

        i = rng.integers(H2, size=C2)
        j = rng.integers(W2, size=C2)
        data = rng.random(C2)
        self.array2 = coo_array((data, (i, j)), shape=(H2, W2)).tocsr()
...
class Getset(Benchmark):
    ...
    def setup(self, N, sparsity_pattern, format):
        ...
        self.A = random_array((1000, 1000), density=1e-5)
...
class NullSlice(Benchmark):
    ...
    def _setup(self, density, format):
        ...
        # faster version of random_array((n, k), format=format, density=density),
        # with non-exact nnz
        nz = int(n * k * density)
        row = np.random.randint(0, n, size=nz)
        col = np.random.randint(0, k, size=nz)
        data = np.ones(nz, dtype=np.float64)
        X = coo_array((data, (row, col)), shape=(n, k))
        X.sum_duplicates()
        X = X.asformat(format)
    ...
    def setup(self, density, format):
        # Unpickling is faster than computing the random array...
...
class Iteration(Benchmark):
    ...
    def setup(self, density, format):
        n = 500
        k = 1000
        self.X = random_array((n, k), format=format, density=density)
...
class Densify(Benchmark):
    ...
    def setup(self, format, order):
        warnings.simplefilter('ignore', SparseEfficiencyWarning)
        self.X = random_array((1000, 1000), format=format, density=0.01)
...
class Random(Benchmark):
    ...
    def time_rand(self, density):
        random_array((self.nrows, self.ncols),
...
class Argmax(Benchmark):
    ...
    def setup(self, density, format, explicit):
        ...
        self.X = random_array((n, n), format=format, density=density)
