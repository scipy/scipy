...
def random_sparse(m, n, nnz_per_row):
    ...
    return sparse.csr_array((vals, (rows, cols)), shape=(m, n))
...
def poisson2d(N, dtype='d', format=None):
    ...
    if N == 1:
        ...
        return sparse.dia((diags, [0]), shape=(1, 1)).asformat(format)
...
class Arithmetic(Benchmark):
    ...
    def setup(self, format, XY, op):
        A = poisson2d(250, format=format)
        B = (A @ A).asformat(format)
        matrices = dict(A=A, B=B)
...
class Construction(Benchmark):
    ...
    def setup(self, name, format):
        ...
        formats = {'lil': sparse.lil_array, 'dok': sparse.dok_array}
...
class NullSlice(Benchmark):
    ...
    def _setup(self, density, format):
        ...
        X = sparse.coo_array((data, (row, col)), shape=(n, k))
        X.sum_duplicates()
        X = X.asformat(format)
