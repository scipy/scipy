...
#
# Hacked by Pearu (14Apr2004):
#  - renamed to my_pydoc.py
#  - support f2py generated fortran objects
#  - changed html.head output to add some plone specific metadata
#  - other minor changes
#
...
def getdoc(object):
    ...
    if not result.count('\n') and len(result)>100:
        # Deal with badly formated documentation strings like in Numeric.array.
        words = result.split()
        lines = [[]]
        l = 0
        while words:
...
def isdata(object):
    ...
    if str(type(object))=="<type 'fortran'>":
        return 0
    return not (inspect.ismodule(object) or inspect.isclass(object) or
                inspect.isroutine(object) or inspect.isframe(object) or
                inspect.istraceback(object) or inspect.iscode(object))
def isf2py(object):
    return str(type(object))=="<type 'fortran'>"
def isfunction(object):
    return inspect.isfunction(object)
def isroutine(object):
    return inspect.isroutine(object)
...
class Doc:
    def document(self, object, name=None, *args):
        ...
        try:
            ...
            if isf2py(object): return self.docf2py(*args)
            if isroutine(object): return self.docroutine(*args)
    ...
    docmodule = docclass = docroutine = docother = docf2py = fail
...
class HTMLDoc(Doc):
    ...
    def __page(self, title, contents):
        """Format an HTML page."""
        return '''
...
%s
    ...
    def page(self, title, contents):
        ...
        <head>
        <title>%s</title>
        <meta name="Subject" content="%s by pydoc." />
        <meta name="Type" content="Document" />
        <meta name="Format" content="text/html" />
        </head>
        <body>
        %s
        </body></html>''' % (title, title, contents)
...
<tr><td bgcolor="%s">%s</td><td>%s</td>''' % (bgcol, marginalia, gap)
    ...
    def docmodule(self, object, name=None, mod=None, *ignored):
        ...
        funcs, fdict = [], {}
        for key, value in inspect.getmembers(object, isroutine):
            if inspect.isbuiltin(value) or inspect.getmodule(value) is object:
                if visiblename(key):
                    funcs.append((key, value))
                    fdict[key] = '#-' + key
                    if isfunction(value): fdict[value] = fdict[key]

        for key, value in inspect.getmembers(object, isf2py):
            if visiblename(key):
                funcs.append((key, value))
                fdict[key] = '#-' + key
    ...
    def docf2py(self, object, name=None, mod=None,
                   funcs={}, classes={}, methods={}, cl=None):
        """Produce HTML documentation for a function or method object."""
