...
def build(*, parent_callback, meson_args, jobs, verbose, werror, asan, debug,
    ...
    if asan:
        ...
        compiler_args = f'-fsanitize=address -fno-omit-frame-pointer -fsanitize-ignorelist={asan_ignore_file}'
        meson_args = meson_args + (f'-Dc_args={compiler_args}', )
        meson_args = meson_args + (f'-Dcpp_args={compiler_args}', )
        meson_args = meson_args + (f'-Dc_link_args=-fsanitize=address', )
        meson_args = meson_args + (f'-Dcpp_link_args=-fsanitize=address', )
