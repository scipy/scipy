...
def get_build_ext_override():
    ...
    if int(os.environ.get("SCIPY_USE_PYTHRAN", 1)):
        ...
        try:
            ...
            if LooseVersion(pythran.__version__) < LooseVersion("0.9.11"):
                raise RuntimeError(
                    "The installed `pythran` is too old, >= "
                    "0.9.11 is needed, {} detected. Please "
                    "upgrade Pythran, or use `export "
                    "SCIPY_USE_PYTHRAN=0`.".format(pythran.__version__)
                )
        except ImportError:
    ...
    class build_ext(BaseBuildExt):
        def finalize_options(self):
            ...
            if self.parallel:
                print(
                    "NOTE: -j build option not supported. Set NPY_NUM_BUILD_JOBS=4 "
                    "for parallel build."
                )
            self.parallel = None
...
def generate_cython():
    ...
    p = subprocess.call(
        [sys.executable, os.path.join(cwd, "tools", "cythonize.py"), "scipy"], cwd=cwd
    )
    if p != 0:
        ...
        try:
            ...
            if LooseVersion(pip.__version__) < LooseVersion("18.0.0"):
                raise RuntimeError(
                    "Cython not found or too old. Possibly due "
