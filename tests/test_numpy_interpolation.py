...
# ============================================================
# New interp1d kinds: nearest-up, zero, previous, next, slinear,
#                     quadratic, and integer-order B-splines
# ============================================================

def _scipy_kind(x, y, kind, xq, fill_value='extrapolate'):
    """Thin wrapper: scipy interp1d → values at xq."""
    f = scipy_interp1d(x, y, kind=kind, fill_value=fill_value,
                       bounds_error=(fill_value != 'extrapolate'))
    return f(xq)

def _our_kind(x, y, kind, xq, fill_value='extrapolate'):
    """Thin wrapper: our interp1d → values at xq."""
...
def test_zero_kind():
    """'zero' — left-step (piecewise constant, value of left data point)."""
    print("test_zero_kind")
    x  = np.array([0., 1., 2., 3., 4.])
    y  = np.array([10., 20., 30., 40., 50.])
    xq = np.array([0., 0.1, 0.9, 1.0, 1.5, 2.9, 3.0, 4.0])

    ref = _scipy_kind(x, y, 'zero', xq)
    got = _our_kind(x, y, 'zero', xq)
    assert check(got, ref, "zero vs scipy")

    # Explicit step semantics: value at left edge
    expected = np.array([10., 10., 10., 20., 20., 30., 40., 50.])
    assert check(got, expected, "zero explicit values")


def test_previous_kind():
    """'previous' — synonym for 'zero'."""
    print("test_previous_kind")
    x  = np.array([0., 1., 2., 3., 4.])
    y  = np.array([10., 20., 30., 40., 50.])
    xq = np.array([0., 0.5, 1.0, 2.5, 3.9, 4.0])

    ref = _scipy_kind(x, y, 'previous', xq)
    got = _our_kind(x, y, 'previous', xq)
    assert check(got, ref, "previous vs scipy")

    zero_got = _our_kind(x, y, 'zero', xq)
    assert check(got, zero_got, "previous == zero")


def test_next_kind():
    """'next' — right-step (piecewise constant, value of right data point)."""
    print("test_next_kind")
    x  = np.array([0., 1., 2., 3., 4.])
    y  = np.array([10., 20., 30., 40., 50.])
    xq = np.array([0., 0.1, 0.9, 1.0, 1.5, 2.9, 3.0, 4.0])

    ref = _scipy_kind(x, y, 'next', xq)
    got = _our_kind(x, y, 'next', xq)
    assert check(got, ref, "next vs scipy")

    # value at right edge
    expected = np.array([10., 20., 20., 20., 30., 40., 40., 50.])
    assert check(got, expected, "next explicit values")


def test_nearest_up_kind():
    """'nearest-up' — rounds midpoints up; 'nearest' rounds midpoints down."""
    print("test_nearest_up_kind")
    x  = np.array([0., 1., 2., 3., 4.])
    y  = np.array([0., 1., 4., 9., 16.])
    xq = np.array([0., 0.5, 1.0, 1.5, 2.5, 3.5, 4.0])

    # scipy does not have 'nearest-up' but documents the tie-breaking:
    # nearest-up uses right boundary → midpoints go to higher index
    got_up  = _our_kind(x, y, 'nearest-up', xq)
    got_near = _our_kind(x, y, 'nearest',   xq)

    # at non-midpoints both should agree
    non_mid = np.array([0., 1., 4., 9., 16.])     # at exact nodes
    assert check(_our_kind(x, y, 'nearest-up', x), non_mid, "nearest-up at nodes")
    assert check(_our_kind(x, y, 'nearest',    x), non_mid, "nearest at nodes")

    # midpoints: nearest-up → higher, nearest → lower
    mids = np.array([0.5, 1.5, 2.5, 3.5])
    up   = _our_kind(x, y, 'nearest-up', mids)
    dn   = _our_kind(x, y, 'nearest',    mids)
    # nearest-up midpoint 0.5 → index 1 (y=1); nearest → index 0 (y=0)
    assert check(up, np.array([1., 4., 9., 16.]), "nearest-up at midpoints")
    assert check(dn, np.array([0., 1.,  4.,  9.]), "nearest at midpoints")


def test_slinear_kind():
    """'slinear' is an alias for 'linear'."""
    print("test_slinear_kind")
    x  = np.linspace(0., 4., 9)
    y  = np.sin(x)
    xq = np.linspace(0., 4., 37)

    ref_lin   = _our_kind(x, y, 'linear',  xq)
    got_slin  = _our_kind(x, y, 'slinear', xq)
    assert check(got_slin, ref_lin, "slinear == linear")

    ref_scipy = _scipy_kind(x, y, 'slinear', xq)
    assert check(got_slin, ref_scipy, "slinear vs scipy")


def test_quadratic_kind():
    """'quadratic' — degree-2 B-spline interpolation."""
    print("test_quadratic_kind")
    x  = np.array([0., 1., 2., 3., 4.])
    y  = x**2   # exact polynomial of degree 2 → should be reproduced exactly

    xq = np.array([0., 0.5, 1., 1.5, 2., 2.5, 3., 3.5, 4.])
    ref = _scipy_kind(x, y, 'quadratic', xq)
    got = _our_kind(x, y, 'quadratic', xq)
    assert check(got, ref, "quadratic vs scipy (x^2)")

    # Extra: sine curve — match scipy within tight tolerance
    x2 = np.linspace(0., 2*np.pi, 12)
    y2 = np.sin(x2)
    xq2 = np.linspace(0., 2*np.pi, 50)
    ref2 = _scipy_kind(x2, y2, 'quadratic', xq2)
    got2 = _our_kind(x2, y2, 'quadratic', xq2)
    assert check(got2, ref2, "quadratic vs scipy (sin)")


def test_integer_order_bspline():
    """Integer kind=k uses B-spline of degree k."""
    print("test_integer_order_bspline")
    x  = np.array([0., 1., 2., 3., 4., 5.])
    y  = np.exp(x)
    xq = np.linspace(0., 5., 30)

    for k in (0, 1, 2, 3, 4):
        ref = _scipy_kind(x, y, k, xq)
        got = _our_kind(x, y, k, xq)
        assert check(got, ref, f"integer kind={k} vs scipy")

    # k=0 should match 'zero'
    got0  = _our_kind(x, y, 0, xq)
    got_z = _our_kind(x, y, 'zero', xq)
    assert check(got0, got_z, "kind=0 == 'zero'")

    # k=1 should match 'linear'
    got1  = _our_kind(x, y, 1, xq)
    got_l = _our_kind(x, y, 'linear', xq)
    assert check(got1, got_l, "kind=1 == 'linear'")

    # k=3 should match 'cubic' (not-a-knot vs natural may differ slightly)
    got3   = _our_kind(x, y, 3, xq)
    ref_cs = _scipy_kind(x, y, 'cubic', xq)
    assert check(got3, ref_cs, "kind=3 vs scipy cubic", atol=1e-10)


def test_step_kinds_2d_y():
    """zero/previous/next work for 2-D y arrays."""
    print("test_step_kinds_2d_y")
    x  = np.array([0., 1., 2., 3.])
    y  = np.column_stack([x, x**2])  # shape (4, 2)
    xq = np.array([0., 0.5, 1.5, 2.5, 3.])

    for kind in ('zero', 'previous', 'next'):
        ref = scipy_interp1d(x, y, kind=kind, axis=0,
                             fill_value='extrapolate')(xq)
        got = interp1d(x, y, kind=kind, axis=0,
                       fill_value='extrapolate')(xq)
        assert check(got, ref, f"{kind} 2-D y vs scipy")


def test_quadratic_2d_y():
    """quadratic/slinear work for 2-D y arrays."""
    print("test_quadratic_2d_y")
    x  = np.linspace(0., 4., 8)
    y  = np.column_stack([np.sin(x), np.cos(x)])
    xq = np.linspace(0., 4., 25)

    for kind in ('slinear', 'quadratic'):
        ref = scipy_interp1d(x, y, kind=kind, axis=0,
                             fill_value='extrapolate')(xq)
        got = interp1d(x, y, kind=kind, axis=0,
                       fill_value='extrapolate')(xq)
...
def test_new_kinds_fill_value_nan():
    """Out-of-range queries return NaN when bounds_error=False, fill_value=nan."""
    print("test_new_kinds_fill_value_nan")
    x  = np.array([1., 2., 3., 4.])
    y  = np.array([1., 4., 9., 16.])
    xq = np.array([0., 1., 2.5, 4., 5.])

    for kind in ('zero', 'previous', 'next', 'nearest-up', 'slinear', 'quadratic'):
        f = interp1d(x, y, kind=kind, bounds_error=False, fill_value=np.nan)
        got = f(xq)
        assert np.isnan(got[0]),  f"{kind}: left OOB not NaN"
        assert np.isnan(got[-1]), f"{kind}: right OOB not NaN"
        assert np.isfinite(got[1:-1]).all(), f"{kind}: interior has NaN"
        print(f"  [PASS] {kind} fill_value=nan OOB")


def test_new_kinds_extrapolate():
    """fill_value='extrapolate' does not raise for out-of-range queries."""
    print("test_new_kinds_extrapolate")
    x  = np.array([0., 1., 2., 3., 4.])
    y  = np.array([0., 1., 4., 9., 16.])
    xq = np.array([-1., 5.])
    ...
    for kind in ('zero', 'previous', 'next', 'nearest-up', 'slinear', 'quadratic'):
        try:
            f   = interp1d(x, y, kind=kind, fill_value='extrapolate')
            got = f(xq)
            print(f"  [PASS] {kind} extrapolate no error, got {got}")
        except Exception as e:
            print(f"  [FAIL] {kind} extrapolate raised {e}")
            assert False, f"{kind} raised on extrapolate: {e}"


def test_new_kinds_bounds_error():
    """bounds_error=True raises on out-of-range queries for all new kinds."""
    print("test_new_kinds_bounds_error")
    x  = np.array([0., 1., 2., 3., 4.])
    y  = np.array([0., 1., 4., 9., 16.])

    for kind in ('zero', 'previous', 'next', 'nearest-up', 'slinear', 'quadratic'):
        f = interp1d(x, y, kind=kind, bounds_error=True)
        try:
            f(np.array([-1.]))
            print(f"  [FAIL] {kind}: no error on OOB")
            assert False
        except ValueError:
            print(f"  [PASS] {kind} raises ValueError on OOB")
...
if __name__ == '__main__':
    tests = [
        ...
        test_previous_kind,
        test_next_kind,
        test_nearest_up_kind,
        test_slinear_kind,
        test_quadratic_kind,
        test_integer_order_bspline,
        test_step_kinds_2d_y,
        test_quadratic_2d_y,
        test_new_kinds_fill_value_nan,
        test_new_kinds_extrapolate,
        test_new_kinds_bounds_error,
    ]
    for t in tests:
        t()
    print("\nDone.")
