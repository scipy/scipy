...
- In SciPy's bindings for BLAS or LAPACK (two sets: hand-written C++, generated
  Cython wrappers),
...
called, and with what inputs. The answer is in the C++ wrappers under
``scipy/linalg/src/``: ``src/lapack/lapack_*.cpp`` for LAPACK and
``src/blas/blas_l[123].cpp`` for BLAS. Each routine is one function template
whose body reads as the argument list, the checks and the BLAS/LAPACK call, so
the exact arguments handed to the library are visible there directly. This can
then be reproduced in C/Fortran by defining some integer/float variables and
arrays (typically small arrays with hardcoded numbers are enough).
...
Here is an example ``gdb`` session::
    ...
    lapack::capi::potrf<double> (self=<optimized out>, args=<optimized out>,
        kwds=<optimized out>)
        at ../scipy/linalg/src/lapack/lapack_pos_def.cpp:176
    ....
    ...
    (gdb) bt
    #0  0x00007ffff3056b1e in lapack::capi::potrf<double> ()
    ...
    #1  0x0000555555734323 in _PyObject_MakeTpCall (tstate=0x555555ad0558 <_PyRuntime+166328>,
        callable=<lapack_function at remote 0x7ffff40ffc00>, args=<optimized out>, nargs=1,
