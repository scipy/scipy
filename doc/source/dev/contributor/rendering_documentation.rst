...
.. note::
   ...
   - The SciPy documentation contains interactive examples rendered with the
     ...
     enabled for local builds of the documentation, or in docs CI previews. For
     more details, see :ref:`interactive-docs`.
...
documents written in MyST-NB_ markdown format. These files are executable,
meaning that their content is executed when the SciPy documentation is built
(locally or on CI) and any outputs generated by the execution are rendered in
the final HTML files. In addition, they can be made interactive in the SciPy
documentation website by using the
...
ignored to the ``ignore_patterns`` list. This list expects elements to be
JavaScript Regex patterns. The initial version of this file in a clean
