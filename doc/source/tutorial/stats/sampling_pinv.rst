...
error. Its primary purpose is to provide very fast sampling that is nearly
...
variate :math:`X` is generated by transforming the uniform random variate :math:`U`
...
* The algorithm has problems when the distribution has heavy tails (as then
  ...
  when the requested u-resolution is less than 1e-12.
...
The generator has been set up and we can start sampling from our distribution:
...
generated RVs follow the exact distribution more closely. Note
...
fast PDF is critical to the algorithm. However, this helps reduce the number
