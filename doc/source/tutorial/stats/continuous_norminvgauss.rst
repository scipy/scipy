...
A normal inverse Gaussian random variable with parameters :math:`a` and :math:`b` can be expressed  as :math:`X = b V + \sqrt{V} Z` where :math:`Z` is ``norm(0,1)`` and :math:`V` is ``invgauss(mu=1/sqrt(a**2 - b**2))``. Hence, the normal inverse Gaussian distribution is a special case of normal variance-mean mixtures.
