...
``y.ndim > 1``. The interpretation is that the first dimension of ``y`` is the
interpolation axis, and the trailing dimensions represent a *batch* of 1D data arrays.
One way of thinking about it is to imagine a collection (a *batch*) of functions,
:math:`f(x), g(x), h(x), \dots`, sampled at the points :math:`x_j`, such that the ``y``
array represents a collection of :math:`f(x_j), g(x_j), h(x_j),\dots`, stacked along
the the first dimension.
...
2. The batching is along the *trailing* dimensions, in contrast to the NumPy functions
   where the broadcasting works across the *leading* dimensions.
; 
...
- ``y.shape[axis] == x.shape[0]`` (otherwise en error is raised)
