...
A further option for nonparametric interpolation is isotonic regression via
`IsotonicInterpolator` which constructs a linear interpolant with constant
regions. It is the only interpolator in scipy that enforces a truly
monotonically increasing solution.

To illustrate, let's consider data with an outlier:

.. plot::

    >>> from scipy.interpolate import CubicSpline, PchipInterpolator
    >>> from scipy.interpolate import Akima1DInterpolator, IsotonicInterpolator
    ...
    >>> plt.plot(xx, IsotonicInterpolator(x, y)(xx), '-', label='isotonic')
