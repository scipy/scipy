Building SciPy for Contributors (advanced)
``````````````````````````````````````````

.. tip::

    Most developers should see instead :doc:`the main contributor building guide <contributor>`,
    as many of the steps described below can now be accomplished automatically
    via SciPy's Pixi workspace.
    This removes the need for contributors to keep track of development environments
    and installed dependencies, as running a Pixi task automatically installs and uses
    a suitable environment.

    For contributors who do not want to use Pixi or who require customised environments,
