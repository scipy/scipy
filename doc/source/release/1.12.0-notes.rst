...
- `scipy.integrate.trapz`, `scipy.integrate.cumtrapz`, and `scipy.integrate.simps` have
  ...
  and `scipy.integrate.simpson` and will be removed in SciPy 1.14.
- The deprecation of the ``tol`` argument of ``scipy.sparse.linalg.{gcrotmk,lgmres,minres,tfqmr}``
  is now consistent with the other iterative solvers and will be removed in SciPy 1.14
  in favour of ``rtol``. Further, the default value of ``atol`` for these functions is due
...
- The use of ``scale='raw'`` has been removed in favour of ``'scale=1'`` in
  `scipy.stats.iqr`.
