...
- The batching loops of `scipy.linalg.qr` and `scipy.linalg.eig` have been
  moved to C, providing a substantial speedup for batched input.
...
- It is now possible to compose ``Rotation``s and ``RigidTransform``s directly,
  by automatically promoting ``Rotation``s when the two are composed via
