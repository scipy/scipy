...
sphinx_inspect.isdescriptor = lambda obj: old_isdesc(obj) and not isinstance(
    obj, ua._Function
)
...
html_theme_options = {
    ...
    "icon_links": [
        ...
        },
        {
            ...
            # icon is defined in file `_static/custom-icons.js`
            "icon": "fa-custom fa-SciPy_Forum",
            "type": "fontawesome",
        },
    ],
    "logo": {
...
SphinxDocString._str_examples = _rng_html_rewrite(SphinxDocString._str_examples)
                    ...
                    if module_name and class_name and obj_name:
                        ...
                        for parent in inspect.getmro(cls)[1:]:
                            if obj_name in parent.__dict__:
                                ...
                                real_name = (
                                    f"{parent.__module__}.{parent.__name__}.{obj_name}"
                                )
                                break
            ...
            if qualname.split(".")[0] in newdist.__all__:
                if inspect.isfunction(obj):
                    if obj.__qualname__ != qualname:
                        # This is an inherited object; do not generate a stub
                        # file for it
                    ...
                    # Distribution-specific properties are defined by
                    # ._update_parameters, and their qualname is
                    # ._update_parameters.<locals>.<lambda>
                    if (
                        obj.fget.__qualname__ != qualname
                        and obj.fget.__name__ == qualname.split(".")[-1]
                    ):
                        # This is an inherited property; do not generate a stub
                        # file for it
                        app.config.autosummary_filename_map[f"{name}"] = (
