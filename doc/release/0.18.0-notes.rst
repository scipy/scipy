...
In the `scipy.optimize` module, the interpretation of the `maxfun` option
in `L-BFGS-B` based routines has changed.
...
function evaluations, the new strategy allows the current
iteration to finish, even if more than `maxfun` total evaluations are required.
