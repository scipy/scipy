...
`scipy.optimize.optimize._minimize_neldermead` to support bounds constraints
...
The ``rotation`` methods `from_rotvec` and `as_rotvec` now accept a `degrees`
...
An overhaul of the Mann-Whitney-Wilcoxon test, ``mannwhitneyu``, including
support n-dimensional input, an exact test method, and improved documentation.

The method of moments (``MM``) option is now available for
``rv_continuous.fit``.

Optional bounds on lambda may now be specified for ``boxcox`` and 
``boxcox_normmax``.

An implementation of the relative risk calculation is now available in
`scipy.stats.contingency.relative_risk`

Added the two-sample version of Cramer-von Mises test in
`scipy.stats.cramervonmises_2samp`.

An implementation of the Generalized Hyperbolic Distribution was added
in `scipy.stats.genhyperbolic`.
...
`scipy.stats.boostrap` has been added to allow estimation of the confidence
...
``mood``, and ``ranksums`` functions to allow one-tailed hypothesis testing.
...
The ``alternative`` parameter was added to `scipy.stats.ansari` to enable
specification of the alternative hypothesis. Likewise for ``linregress`` and
``spearmanr``.
...
The function `scipy.stats.mannwhitenyu` has been rewritten. There are two 
...
The new ``axis`` parameter of `scipy.stats.mannwhitenyu` allows vectorized
computation of the test statistic and p-values along a given axis of the
provided samples. The new parameter ``method`` enables control over the method
used for p-value calculation ("auto", "exact", or "asymptotic"). Note that the
exact calculation is based on the null distribution of the statistic derived
without ties.

The new function `scipy.stats.binomtest` replaces `scipy.stats.binom_test`. The
new function returns an object that allows confidence intervals for the
estimated proportion to be computed. Also, performance was improved from O(n)
to O(log(n)) by using binary search.
...
The new function `scipy.stats.barnard_exact` performs Barnard exact test for
2x2 contingency tables.

The new function `scipy.stats.differential_entropy` calculates the differential
entropy with five ``method`` values: ``{'vasicek', 'van es', 'ebrahimi',
'correa', 'auto'}``.

The new function `scipy.stats.page_trend_test` performs Page's test for ordered
alternatives.

The new function `scipy.stats.somersd` computes Somers' D, a measure of ordinal
...
A new `scipy.stats.qmc` module for quasi-Monte Carlo
...
Python-to-C++ transpiler, Pythran, which can be optionally disabled but is
enabled by default at build time.
...
* `#13302 <https://github.com/scipy/scipy/pull/13302>`__: BUG: Fix ndimage.morphology.distance_transform_\* argument handling
...
* `#14020 <https://github.com/scipy/scipy/pull/14020>`__: MAINT: modify np.int_ to np.int32 to make it the same for 32/64...
