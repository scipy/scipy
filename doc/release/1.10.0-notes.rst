...
==============================
- Added `scipy.integrate.qmc_quad`, which uses performs quadrature using
...
- `scipy.interpolate.interp1d` now can take a single value for non-spline
  methods.
- A new ``extrapolate`` argument is available to ``BSpline.design_matrix``,
  allowing extrapolation based on the first and last intervals.
- A new function ``make_smoothing_spline`` has been added. It is an
...
===========================
- ``lu_factor`` now accepts rectangular arrays instead of being restricted
  to square arrays.


`scipy.ndimage` improvements
============================
- The new ``value_indices()`` function provides a time-efficient method to
  search for the locations of individual values with an array of image data.
- A new ``radius`` argument is supported by ``gaussian_filter1d`` and
  ``gaussian_filter`` for adjusting the kernel size of the filter.
...
- some additional options (``parallel``, ``threads``, ``mip_rel_gap``) can now
  be properly passed to the HiGHS solver
...
- The new function `scipy.special.powm1`, a ufunc with signature
  ...
  1.
- ``erfinv`` is now more accurate as it leverages the Boost equivalent under
  the hood.
...
- Added `scipy.stats.oddsratio` to compute both the conditional and
...
- Improved the speed and/or accuracy of functions of several statistical
  ...
  * Added a parameter ``lower_limit`` to ``stats.multivariate_normal``,
...
- Improved the performance of `stats.hdquantile_sd`
- Added the ``scramble`` optional argument to `stats.qmc.LatinHypercube`. It
...
- Improved the performance of `scipy.stats.binned_statistics_dd` for several
  NumPy statistics, and binned statistics methods now support complex data.
- Added ``axis``, ``nan_policy``, and masked array support to
  `stats.jarque_bera`
- Added the ``nan_policy`` optional argument to `stats.rankdata`.
...
* Andrew Nelson (50)
* Nico Schlömer (1)
* Nikita Karetnikov (ニキータ カレートニコフ) (1) +
* NiMlr (1) +
