# numpy_interpolation._core  –  implementation
# Internal module. Public API is re-exported from numpy_interpolation/__init__.py
...
# ------------------------------------------------------------------
# 1e  B-spline helpers  (arbitrary-degree interpolation)
# ------------------------------------------------------------------

def _bspline_design_matrix(x_eval, t, k):
    """Collocation matrix: B[i, j] = N_{j,k}(x_eval[i]).

    Piegl & Tiller NURBS Book Algorithm A2.2 — correct at repeated knots.

    x_eval : (Q,) query points — clamped to [t[0], t[-1]] internally
    t      : knot vector (M = n+k+1 elements)
    k      : spline degree
    Returns (Q, n) where n = M-k-1.
    """
    nt = len(t)
    n  = nt - k - 1
    xc = np.clip(x_eval, t[0], t[-1])
    Q  = len(xc)
    B  = np.zeros((Q, n))
    ...
    for qi in range(Q):
        xq = xc[qi]
        # Find knot span: largest i such that t[i] <= xq < t[i+1]
        i = int(np.searchsorted(t, xq, side='right')) - 1
        i = min(max(i, k), n - 1)
...
def _bspline_interp(x, y, k):
    """Fit an interpolating B-spline of degree k through (x, y).

    Interior knots are placed using the Greville averaging formula:
        t_{k+j} = mean(x[j : j+k])  for j = 1, ..., n-k-1

    Returns
    -------
    t : (n+k+1,) knot vector
    c : (n,) or (n, m) B-spline coefficients
    """
    n    = len(x)
    y1d  = y.ndim == 1
    Y    = y[:, None] if y1d else y

    n_int = n - k - 1
    t_int = (np.array([x[j:j + k].mean() for j in range(1, n_int + 1)])
             if n_int > 0 else np.empty(0))
    t = np.concatenate([np.repeat(x[0], k + 1), t_int, np.repeat(x[-1], k + 1)])

    B = _bspline_design_matrix(x, t, k)
    try:
        c = np.linalg.solve(B, Y)
    except np.linalg.LinAlgError:
        c = np.linalg.lstsq(B, Y, rcond=None)[0]

    return t, (c[:, 0] if y1d else c)
...
def interp1d(x, y, kind='linear', axis=0, bounds_error=True,
    ...
    """
    # ------------------------------------------------------------------
    # Input validation + kind normalisation
    # ------------------------------------------------------------------
    _VALID_STR_KINDS = frozenset([
        'linear', 'nearest', 'nearest-up', 'zero', 'slinear',
        'quadratic', 'cubic', 'previous', 'next', 'lagrange', 'newton',
    ])
    if isinstance(kind, int):
        if kind < 0:
            raise ValueError(f"Integer kind must be >= 0, got {kind}")
    elif kind not in _VALID_STR_KINDS:
        raise ValueError(
            f"kind must be one of {sorted(_VALID_STR_KINDS)} "
            f"or a non-negative integer, got {kind!r}"
        )

    # Resolve to internal canonical tokens
    _bspline_deg = None
    if isinstance(kind, int):
        _bspline_deg = kind
        kind = {0: 'zero', 1: 'linear', 2: 'quadratic', 3: 'cubic'}.get(kind, '_bspline')
    else:
        if kind == 'slinear':   kind = 'linear'     # slinear == linear
        if kind == 'previous':  kind = 'zero'        # previous == left-step
        if kind == 'quadratic': _bspline_deg = 2
    ...
    # ------------------------------------------------------------------
    if kind == 'nearest':
        x_bds    = x[:-1] / 2.0 + x[1:] / 2.0   # midpoints (rounds-down at tie)

    elif kind == 'nearest-up':
        x_bds_up = x[:-1] / 2.0 + x[1:] / 2.0   # same midpoints, rounds-up at tie

    elif kind == 'cubic':
        c = _cubic_spline_coeffs(x, y)

    elif kind in ('quadratic', '_bspline'):
        t_bs, c_bs = _bspline_interp(x, y, _bspline_deg)
    ...
    # 'linear', 'zero', 'next' need no precomputation
    ...
    def _evaluate(x_new):
        ...
        if kind == 'linear':
            ...
            # Same midpoints but rounds UP at tie (side='right')
            idx = np.searchsorted(x_bds_up, xf, side='right').clip(0, n - 1)
            out = y[idx]

        elif kind == 'zero':
            # Left step / previous: return value of the left node
            idx = (np.searchsorted(x, xf, side='right') - 1).clip(0, n - 1)
            out = y[idx]

        elif kind == 'next':
            # Right step: return value of the next (right) node
            idx = np.searchsorted(x, xf, side='left').clip(0, n - 1)
            out = y[idx]
            ...
            B   = _bspline_design_matrix(xf, t_bs, _bspline_deg)
            c1d = c_bs.ndim == 1
            C   = c_bs[:, None] if c1d else c_bs
            res = B @ C
            out = res[:, 0] if c1d else res
...
# ------------------------------------------------------------------
# 4e  Natural Neighbor (Sibson) interpolation
# ------------------------------------------------------------------

def _circumcenter_2d(A, B, C):
    """Circumcenter of 2-D triangle (A, B, C). Returns inf vector if degenerate."""
    ax, ay = A[0], A[1]
    bx, by = B[0], B[1]
    cx, cy = C[0], C[1]
...
def _polygon_area_2d(verts):
    """Absolute area of a simple polygon (N×2) via the shoelace formula."""
    x, y = verts[:, 0], verts[:, 1]
    return 0.5 * abs(np.dot(x, np.roll(y, -1)) - np.dot(np.roll(x, -1), y))


def _in_circumcircle_2d(p0, p1, p2, q):
    """True iff q lies strictly inside the circumcircle of CCW triangle (p0,p1,p2).

    Uses the lifted-paraboloid determinant test (exact for floating-point).
    """
    ax = p0[0] - q[0];  ay = p0[1] - q[1];  az = ax*ax + ay*ay
    bx = p1[0] - q[0];  by = p1[1] - q[1];  bz = bx*bx + by*by
    cx = p2[0] - q[0];  cy = p2[1] - q[1];  cz = cx*cx + cy*cy
    return (ax*(by*cz - bz*cy) - bx*(ay*cz - az*cy) + cx*(ay*bz - az*by)) > 0.0


def _natural_2d(pts, vals, xi, fill_value=np.nan):
    """Sibson natural-neighbor 2-D interpolation.

    For each query point q:
      1. Find cavity triangles (circumcircle contains q) in the Delaunay triangulation.
      2. Natural neighbors = vertices on the cavity boundary.
      3. Sibson weight for neighbor nᵢ = area stolen from nᵢ's Voronoi cell by q.
         Stolen polygon vertices = circumcenters of old cavity triangles adjacent to nᵢ
         UNION circumcenters of new virtual triangles (q, nᵢ, nⱼ) per boundary edge.
    ...
    vals = np.asarray(vals, dtype=float)
    y1d  = vals.ndim == 1
    V    = vals[:, None] if y1d else vals
    m    = V.shape[1]

    tris = _delaunay_2d(pts)       # (T, 3), CCW
    T    = len(tris)

    # Pre-compute circumcenters of all triangles (original coordinate space)
    cc = np.array([_circumcenter_2d(pts[tris[t, 0]], pts[tris[t, 1]], pts[tris[t, 2]])
                   for t in range(T)])

    # Build edge → [triangle_indices] adjacency map
    edge_adj = {}
    for t_idx in range(T):
        ...
        for k in range(3):
            ...
            edge_adj.setdefault(e, []).append(t_idx)
    ...
    for qi in range(Q):
        ...
        # Exact hit on a data point → return its value directly
        dists = np.hypot(pts[:, 0] - q[0], pts[:, 1] - q[1])
        exact = np.where(dists < 1e-12)[0]
        if len(exact):
            out[qi] = V[exact[0]]
            continue

        # ---- cavity: triangles whose circumcircle strictly contains q ----
        in_cav = np.array([
            _in_circumcircle_2d(pts[tris[t, 0]], pts[tris[t, 1]], pts[tris[t, 2]], q)
            for t in range(T)
        ])
        cav_idx = np.where(in_cav)[0]
        if len(cav_idx) == 0:
            continue  # q is outside the convex hull

        # ---- boundary edges (cavity edges with one non-cavity neighbour) ----
        boundary = []   # list of (v0, v1)  — directed so that outer triangle exists
        for t_idx in cav_idx:
            tri = tris[t_idx]
            for k in range(3):
                v0, v1 = int(tri[k]), int(tri[(k+1) % 3])
                e = (min(v0, v1), max(v0, v1))
                for other in edge_adj[e]:
                    if not in_cav[other]:
                        boundary.append((v0, v1))
                        break

        if not boundary:
            continue

        # ---- natural neighbours: unique vertices on boundary ----
        nn = sorted({v for v0, v1 in boundary for v in (v0, v1)})

        # ---- stolen-area polygon per natural neighbour ----
        stolen = {}
        for ni in nn:
            poly = []

            # Old circumcenters from cavity triangles touching ni
            for t_idx in cav_idx:
                tri = tris[t_idx]
                if ni in (int(tri[0]), int(tri[1]), int(tri[2])):
                    ccv = cc[t_idx]
                    if not np.any(np.isinf(ccv)):
                        poly.append(ccv)

            # New circumcenters from virtual triangles (q, ni, nj) per boundary edge
            for v0, v1 in boundary:
                if ni in (v0, v1):
                    nj = v1 if v0 == ni else v0
                    ccv = _circumcenter_2d(q, pts[ni], pts[nj])
                    if not np.any(np.isinf(ccv)):
                        poly.append(ccv)
            ...
            poly = poly[np.argsort(ang)]   # sort CCW around centroid
            stolen[ni] = _polygon_area_2d(poly)

        total = sum(stolen.values())
        if total < 1e-14:
            continue

        weights = np.array([stolen[ni] / total for ni in nn])
        for k in range(m):
            out[qi, k] = np.dot(weights, V[nn, k])

    return out[:, 0] if y1d else out


# ------------------------------------------------------------------
# 4f  MATLAB v4 – biharmonic thin-plate spline (global RBF)
# ------------------------------------------------------------------

def _v4_2d(pts, vals, xi):
    """MATLAB v4 biharmonic thin-plate spline (radial basis function) interpolation.

    Basis:  φ(r) = r² log r,  with φ(0) = 0
    System: [Φ  P ] [w]   [f]
            [Pᵀ 0 ] [c] = [0]
    where Φᵢⱼ = rᵢⱼ² log rᵢⱼ and P = [1, xᵢ, yᵢ].

    Globally smooth, reproduces linear polynomials exactly, no fill_value
    (extrapolates naturally via the RBF).
    ...
    # φ(r) = r² log r,  φ(0) = 0
    with np.errstate(divide='ignore', invalid='ignore'):
        Phi = np.where(r2 == 0.0, 0.0,
                       r2 * np.log(np.sqrt(np.where(r2 == 0.0, 1.0, r2))))

    # Polynomial part: P = [1, x, y]  (N × 3)
    P = np.column_stack([np.ones(n), pts[:, 0], pts[:, 1]])

    # Build (N+3) × (N+3) saddle-point system
    A       = np.zeros((n + 3, n + 3))
    A[:n, :n] = Phi
    A[:n, n:] = P
    A[n:, :n] = P.T
    ...
    w = coeffs[:n]   # (N, m)  RBF weights
    c = coeffs[n:]   # (3, m)  polynomial coefficients [const, dx, dy]

    # Evaluate at query points  (Q × N distances)
    dx_q = xi[:, 0][:, None] - pts[:, 0][None, :]
    dy_q = xi[:, 1][:, None] - pts[:, 1][None, :]
    r2_q = dx_q*dx_q + dy_q*dy_q

    with np.errstate(divide='ignore', invalid='ignore'):
        Phi_q = np.where(r2_q == 0.0, 0.0,
                         r2_q * np.log(np.sqrt(np.where(r2_q == 0.0, 1.0, r2_q))))

    Pq  = np.column_stack([np.ones(len(xi)), xi[:, 0], xi[:, 1]])  # (Q, 3)
    out = Phi_q @ w + Pq @ c                                         # (Q, m)
...
def griddata(points, values, xi, method='linear', fill_value=np.nan,
    ...
    method : {'linear', 'nearest', 'cubic', 'natural', 'v4'}
        ...
        * 'natural'  – Sibson natural-neighbor; stolen-Voronoi-area weights;
                       C1 everywhere except at data points; fill_value outside
                       the convex hull.
        * 'v4'       – MATLAB v4 biharmonic thin-plate spline (global RBF,
                       basis φ(r)=r²log r); smooth everywhere; extrapolates.
    ...
    if method == 'nearest':
        ...
        if points.shape[1] != 2:
            raise ValueError("natural griddata is only supported for 2-D data")
        out_flat = _natural_2d(points, values, xi_flat, fill_value)

    elif method == 'v4':
        if points.shape[1] != 2:
            raise ValueError("v4 griddata is only supported for 2-D data")
        out_flat = _v4_2d(points, values, xi_flat)

    else:
        raise ValueError(
            f"method must be 'nearest', 'linear', 'cubic', 'natural', or 'v4', "
            f"got {method!r}"
        )
