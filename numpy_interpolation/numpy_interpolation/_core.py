...
def interp1d(x, y, kind='linear', axis=0, bounds_error=True,
    ...
    precision : int or None
        ...
        Use precision=40 for 10-32 accuracy; combine with string inputs and
    ...
    if kind == 'nearest':
        ...
        # Not-a-knot C2 spline coefficients via Thomas algorithm
...
def _delaunay_2d(pts):
    ...
    # Normalise to [0,1]^2 for numerical stability
    lo    = pts.min(0)
    hi    = pts.max(0)
    span  = hi - lo
    span[span < 1e-12] = 1.0
...
def _natural_2d(pts, vals, xi, fill_value=np.nan):
    ...
    For each query point q:
      ...
      3. Sibson weight for neighbor ni = area stolen from ni's Voronoi cell by q.
         Stolen polygon vertices = circumcenters of old cavity triangles adjacent to ni
         UNION circumcenters of new virtual triangles (q, ni, nj) per boundary edge.
...
def _v4_2d(pts, vals, xi):
    ...
    Basis:  phi(r) = r^2 log r,  with phi(0) = 0
    System: [Phi  P ] [w]   [f]
            [P^T  0 ] [c] = [0]
    where Phi_ij = r_ij^2 log r_ij and P = [1, x_i, y_i].
    ...
    # Polynomial part: P = [1, x, y]  (N x 3)
    P = np.column_stack([np.ones(n), pts[:, 0], pts[:, 1]])

    # Build (N+3) x (N+3) saddle-point system
    A       = np.zeros((n + 3, n + 3))
    A[:n, :n] = Phi
    A[:n, n:] = P
    A[n:, :n] = P.T
    ...
    # Evaluate at query points  (Q x N distances)
...
def griddata(points, values, xi, method='linear', fill_value=np.nan,
    ...
    method : {'linear', 'nearest', 'cubic', 'natural', 'v4'}
        ...
        * 'v4'       - MATLAB v4 biharmonic thin-plate spline (global RBF,
                       basis phi(r)=r^2*log r); smooth everywhere; extrapolates.
    fill_value : float, default np.nan
        Value for queries outside the convex hull ('linear', 'natural').
